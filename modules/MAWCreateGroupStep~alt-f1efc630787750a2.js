__d("MAWCreateGroupStep", [
	...
	async function p(t, n, a, i, l) {
		...
		var p = await _(a, m, n, c, i == null ? void 0 : i.deduplicationKey);
		...
		var f = p.value.group;
		c.addPoint("start_create_group"), await o("WACreateGroup").createGroups([{
			extras: i,
			folder: null,
			groupStatus: "added",
			groupToCreate: f,
		...
		var g = f.participants.map(function(e) {
		...
		}), c.addPoint("persist_get_devices_create_group"), await o("MAWCreateGroupInvitesStep").createGroupInvitesStep(f.jid, (u = f.creator) != null ? u : o("WAGlobals").getMyUserJid(), f.participants.map(function(e) {
		...
		var h = await o("MAWConvertCreateGroupResultApi").convertCreateGroupResult(f);
		return c.endSuccess(), o("WAResultOrError").makeResult(h);
	}
	async function _(e, t, n, a, i) {
		var l;
		a.addPoint("graphql_request_start");
		var s = o("WAGlobals").getMyUserJid(), p = Array.from(new Set([].concat(t, [s]))), _ = i != null ? i : o("MAWExternalId").generateExternalId(), f = Array.from(new Set(e)), g = await o("E2EEMetadataMailboxCreateGroupThreadMutation").createMISOTGroupThreadOverGraphQL({
		...
		if ((h == null ? void 0 : h.success) === !0 && h.gid != null) {
			...
			if (h.participants != null) {
				...
				for (var S of h.participants) await v();
			}
