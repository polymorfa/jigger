__d("MAWShareAttachment", [
	...
	var d = 3e5;
	async function m(t, n, a, i, l, u, m, p, _, f, g, h, y, C, b, v, S, R, L, E, k, I, T) {
		var D, x, $, P, N, M, w, A, F, O, B, W, q, U, V;
		if (t != null) try {
			await o("MAWWaitForBackendSetup").waitForBackendSetup("fetch-devices-and-share-xma");
		} catch (e) {
		...
		var G = await o("MAWGetEphemeralSettings").getEphemeralSetting(n, b);
		...
		if (b != null) {
			var X = null;
			l != null && (X = await o("MAWImagePreProcess").optimisticImagePreprocess(l.file)), await I({
		...
		return await o("MAWMessageSendReporter").MAWMessageSendReporter({
			chatJid: n,
			externalId: E
		}, Y, {
			qplEventType: r("qpl")._(25313175, "1551"),
	...
	async function p(t, n, a, i, l, s, c, d, p, _, f, g, h, y) {
		var C, b, v, S, R, L, E, k, I, T, D, x, $ = l, P = await (u || (u = o("LSDatabaseSingleton"))).LSDatabaseSingleton, N = {
			linkType: "unknown",
		...
		if (d == null && t != null) {
			...
			try {
				o("sendToSentQPLLogger").markSendToSentPoint($, "xma_data_query_start"), c.addMarkerPoint("xma_data_query_start", "AppTiming"), p != null ? N = await p : N = await o("MAWFetchXMAData").fetchXMAData(t, M, [$]), o("sendToSentQPLLogger").markSendToSentPoint($, "xma_data_query_end"), c.addMarkerPoint("xma_data_query_end", "AppTiming");
			} catch (e) {
		...
		var K = y != null ? await y : await o("MAWFetchXMADataUtils").makeXMAFileArgs(N, $), Q = K.faviconFileArgs, X = K.headerImageFileArgs, Y = K.previewFileArgs;
		if (o("sendToSentQPLLogger").addSendToSentAnnotations(l, { string: {
			...
			if (a.isTargetE2E === !0) {
				...
				var Z = h != null ? await h : await o("sendToSentQPLLogger").measureSendToSentPerformance("maybe_convert_preview_gif_to_thumbnail", $, function() {
					return o("MAWImageUtils").generatePreviewBlobForShare(Y);
				});
				o("sendToSentQPLLogger").markSendToSentPoint($, "fetch_existing_chat_jid_start"), await o("sendToSentQPLLogger").measureSendToSentPerformance("maybe_load_existing_thread", $, function() {
					return r("MAWMiActMaybeCreateActThread")(P, J, "shareSecureAttachment", $);
				});
				var ee = await o("MAWMiActOnActThreadReady").waitForACTThreadReady(P.tables, J, "fetch-existing-chatJid", $), te = ee.chatJid, ne = ee.serverThreadKey, re, oe = o("WAJids").interpretAsUserJid(te);
				return oe != null ? re = [oe] : re = await o("MAWFetchUserJids").fetchUserJids(P, n.targetID), o("sendToSentQPLLogger").markSendToSentPoint($, "fetch_existing_chat_jid_end"), m(re, te, n, Q, Z != null ? Z : void 0, X, O, B, U, V, H, W, i, $, ne, z, s, P, _, f, F, g, c);
