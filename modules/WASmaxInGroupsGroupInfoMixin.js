__d("WASmaxInGroupsGroupInfoMixin", [
	...
	function s(e) {
		...
		if (!t.success) return t;
		var n = o("WASmaxInGroupsGroupInfoDescriptionMixin").parseGroupInfoDescriptionMixin(e);
		return o("WAResultOrError").makeResult({ groupInfoDescriptionMixin: n.success ? n.value : null });
	}
	function u(e) {
	...
	function d(e) {
		...
		if (!t.success) return t;
		var n = o("WASmaxParseUtils").optionalLiteral(o("WASmaxParseUtils").attrString, e, "can_auto_file", "true");
		return n.success ? o("WAResultOrError").makeResult({ canAutoFile: n.value }) : n;
	}
	function m(e) {
		var t = o("WASmaxParseUtils").assertTag(e, "appeal_status");
		if (!t.success) return t;
		var n = o("WASmaxParseUtils").attrStringEnum(e, "type", o("WASmaxInGroupsEnums").ENUM_APPROVED_INREVIEW_NONE_REJECTED);
	...
	function _(e) {
		...
		if (!t.success) return t;
		var n = o("WASmaxParseUtils").literal(o("WASmaxParseUtils").attrString, e, "type", "invite");
		if (!n.success) return n;
	...
	function f(e) {
		...
		return o("WAResultOrError").makeResult({ membershipApprovalGroupJoinModeEnabledMixin: n.success ? n.value : null });
	}
	function g(e) {
	...
	function y(e) {
		...
		if (!t.success) return t;
		var n = o("WASmaxParseUtils").optional(o("WASmaxParseUtils").attrIntRange, e, "trigger", 0, 20);
		return n.success ? o("WAResultOrError").makeResult({ trigger: n.value }) : n;
	}
	function C(t) {
		...
		if (!R.success) return R;
		var L = o("WASmaxParseUtils").optionalChildWithTag(t, "appeal_update_time", p);
		if (!L.success) return L;
		...
		var I = o("WASmaxParseUtils").optionalChildWithTag(t, "membership_approval_mode", f);
		if (!I.success) return I;
		var T = o("WASmaxParseUtils").optionalChild(t, "incognito");
		if (!T.success) return T;
		var D = o("WASmaxParseUtils").optionalChild(t, "allow_admin_reports");
		if (!D.success) return D;
		var x = o("WASmaxParseUtils").optionalChild(t, "allow_non_admin_sub_group_creation");
		...
		if (!A.success) return A;
		var F = o("WASmaxParseUtils").optionalChildWithTag(t, "evolution_version", h);
		if (!F.success) return F;
		var O = o("WASmaxParseUtils").optionalChild(t, "group_safety_check");
		if (!O.success) return O;
		var B = o("WASmaxParseUtils").optionalChild(t, "participant_label_enabled");
		if (!B.success) return B;
		var W = o("WASmaxParseUtils").optionalChildWithTag(t, "limit_sharing_enabled", y);
		if (!W.success) return W;
		var q = o("WASmaxParseUtils").optionalLiteral(o("WASmaxParseUtils").attrString, t, "ack", "false");
		if (!q.success) return q;
		var U = o("WASmaxInGroupsGroupInfoAttributesMixin").parseGroupInfoAttributesMixin(t);
		if (!U.success) return U;
		var V = o("WASmaxInGroupsGroupMemberAddModeMixin").parseGroupMemberAddModeMixin(t), H = o("WASmaxInGroupsGroupMemberLinkModeMixin").parseGroupMemberLinkModeMixin(t), G = o("WASmaxInGroupsGroupMemberShareGroupHistoryModeMixin").parseGroupMemberShareGroupHistoryModeMixin(t), z = o("WASmaxParseUtils").mapChildrenWithTag(t, "participant", 0, 19999, e);
		return z.success ? o("WAResultOrError").makeResult(babelHelpers.extends({ ack: q.value }, U.value, {
	...
	l.parseGroupInfoParticipant = e, l.parseGroupInfoDescription = s, l.parseGroupInfoParent = u, l.parseGroupInfoEphemeral = c, l.parseGroupInfoSuspended = d, l.parseGroupInfoAppealStatus = m, l.parseGroupInfoAppealUpdateTime = p, l.parseGroupInfoGrowthLocked = _, l.parseGroupInfoMembershipApprovalMode = f, l.parseGroupInfoLinkedParent = g, l.parseGroupInfoEvolutionVersion = h, l.parseGroupInfoLimitSharingEnabled = y, l.parseGroupInfoMixin = C;
}), 98);
