__d("WASmaxInWaffleForceSuspendStateErrors", [
	"WAResultOrError",
	...
	function e(e) {
		...
		var r = o("WASmaxInWaffleIQErrorInternalServerErrorMixin").parseIQErrorInternalServerErrorMixin(e);
		if (r.success) return o("WAResultOrError").makeResult({
			name: "IQErrorInternalServerError",
			value: r.value
		});
		var a = o("WASmaxInWaffleIQErrorServiceUnavailableMixin").parseIQErrorServiceUnavailableMixin(e);
		if (a.success) return o("WAResultOrError").makeResult({
			name: "IQErrorServiceUnavailable",
			value: a.value
		});
		var i = o("WASmaxInWaffleIQErrorRateOverlimitMixin").parseIQErrorRateOverlimitMixin(e);
		if (i.success) return o("WAResultOrError").makeResult({
			name: "IQErrorRateOverlimit",
			value: i.value
		});
		var l = o("WASmaxInWaffleIQErrorNotAllowedMixin").parseIQErrorNotAllowedMixin(e);
		if (l.success) return o("WAResultOrError").makeResult({
			name: "IQErrorNotAllowed",
			value: l.value
		});
		var s = o("WASmaxInWaffleIQErrorConflictMixin").parseIQErrorConflictMixin(e);
		if (s.success) return o("WAResultOrError").makeResult({
			name: "IQErrorConflict",
			value: s.value
		});
		var u = o("WASmaxInWaffleIQErrorWFNotFoundMixin").parseIQErrorWFNotFoundMixin(e);
		if (u.success) return o("WAResultOrError").makeResult({
			name: "IQErrorWFNotFound",
			value: u.value
		});
		var c = o("WASmaxInWaffleIQErrorWFStateMismatchMixin").parseIQErrorWFStateMismatchMixin(e);
