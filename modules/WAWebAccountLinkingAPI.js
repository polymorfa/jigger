__d("WAWebAccountLinkingAPI", [
	...
	function x() {
		return x = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, n) {
			try {
				var a = yield o("WAWebDirectConnectionX509").extractCertificates(e), i = yield o("WAWebAccountLinkingCryptoUtils").validateCertificateChain(a);
				if (i != null) {
					var l = yield i.getPublicKey({ algorithm: {
						algorithm: {
							name: "RSA-OAEP",
							hash: { name: "SHA-1" }
						},
					...
					return t != null && (u = yield o("WAWebAccountLinkingCryptoUtils").importPasswordPublicKey(t)), {
						encryptionKey: l,
						passwordPublicKey: u,
	...
	function P() {
		return P = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			try {
				var t = yield o("WASmaxWaffleGetCertificateRPC").sendGetCertificateRPC({
					hasPasswordPem: !0,
				...
				if (t.name === "GetCertificateResponseSuccess") {
					...
					if (i != null) {
						var l = String.fromCharCode.apply(null, i), s = yield o("WAWebDirectConnectionX509").extractCertificates(l), p = yield o("WAWebAccountLinkingCryptoUtils").validateCertificateChain(s);
						if (p != null) {
							var _, f, g = yield p.getPublicKey({ algorithm: {
								algorithm: {
									name: "RSA-OAEP",
									hash: { name: "SHA-1" }
								},
							...
							if (b != null) try {
								C = String.fromCharCode.apply(null, b.elementValue), h = yield o("WAWebAccountLinkingCryptoUtils").importPasswordPublicKey(C), y = b.keyId;
							} catch (e) {
							...
							return yield o("WAWebWaffleCertificateCache").saveCertToIDB(l, C, y, v), {
								encryptionKey: g,
								passwordPublicKey: h,
								passwordKeyId: y
							};
	...
	var N = o("WAWebWaffleIQErrorHandler").createWaffleOperationRetryState(), M = o("WAWebWaffleIQErrorHandler").createWaffleOperationRetryState(), w = null;
	function A(e, t) {
	...
	function F() {
		return F = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			e: {
				...
				if (e === "purge") {
					return o("WAWebMetaAiWaffleAuthTokenCache").clearMetaAiWaffleAuthTokenBlobCache(), yield L.purgeWaffleData(), !0;
					break e;
				}
				if (e === "pause") {
					return yield o("WAWebAccountLinkingHandler").handlePausedState(), !0;
					break e;
				}
				if (e === "server_purge") {
					var i = yield j();
					return i && (o("WAWebMetaAiWaffleAuthTokenCache").clearMetaAiWaffleAuthTokenBlobCache(), yield L.purgeWaffleData()), i;
					break e;
				}
				if (e === "server_pause") {
					var l = yield Q();
					return l && (yield o("WAWebAccountLinkingHandler").handlePausedState()), l;
					break e;
				}
	...
	function B() {
		return B = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			var e = w;
			return e != null ? (o("WAWebWaffleLifecycleWamLogger").logRefreshToken({ traceAction: o("WAWebWamEnumWaffleLifecycleTraceActionType").WAFFLE_LIFECYCLE_TRACE_ACTION_TYPE.REFRESH_TOKEN_DEDUPLICATED }), e) : (w = W().finally(function() {
				w = null;
			}), w);
	...
	function q() {
		return q = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			var n = t.fbid, a = t.nonce, i = yield o("WAWebAccountLinkingCryptoUtils").generateRSAKeys(), l = i.privateKey, s = i.publicKey, u = yield o("WAWebAccountLinkingCryptoUtils").cryptoKeyToPem(s, !0), c = {
				version: 1,
				timestamp: Date.now(),
				nonce: a,
				client_pub_key: u,
				client_pub_key_type: "RSA 2048"
			}, d = yield o("WAWebAccountLinkingCryptoUtils").wrapPayloadWithRSAAESEncryption(c);
			if (n != null) {
				var m = yield o("WASmaxWaffleRefreshAccessTokensRPC").sendRefreshAccessTokensRPC({
				...
				if (m.name === "RefreshAccessTokensResponseSuccess") {
					...
					try {
						var b = yield o("WAWebAccountLinkingCryptoUtils").decryptRSAEncryptedPayload(l, h, g, y, C);
						if ("access_token" in b) return yield L.updateAccesstoken(b.access_token), o("WAWebMetaAiWaffleAuthTokenCache").refreshMetaAiWaffleAuthTokenBlob(), o("WAWebWaffleLifecycleWamLogger").logRefreshToken({
						...
						o("WALogger").ERROR(p || (p = babelHelpers.taggedTemplateLiteralLoose(["[WAFFLE] Failed to refresh access token"]))).catching(r("getErrorSafe")(t)), U(e, !1);
					}
				} else {
					var v = m.value.errorRefreshAccessTokensErrors, S = yield o("WAWebWaffleIQErrorHandler").handleCommonWaffleIQError("refreshAccessToken", v.name);
					...
					}), yield A(S, N), o("WALogger").ERROR(_ || (_ = babelHelpers.taggedTemplateLiteralLoose(["[WAFFLE] Refresh access token RPC failed: ", ""])), v.name);
				}
			} else U(e, !1);
			return !1;
	...
	function H() {
		return H = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			k("ping");
			var e = Date.now(), t = yield L.getAccountLinkingData();
			if (t != null) {
				...
				if (n != null) {
					var a = {
						...
						timestamp: Date.now(),
						access_token: n
					}, i = yield o("WAWebAccountLinkingCryptoUtils").wrapPayloadWithRSAAESEncryption(a);
					if (r != null) {
						...
						if (l.name === "WFPingResponseSuccess") {
							...
							yield L.updatePingInterval(s), o("WAWebWaffleLifecycleWamLogger").logPing({
								elapsedMs: Date.now() - e,
								hasAccessToken: !0
							});
						} else {
							var u = l.value.errorWfPingErrors, c = yield o("WAWebWaffleIQErrorHandler").handleCommonWaffleIQError("ping", u.name);
							o("WAWebWaffleLifecycleWamLogger").logPing({
								...
								hasAccessToken: !0
							}), yield A(c, M), o("WALogger").ERROR(f || (f = babelHelpers.taggedTemplateLiteralLoose(["[WAFFLE] Ping failed: ", ""])), u.name);
						}
	...
	function z() {
		return z = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			k("stateExists");
			var e = yield o("WASmaxWaffleStateExistsRPC").sendStateExistsRPC({ timestampElementValue: Date.now() });
			if (e.name === "StateExistsResponseSuccess") {
				...
				yield o("WAWebWaffleIQErrorHandler").handleCommonWaffleIQError("stateExists", n.name), o("WALogger").ERROR(y || (y = babelHelpers.taggedTemplateLiteralLoose(["[WAFFLE] StateExists RPC failed: ", ""])), n.name);
			}
	...
	function K() {
		return K = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			k("forceDeleteState");
			var e = yield o("WASmaxWaffleForceDeleteStateRPC").sendForceDeleteStateRPC({ timestampElementValue: Math.floor(Date.now() / 1e3) });
	...
	function X() {
		return X = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			k("forceSuspendState");
			var e = yield o("WASmaxWaffleForceSuspendStateRPC").sendForceSuspendStateRPC({ timestampElementValue: Math.floor(Date.now() / 1e3) });
	...
	function J() {
		return J = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			if (e != null) {
				...
				try {
					n = yield o("WAWebRelayClient").commitMutation(r("WAWebWaffleFXServiceDataQueryV2Mutation"), {}, { accessToken: t });
				} catch (e) {
	...
	l.assertModeAllowed = k, l.fetchValidCertificate = I, l.handleRecoveryAction = A, l.refreshAccessToken = O, l.ping = V, l.stateExists = G, l.forceDeleteState = j, l.forceSuspendState = Q, l.fetchServiceData = Y;
}), 98);
