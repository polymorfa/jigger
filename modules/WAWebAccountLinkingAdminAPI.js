__d("WAWebAccountLinkingAdminAPI", [
	...
	function I() {
		return I = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			...
			var g = yield o("WAWebAccountLinkingCryptoUtils").encryptPassword(m, f, _), h = yield o("WAWebAccountLinkingCryptoUtils").generateRSAKeys(), y = h.privateKey, C = h.publicKey, b = yield o("WAWebAccountLinkingCryptoUtils").cryptoKeyToPem(C, !0), v = {
				version: 1,
				timestamp: Date.now(),
				password: g,
				client_pub_key: b,
				client_pub_key_type: "RSA 2048"
			}, L = yield o("WAWebAccountLinkingCryptoUtils").wrapPayloadWithRSAAESEncryption(v, p.encryptionKey), E = yield o("WASmaxWaffleGenerateWAEntACUserRPC").sendGenerateWAEntACUserRPC({
			...
			if (E.name === "GenerateWAEntACUserResponseSuccess") {
				...
				try {
					var $ = yield o("WAWebAccountLinkingCryptoUtils").decryptRSAEncryptedPayload(y, T, I, D, x);
					"fbid" in $ ? yield S.updateEntCreationData(String($.fbid), g) : o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose(["[WAFFLE] GenerateWAEntACUser success, no fbid"])));
				} catch (e) {
				...
				var P = E.value.errorGenerateWaEntAcUserErrors, N = yield o("WAWebWaffleIQErrorHandler").handleCommonWaffleIQError("generateWAEntACUser", P.name);
				yield o("WAWebAccountLinkingAPI").handleRecoveryAction(N, R), o("WALogger").ERROR(d || (d = babelHelpers.taggedTemplateLiteralLoose(["[WAFFLE] GenerateWAEntACUser RPC failed: ", ""])), P.name);
			}
	...
	function D() {
		return D = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			var r = yield o("WAWebAccountLinkingCryptoUtils").generateRSAKeys(), a = r.privateKey, i = r.publicKey, l = yield o("WAWebAccountLinkingCryptoUtils").cryptoKeyToPem(i, !0), s = {
				version: 1,
				timestamp: Math.floor(Date.now() / 1e3),
				password: t,
				client_pub_key: l,
				client_pub_key_type: "RSA 2048"
			}, u = yield o("WAWebAccountLinkingCryptoUtils").wrapPayloadWithRSAAESEncryption(s), c = yield o("WAWebAccountLinkingCryptoUtils").computeIdSign(n), d = yield o("WASmaxWaffleGenerateAccessTokensRPC").sendGenerateAccessTokensRPC({
				rSAEncryptionMetadataMixinArgs: {
			...
			if (d.name === "GenerateAccessTokensResponseSuccess") {
				...
				try {
					var R = yield o("WAWebAccountLinkingCryptoUtils").decryptRSAEncryptedPayload(a, C, y, b, v);
				...
				var k = d.value.errorGenerateAccessTokensErrors, I = yield o("WAWebWaffleIQErrorHandler").handleCommonWaffleIQError("generateAccessTokens", k.name);
				yield o("WAWebAccountLinkingAPI").handleRecoveryAction(I, L), o("WALogger").ERROR(g || (g = babelHelpers.taggedTemplateLiteralLoose(["[WAFFLE] GenerateAccessTokens RPC failed: ", ""])), k.name);
			}
	...
	function N() {
		return N = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, n) {
			...
			var s = yield o("WAWebAccountLinkingCryptoUtils").generateRSAKeys(), u = s.privateKey, c = s.publicKey, d = yield o("WAWebAccountLinkingCryptoUtils").cryptoKeyToPem(c, !0), m = {
				version: 1,
				timestamp: Math.floor(Date.now() / 1e3),
				access_token: i,
				client_pub_key: d,
				...
				family_device_id: n
			}, p = yield o("WAWebAccountLinkingCryptoUtils").wrapPayloadWithRSAAESEncryption(m), _;
			try {
				_ = yield o("WASmaxWaffleEncryptedPayloadRequestRPC").sendEncryptedPayloadRequestRPC({
			...
			if (_.name === "EncryptedPayloadRequestResponseSuccess") {
				var f = o("WAWebAPIParser").parseRSAEncryptionMetadataMixin(_.value.encryptionMetadataRSAEncryptionMetadataMixin), g = f.data, R = f.key, L = f.nonce, k = f.tag, I = yield o("WAWebAccountLinkingCryptoUtils").decryptRSAEncryptedPayload(u, R, g, L, k);
				if ("link_mutation_succeeded" in I && I.success === !0 && I.link_mutation_succeeded === !0) yield S.updateAccountLinkingState(o("WAWebAccountLinkingConstants").AccountLinkState.Active), o("WAWebMetaAiWaffleAuthTokenCache").refreshMetaAiWaffleAuthTokenBlob();
				else {
					var T = "success" in I ? I.success : void 0, D = "link_mutation_succeeded" in I ? I.link_mutation_succeeded : void 0;
					throw o("WALogger").ERROR(b || (b = babelHelpers.taggedTemplateLiteralLoose([
						...
						""
					])), String(T), String(D)).sendLogs("waffle-linking-mutation-failed"), r("err")("Linking mutation failed");
				}
			} else {
				var P = _.value.errorEncryptedPayloadRequestErrors, N = yield o("WAWebWaffleIQErrorHandler").handleCommonWaffleIQError("linkAction", P.name);
				throw yield o("WAWebAccountLinkingAPI").handleRecoveryAction(N, E), o("WALogger").ERROR(v || (v = babelHelpers.taggedTemplateLiteralLoose(["[WAFFLE] Linking mutation RPC error: ", ""])), P.name).sendLogs("waffle-linking-mutation-rpc-error"), r("err")("Linking mutation RPC error: %s", P.name);
			}
		}), N.apply(this, arguments);
	}
	l.generateWAEntACUser = k, l.generateAccessTokens = T, l.sendLinkingMutation = P;
}), 98);
