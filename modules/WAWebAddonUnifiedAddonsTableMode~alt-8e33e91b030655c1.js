__d("WAWebAddonUnifiedAddonsTableMode", [
	...
	var e = function(t) {
		...
		bulkGetByParentMsgKey: async function(n) {
			var t = await o("WAWebSchemaUnifiedAddons").getTable().anyOf(["targetMsgKey"], n.map(function(e) {
				return e.toString();
			}));
			return e(t);
		},
		bulkGetByChatWid: async function(n) {
			var t = await o("WAWebSchemaUnifiedAddons").getTable().anyOf(["arbitraryIndex"], n.map(function(e) {
				return e.toString();
			}));
			return e(t);
		},
		bulkRemoveByMsgKey: async function(t) {
			await o("WAWebSchemaUnifiedAddons").getTable().bulkRemove(t.map(function(e) {
				return e.toString();
			}));
		},
		getByMsgKey: async function(n) {
			var t = await o("WAWebSchemaUnifiedAddons").getTable().get(n.toString());
			return t == null ? null : e([t])[0];
		},
		updateAck: async function(t, n) {
			await o("WAWebSchemaUnifiedAddons").getTable().merge(t.id.toString(), { ack: n });
		},
