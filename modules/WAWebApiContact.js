__d("WAWebApiContact", [
	...
	function I() {
		return I = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t;
			if (E != null) return o("WALogger").LOG(s || (s = babelHelpers.taggedTemplateLiteralLoose(["warmUpAllLidPnMappings: already warmed up"]))), E.promise;
			var n = self.performance.now();
			E = new (o("WAResolvable")).Resolvable();
			try {
				var a = e;
				a == null ? (a = yield o("WAWebSchemaContact_DO_NOT_USE_DIRECTLY").getContactTable().all(), o("WALogger").LOG(u || (u = babelHelpers.taggedTemplateLiteralLoose([
					"warmUpAllLidPnMappings: queried DB, ",
					" records in ",
					""
				])), a.length, o("WAWebPerformanceUtils").getElapsedTimeMs(n))) : o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose(["warmUpAllLidPnMappings: warming up ", " contacts"])), a.length);
				for (var i of a) {
					var l = i.id, p = i.phoneNumber, _ = i.phoneNumberCreatedAt;
					r("WAWebWid").isStringLid(l) && p != null && _ != null && L(o("WAWebWidFactory").createUserLidOrThrow(l), o("WAWebWidFactory").createUserWidOrThrow(p), _);
				}
			} catch (e) {
				o("WALogger").ERROR(d || (d = babelHelpers.taggedTemplateLiteralLoose(["warmUpAllLidPnMappings: error"]))).sendLogs("warmup-all-lid-pn-mappings-error");
			}
			o("WALogger").LOG(m || (m = babelHelpers.taggedTemplateLiteralLoose(["warmUpAllLidPnMappings: finished in ", ""])), o("WAWebPerformanceUtils").getElapsedTimeMs(n)), (t = E) == null || t.resolve();
	...
	function B() {
		return B = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			if (t != null) {
				t === o("WAWebProtobufsAdv.pb").ADVEncryptionType.HOSTED && (o("WALogger").LOG(p || (p = babelHelpers.taggedTemplateLiteralLoose(["updateContactAdvHostedType: check coex cache for ", ""])), e == null ? void 0 : e.toLogString()), o("WAWebBizCoexHostedAddVerification").assertThrowsWidAdvTypeFromVerificationCache(e));
				var n = o("WAWebWidToJid").widToUserJid(e), a = t === o("WAWebProtobufsAdv.pb").ADVEncryptionType.HOSTED;
				yield r("WAWebLidAwareContactsDB").merge(n, a ? {
					isHosted: !0,
					isOrHasBeenHosted: !0
				} : { isHosted: !1 }).catch(function(e) {
					throw o("WALogger").ERROR(_ || (_ = babelHelpers.taggedTemplateLiteralLoose(["updateContactAdvHostedTypefailed"]))).verbose(), e;
				});
			}
	...
	function q(t, n) {
		...
		t.forEach(function(e) {
			e.isBot() || e.isHosted() || e.isLid() || r.add(o("WAWebWidFactory").asUserWidOrThrow(e).toString());
		});
		var a = new Set();
		...
		a.size > 0 && o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose([
			"LID null - ",
			" PNs, missing: ",
			", caller: ",
			""
		])), r.size, a.size, i).sendLogs("lidInfraAccount-" + i, { sampling: 0 });
	}
	var U = 1e4, V = null;
	function H() {
	...
	function Z() {
		return Z = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			yield r("WAWebLidAwareContactsDB").bulkCreateOrMerge(t, "ApiContact.updateContactsHashes");
		}), Z.apply(this, arguments);
	}
	l.lidPnCache = g, l.lidPnCacheDirtySet = h, l.setNotAddressBookContacts = y, l.createOrMergeAddressBookContacts = C, l.isAddressBookContact = b, l.getContactHash = S, l.updateLidMetadata = R, l.warmUpLidPnMapping = L, l.warmUpAllLidPnMappings = k, l.getAlternateUserWid = T, l.getMeUserWids = D, l.getAlternateDeviceWid = x, l.getLatestLid = $, l.getCurrentLid = P, l.getPhoneNumber = N, l.getPnIfLidIsLatestMapping = M, l.clearLidPnMappingCache = w, l.getContactRecord = A, l.bulkGetContactRecord = F, l.updateContactAdvHostedType = O, l.CheckPnToLidMappingCaller = W, l.checkPnToLidMapping = q, l.armContactHashRepairWait = H, l.markContactHashRepairComplete = G, l.getContactRecordByHash = z, l.updateContactsHashes = J;
}), 98);
