__d("WAWebAttachMediaModel", [
	...
	var e, s, u, c, d, m, p, _, f, g = n("$InternalEnum").Mirrored([
		"Outline",
		"OutlineDone",
		"CropRotateCanvas"
	]), h = n("$InternalEnum").Mirrored(["Standard", "HD"]), y = {
		READY: "ready",
		...
		function a() {
			...
			return e = t.call.apply(t, [this].concat(i)) || this, e.id = o("WAWebBaseModel").prop(), e.file = o("WAWebBaseModel").prop(), e.uiProcessed = o("WAWebBaseModel").prop(!1), e.fileOrigin = o("WAWebBaseModel").prop(), e.isVcardOverMmsDocument = o("WAWebBaseModel").prop(!1), e.stickerMaker = o("WAWebBaseModel").prop(!1), e.supportedTypes = o("WAWebBaseModel").prop(), e.quality = o("WAWebBaseModel").prop(h.Standard), e.originalAttachment = o("WAWebBaseModel").session(), e.state = o("WAWebBaseModel").session(), e.mediaPrep = o("WAWebBaseModel").session(), e.mimetype = o("WAWebBaseModel").session(), e.originalMimetype = o("WAWebBaseModel").session(), e.processPromise = o("WAWebBaseModel").session(), e.exception = o("WAWebBaseModel").session(), e.caption = o("WAWebBaseModel").session(), e.type = o("WAWebBaseModel").session(o("WAWebMsgType").MSG_TYPE.UNKNOWN), e.hdEligible = o("WAWebBaseModel").session(!1), e.metadataByQuality = o("WAWebBaseModel").session(), e.processingProgressPercentage = o("WAWebBaseModel").session(0), e.mediaEditorData = o("WAWebBaseModel").session(), e.lastMediaEditorAction = o("WAWebBaseModel").session(), e.editedFile = o("WAWebBaseModel").session(), e.$AttachMediaImpl$p_1 = o("WAWebBaseModel").session(function() {
				...
				if (this.type === o("WAWebMsgType").MSG_TYPE.DOCUMENT && o("WAWebMimeTypes").isPdfDocument(this.mimetype) && this.filename && this.state !== y.ERROR && o("WAWebTPPdfViewerGatingUtils").isAsyncPdfSendEnabled()) return !0;
				...
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					var t = yield e.attachmentPromise, n = t.file, r = window.URL.createObjectURL(n), a = yield o("WAWebMediaLoad").loadImage(r);
		...
		return i.initialize = function() {
			if (t.prototype.initialize.call(this), this.file instanceof (f || (f = n("Promise")))) this.originalAttachment = this.file, this.processAttachment(this.file);
			else throw r("err")("cannot process non-promise file");
		}, i.processAttachment = function(t) {
			var e, n = this;
			this.state = y.PROCESSING, this.mediaEditorData = r("WAWebMediaEditorData").create(), (e = this.$AttachMediaImpl$p_3) == null || e.abort(), this.$AttachMediaImpl$p_3 = new AbortController(), this.processPromise = this.$AttachMediaImpl$p_4(t).then(function() {
				var e = new Image();
			...
			switch (t != null ? t : this.quality) {
				case h.Standard: return o("WAWebABProps").getABPropConfigValue("web_image_max_edge");
				case h.HD: return o("WAWebABProps").getABPropConfigValue("web_image_max_hd_edge");
			}
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
				this.metadataByQuality = new Map(o("WAWebPREGatingUtils").isPREMediaUploadCacheEnabled() ? [[this.quality, yield o("WAWebMediaDataUtils").getImageMetadata(e, { maxDimension: this.$AttachMediaImpl$p_5(this.quality) })]] : [[h.Standard, yield o("WAWebMediaDataUtils").getImageMetadata(e, { maxDimension: this.$AttachMediaImpl$p_5(h.Standard) })], [h.HD, yield o("WAWebMediaDataUtils").getImageMetadata(e, { maxDimension: this.$AttachMediaImpl$p_5(h.HD) })]]);
			});
			function t(t) {
				return e.apply(this, arguments);
			}
			...
			return t.then((function() {
				var a = n("asyncToGeneratorRuntime").asyncToGenerator(function* (n) {
					...
					e.set({ mediaPrep: C });
					var b = yield C.waitForPrep(), v = b.documentPreview, S = b.mediaBlob instanceof r("WAWebMediaOpaqueData") ? window.URL.createObjectURL(b.mediaBlob.forceToBlob()) : window.URL.createObjectURL(b.mediaBlob), R = {
						file: b.mediaBlob,
						fullPreview: S,
						type: b.type,
						filename: b.filename,
					...
					o("isNonZeroNumber").isNonZeroNumber(b.fullWidth) && o("isNonZeroNumber").isNonZeroNumber(b.fullHeight) && (R.fullPreviewSize = {
						height: b.fullHeight,
						width: b.fullWidth
					}), b.type === o("WAWebMsgType").MSG_TYPE.IMAGE && (R.preview = S, e.originalAttachment = t, b.isGif || (e.hdEligible = yield e.$AttachMediaImpl$p_8({
						attachmentPromise: t,
						signal: new AbortController().signal
					}), e.hdEligible && (yield e.$AttachMediaImpl$p_7(d)))), b.type === o("WAWebMsgType").MSG_TYPE.DOCUMENT && v && (R.fullPreview = v.url, R.preview = v.thumbUrl, R.fullPreviewSize = {
						height: v.height,
						width: v.width
			...
			})()).catch(o("WAFilteredCatch").filteredCatch([(a = o("WAWebMediaFileErrors")).InvalidMediaFileType, a.InvalidImageFileType], function(n) {
			...
			var e = t.chat, o = t.options;
			return this.mediaPrep ? this.state === y.ERROR ? (f || (f = n("Promise"))).reject(this.exception) : this.state === y.PROCESSING && (this.previewable !== !0 || o.addEvenWhilePreparing !== !0) ? (f || (f = n("Promise"))).reject(r("err")("Media still processing")) : (this.$AttachMediaImpl$p_9(e) && (o.aiProvenancePromise = this.$AttachMediaImpl$p_10()), this.$AttachMediaImpl$p_11({
				chat: e,
				options: o
			})) : (f || (f = n("Promise"))).reject(r("err")("MediaPrep not available"));
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				...
				try {
					var r = yield this.originalAttachment, a = r.file;
					...
					var l = yield o("WAWebMediaWorkerProxy").detectAiProvenanceInWorker({
						hints: b(a),
			...
			if (this.state === y.READY) return o("WALogger").LOG(_ || (_ = babelHelpers.taggedTemplateLiteralLoose(["AttachMedia:sendToChat: before sendToChat"]))), this.mediaPrep.sendToChat({
				chat: e,
				options: n
			});
	...
	l.MediaEditorAction = g, l.MediaQuality = h, l.ATTACH_MEDIA_STATE = y, l.AttachMedia = v;
}), 98);
