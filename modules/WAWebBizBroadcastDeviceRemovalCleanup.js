		...
		return v = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			try {
				...
				if (t != null) {
					...
					o("WALogger").LOG(u || (u = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:device-cleanup] provided ids: ", ""])), i);
				} else try {
					var l = yield o("WAWebApiDeviceList").getMyDeviceList();
					a = new Set(l.devices.map(function(e) {
						return e.id;
					}));
					var b = Array.from(a).join(", ");
					o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:device-cleanup] fetched ids: ", ""])), b);
				} catch (e) {
				...
				var v = yield o("WAWebBizBroadcastCampaignAPI").getAllBizBroadcastCampaigns(), S = v.filter(function(e) {
					return !a.has(e.deviceId) && (e.status === o("WAWebSchemaBusinessBroadcastCampaign").BusinessBroadcastCampaignStatus.PROCESSING || e.status === o("WAWebSchemaBusinessBroadcastCampaign").BusinessBroadcastCampaignStatus.SCHEDULED);
				});
				if (S.length === 0) {
				...
				o("WALogger").LOG(_ || (_ = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:device-cleanup] ", " orphaned campaign(s)"])), S.length);
				var R = [], L = 0;
				yield (C || (C = n("Promise"))).all(S.map((function() {
					var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
						try {
							yield o("WAWebBizBroadcastCampaignAPI").updateBizBroadcastCampaign(e.campaignId, {
								pendingBroadcastMessageId: null,
								status: o("WAWebSchemaBusinessBroadcastCampaign").BusinessBroadcastCampaignStatus.FAILED
							}), L++, R.length < 3 && R.push(e.campaignId);
						} catch (t) {
				...
				})())), L > 0 && o("WALogger").LOG(g || (g = babelHelpers.taggedTemplateLiteralLoose([
					"[broadcast:device-cleanup] Marked ",
					" campaign(s) as FAILED => ",
					""
				])), L, R), o("WALogger").LOG(h || (h = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:device-cleanup] Cleanup complete"])));
			} catch (e) {
