__d("WAWebBweMLModelManager", [
	...
	"use strict";
	var e, s, u, c, d, m, p, _ = "WAWebBweMLModelManager", f = "ml_model_download_skip_hash_check", g = {
		inited: !1,
		cacheManager: null
	};
	function h(e) {
	...
	function y() {
		return y = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			if (!g.inited) try {
				...
				r.initialize(t), g.cacheManager = o("WAWebMLModelCacheManager").createMLModelCacheManager(r), o("WAWebBweModelPathProvider").setCacheManager(g.cacheManager), g.inited = !0, yield v();
			} catch (t) {
				o("WALogger").ERROR(e || (e = babelHelpers.taggedTemplateLiteralLoose([
					"",
					"/setupMLModelStorage: Failed to initialize: ",
					""
				])), _, t instanceof Error ? t.message : String(t));
			}
	...
	function b() {
		return b = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = [], r = [], a = g.cacheManager, i = [];
			for (var l of e) {
				if (a != null && a.isReady()) {
					var c = a.getModelFilePath(o("WAWebBweModelHashes").BWE_MODEL_NAME, l);
					if (c != null) {
						t.push(l);
						continue;
					}
			...
			}), m = yield o("WAWebMLModelFetcher").fetchMLModelsFromNetwork(d, { bytecodeVersion: [] }, {
				hashCheckDeterminer: o("WAWebBweModelHashes").determineHashCheckMethod,
				shouldSkipHashCheck: R()
			}), f = [], h = [], y = function* (n) {
				t.push(n.version), a != null && a.isReady() && f.push(a.storeModel(n.modelName, n.version, n.data).then(function(e) {
					e.success || h.push(n.version);
				}));
			};
			for (var C of m.successful) yield* y(C);
			yield (p || (p = n("Promise"))).all(f), h.length > 0 && o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose([
				"",
			...
			for (var v of m.failed) {
				var S, L, E, k, I, T = (S = (L = (E = v.error) == null ? void 0 : E.reason) != null ? L : (k = v.error) == null ? void 0 : k.type) != null ? S : "unknown";
				if (r.push({
					version: (I = v.request.version) != null ? I : 0,
					error: T
				}), b.length < 3) {
					var D;
					b.push("v" + ((D = v.request.version) != null ? D : "unknown") + ": " + T);
				}
			}
			return r.length > 0 && o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose([
				"",
			...
			if (!(e == null || !e.isReady())) {
				...
				if (t.success) {
					...
					r.length > 0 && (o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose([
						...
						" stale models: ",
						""
					])), _, r.length, r.map(function(e) {
						return e.version;
					}).join(", ")), yield e.deleteMultipleModels(r));
				}
			}
	...
	function k() {
		return k = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			return o("WALogger").LOG(m || (m = babelHelpers.taggedTemplateLiteralLoose([
				...
				", failed: ",
				""
			])), _, t.successful.length, t.failed.length), t;
