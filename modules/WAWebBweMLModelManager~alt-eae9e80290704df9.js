__d("WAWebBweMLModelManager", [
	...
	"use strict";
	var e, s, u, c, d, m, p = "WAWebBweMLModelManager", _ = "ml_model_download_skip_hash_check", f = {
		inited: !1,
		cacheManager: null
	};
	async function g(t) {
		if (!f.inited) try {
			...
			r.initialize(t), f.cacheManager = o("WAWebMLModelCacheManager").createMLModelCacheManager(r), o("WAWebBweModelPathProvider").setCacheManager(f.cacheManager), f.inited = !0, await y();
		} catch (t) {
	...
	async function h(e) {
		var t = [], n = [], r = f.cacheManager, a = [];
		for (var i of e) {
			if (r != null && r.isReady()) {
				var l = r.getModelFilePath(o("WAWebBweModelHashes").BWE_MODEL_NAME, i);
				if (l != null) {
					t.push(i);
					continue;
				}
		...
		}), d = await o("WAWebMLModelFetcher").fetchMLModelsFromNetwork(c, { bytecodeVersion: [] }, {
			hashCheckDeterminer: o("WAWebBweModelHashes").determineHashCheckMethod,
			shouldSkipHashCheck: C()
		}), m = [], _ = [], g = async function(n) {
			t.push(n.version), r != null && r.isReady() && m.push(r.storeModel(n.modelName, n.version, n.data).then(function(e) {
				e.success || _.push(n.version);
			}));
		};
		for (var h of d.successful) await g(h);
		await Promise.all(m), _.length > 0 && o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose([
			"",
		...
		for (var b of d.failed) {
			var v, S, R, L, E, k = (v = (S = (R = b.error) == null ? void 0 : R.reason) != null ? S : (L = b.error) == null ? void 0 : L.type) != null ? v : "unknown";
			if (n.push({
				version: (E = b.request.version) != null ? E : 0,
				error: k
			}), y.length < 3) {
				var I;
				y.push("v" + ((I = b.request.version) != null ? I : "unknown") + ": " + k);
			}
		}
		return n.length > 0 && o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose([
			"",
	...
	async function y() {
		...
		if (!(e == null || !e.isReady())) {
			...
			if (t.success) {
				var n = new Set(b()), r = [];
				for (var a of t.value) a.modelName === o("WAWebBweModelHashes").BWE_MODEL_NAME && (n.has(a.version) || r.push({
					modelName: o("WAWebBweModelHashes").BWE_MODEL_NAME,
					version: a.version
				}));
				r.length > 0 && (o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose([
					"",
					"/cleanupStaleModels: Removing ",
					" stale models: ",
					""
				])), p, r.length, r.map(function(e) {
					return e.version;
				}).join(", ")), await e.deleteMultipleModels(r));
			}
		}
	}
	function C() {
	...
	async function v() {
		...
		return o("WALogger").LOG(m || (m = babelHelpers.taggedTemplateLiteralLoose([
			...
			", failed: ",
			""
		])), p, t.successful.length, t.failed.length), t;
	}
	async function S(e) {
		await g(e), await v();
	}
	l.initBweMLModelsForCall = S;
}), 98);
