__d("WAWebContactBridgeApi", [
	...
	var e, s, u = {
		...
		propagateIdentityFieldsToLidContact: s.propagateIdentityFieldsToLidContact,
		...
		getFrequentContacts: function(a) {
			var t = a.count, i = a.includeVoipCallableOnly, l = 5, s = o("WAWebChatCollection").ChatCollection.getModelsArray(), u = s.filter(function(e) {
		...
		getFrequentChatsForSharing: function() {
			var t = o("WAWebChatCollection").ChatCollection.getModelsArray(), r = [];
			for (var a of t) {
				var i = a.id.isGroup(), l = a.id.isRegularUser();
				if (!(!l && !i)) {
					if (l) {
			...
			for (var p of d) {
				var _, f, g, h = o("WAWebProfilePicThumbCollection").ProfilePicThumbCollection.get(p.id), y = (_ = h == null ? void 0 : h.img) != null ? _ : null;
				if (p.id.isGroup()) {
					var C, b, v;
					m.push({
						lid: p.id.toString(),
						name: (C = (b = p.formattedTitle) != null ? b : (v = p.groupMetadata) == null ? void 0 : v.subject) != null ? C : "",
						contactType: "group",
						imgUrl: y
					});
					continue;
				...
				if (S != null) {
					...
					if (R != null) {
						var L = R.toString(), E = (f = o("WAWebLidMigrationUtils").toPn(p.id)) != null ? f : S.phoneNumber, k = (g = E != null ? o("WAWebContactCollection").ContactCollection.get(E) : null) != null ? g : S, I = o("WAWebUserPrefsMeUser").isMeAccount(p.id), T = "personal";
						I ? T = "me" : k.isEnterprise ? T = "enterprise" : k.isSmb ? T = "smb" : k.isBusiness && (T = "business"), m.push({
							lid: L,
							name: o("WAWebFrontendContactGetters").getFormattedUser(k) || S.pushname || S.name || "",
							contactType: T,
							imgUrl: y
						});
					}
