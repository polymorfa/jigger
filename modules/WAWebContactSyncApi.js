__d("WAWebContactSyncApi", [
	...
	var e, s, u, c, d, m, p, _, f, g, h, y, C, b, v, S, R = 500, L = 20;
	function E(e) {
		return k.apply(this, arguments);
	}
	function k() {
		return k = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = e.contactIds, n = e.mode, r = e.shouldDelayBetweenChunks, a = R, i = Math.ceil(t.length / a);
			o("WALogger").LOG(d || (d = babelHelpers.taggedTemplateLiteralLoose([
				"[contact-sync] start ",
				" contacts, ",
				" chunks, size=",
				""
			])), t.length, i, a).tags("contact-sync");
			for (var l = 0; l < t.length; l += a) {
				var s = t.slice(l, l + a);
				r && (yield o("WAPromiseDelays").delayMs(L * 1e3)), yield I({
					contactIds: s,
					shouldSyncDevice: !0,
					mode: n
	...
	function T() {
		return T = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			var a = e.contactIds, i = e.mode, l = e.shouldSyncDevice;
			if (a.length !== 0) {
				o("WAWebAppTracker").AppTracker.start(o("WAWebAppTracker").AppTrackerType.ContactSync);
				try {
					var s = yield V(a), u = new (o("WAWebUsync")).USyncQuery().withContext("background").withMode(i).withBusinessProtocol().withStatusProtocol();
					o("WAWebTextStatusGatingUtils").receiveTextStatusEnabled() && u.withTextStatusProtocol(), l && u.withDeviceProtocol(), u.withDisappearingModeProtocol(), u.withLidProtocol(), u.withUsernameProtocol();
					var c = o("WAWebContactSyncLogger").contactSyncLogger.createEventContext({
						syncType: o("WAWebContactSyncLogger").getSyncTypeString("background", i),
					...
					}), p = new Map(r("compactMap")(d, function(e) {
					...
					s.forEach(function(e) {
						...
						var r = o("WAWebUsernameTypes").serializeMaybeUsername(f.get(o("WAWebWidFactory").asUserWidOrThrow(e.id)));
						r != null && t.withUsername(r), u.withUser(t);
					});
					for (var g = null, h = 3, y = 10 * Math.random(); h-- > 0;) try {
						g = yield u.execute();
						var C = g.error.all;
						if (C) {
							var b;
							throw y = (b = C.errorBackoff) != null ? b : 10 * Math.random(), r("err")("syncContactList: error " + C.errorCode + ": " + C.errorText);
						} else h = 0;
					} catch (e) {
						var v = r("getErrorSafe")(e);
						h === 0 && (o("WALogger").ERROR(m || (m = babelHelpers.taggedTemplateLiteralLoose(["failed to execute usync query"]))).verbose().sendLogs("failed to execute usync query: " + v.message), o("WAWebContactSyncLogger").contactSyncLogger.logFailure(c, v.errorCode, g != null ? g : void 0, o("WAWebContactSyncErrorCodes").BACKGROUND_SYNC)), yield o("WAPromiseDelays").delayMs(y * 1e3);
					}
					...
					yield (S || (S = n("Promise"))).all([
						...
						x(g),
						A(g, I)
					]), o("WAWebContactSyncLogger").contactSyncLogger.logSuccess(c, g, I), yield O(a);
				} finally {
	...
	function $() {
		return $ = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			if (!o("WAWebTextStatusGatingUtils").receiveTextStatusEnabled()) return (S || (S = n("Promise"))).resolve();
			...
			yield (S || (S = n("Promise"))).all(e.list.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					if (e.text_status) {
						var t = e.text_status, n = t.emoji, r = t.ephemeralDurationSeconds, a = t.lastUpdateTime, i = t.text;
						yield o("WAWebUpdateTextStatusForContact").updateTextStatusForContact({
							contactId: e.id,
							textString: i,
							emoji: n,
							ephemeralDuration: r,
	...
	function N() {
		return N = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			yield (S || (S = n("Promise"))).all(e.list.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					e.disappearing_mode && (yield o("WAWebUpdateDisappearingModeForContact").updateDisappearingModeForContact({
						contactId: e.id,
	...
	function w() {
		return w = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = e.error.lid;
			t && o("WALogger").WARN(f || (f = babelHelpers.taggedTemplateLiteralLoose([
				"handleLidSync: error ",
				": ",
				""
			])), t.errorCode, t.errorText);
			...
			}), a > 0 && o("WALogger").WARN(g || (g = babelHelpers.taggedTemplateLiteralLoose(["handleLidSync: skip ", " missing id"])), a), l > 0 && o("WALogger").WARN(h || (h = babelHelpers.taggedTemplateLiteralLoose([
				"handleLidSync: skip ",
				" lidResults (lid as id) => ",
				""
			])), l, i), u > 0 && o("WALogger").WARN(y || (y = babelHelpers.taggedTemplateLiteralLoose([
				"handleLidSync: skip ",
				" pnResults (pn as id) => ",
				""
			])), u, s), n.length > 0 && r.length > 0 && o("WALogger").WARN(C || (C = babelHelpers.taggedTemplateLiteralLoose([
				"handleLidSync: mixed types - lid: ",
				", pn: ",
				""
			])), n.length, r.length);
			var c = [].concat(n, r);
			c.length > 0 && (yield o("WAWebDBCreateLidPnMappings").createLidPnMappings({
				mappings: c,
				flushImmediately: !0,
	...
	function F() {
		return F = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			t.businessChange = n.length, yield o("WAWebBizVerifiedNameAction").handleVerifiedNameSync(n.map(function(e) {
	...
	function O(e) {
		...
		return o("WALogger").LOG(s || (s = babelHelpers.taggedTemplateLiteralLoose(["mark ", " contacts completed for contact sync"])), t.length).tags("contact-sync"), r("WAWebLidAwareContactsDB").bulkCreateOrMerge(t, "ContactSyncApi.markContactsSyncCompleted");
	}
	function B(e, t, n) {
		return W.apply(this, arguments);
	}
	function W() {
		return W = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, n) {
			...
			r && o("WALogger").WARN(b || (b = babelHelpers.taggedTemplateLiteralLoose([
			...
			return n.deviceChange = a.length, o("WALogger").LOG(v || (v = babelHelpers.taggedTemplateLiteralLoose([
				"handleSyncDevice: skipping ",
				" out of ",
				" records"
			])), e.list.length - a.length, e.list.length), a = o("WAWebContactSyncUtils").backfillMissingDeviceSyncEntries(t, a), yield o("WAWebGetIdentityKeysJob").getAndStoreIdentityKeys(a.map(function(e) {
				return e.wid;
	...
	function U() {
		return U = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			var t = new Map(), n = e.map(function(e) {
				return o("WAWebWidFactory").asUserWidOrThrow(e.id);
			}), r = yield o("WAWebApiContactUsernameFields").bulkGetContactToUsernameInfoMap([].concat(n));
	...
	function G(e) {
		if (Number.isNaN(e) || e <= o("WATimeUtils").HOUR_SECONDS) {
			o("WALogger").WARN(u || (u = babelHelpers.taggedTemplateLiteralLoose(["invalid contact sync refresh period: ", ""])), e).tags("contact-sync");
			return;
		}
		o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose(["set contact sync refresh period: ", ""])), e).tags("contact-sync"), r("nullthrows")(r("WAWebLocalStorage")).setItem(o("WAWebUserPrefsKeys").KEYS.CONTACT_SYNC_REFRESH, String(e));
	}
	l.syncContactListInChunks = E, l.syncContactList = I, l.handleLidSync = M, l.markContactsSyncCompleted = O;
}), 98);
