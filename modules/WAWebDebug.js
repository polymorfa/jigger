__d("WAWebDebug", [
	...
	var e, s, u, c, d, m, p, _, f, g, h, y, C, b, v, S = v || (v = o("react")), R = (e = n("cr:5553")) == null ? void 0 : e.VoipBridgePerfSimulator;
	function L() {
	...
	function E() {
		return (b || (b = n("Promise"))).all([o("WAWebModelStorage").destroy(), o("WAWebOffdStorage").destroy()]).then(function() {
	...
	function I() {
		return I = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			return yield o("WAWebUserPrefsMultiDevice").clearRegistrationInfo(), (b || (b = n("Promise"))).all([
				o("WAWebModelStorage").destroy(),
				o("WAWebSignalStorage").destroy(),
				o("WAWebOffdStorage").destroy()
			]).then(function() {
				o("WALogger").LOG(d || (d = babelHelpers.taggedTemplateLiteralLoose(["[reload] signOut"]))), location.reload();
			});
	...
	function w() {
		return w = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			if (n == null) return o("WALogger").WARN(m || (m = babelHelpers.taggedTemplateLiteralLoose(["debug: setDeviceListEntry: no entry for ", ""])), e), !1;
			var r = new Set([
				"timestamp",
				"expectedTs",
			...
			for (var a of Object.keys(t)) if (!r.has(a)) return o("WALogger").WARN(p || (p = babelHelpers.taggedTemplateLiteralLoose(["debug: setDeviceListEntry: unsupported key: ", ""])), a), !1;
			return o("WAWebSchemaDeviceList").getDeviceListTable().merge(e, t).then(function() {
				return o("WALogger").LOG(_ || (_ = babelHelpers.taggedTemplateLiteralLoose(["debug: setDeviceListEntry: update successful"]))), !0;
			});
	...
	function F() {
		return F = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = yield o("WAWebSchemaPrivacyDisallowedList").getPrivacyDisallowedListTable().get("groupadd");
			o("WALogger").LOG(f || (f = babelHelpers.taggedTemplateLiteralLoose(["debug:setPrivacyForOneCategory"])));
	...
	function B() {
		return B = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = yield o("WAWebSchemaPrivacyDisallowedList").getPrivacyDisallowedListTable().get(e);
			return o("WALogger").LOG(g || (g = babelHelpers.taggedTemplateLiteralLoose(["debug:getPrivacyDisallowedListTableContents"]))), t;
	...
	q.doc = "Trigger update check polling and install if an update is available";
	function U() {}
	function V() {}
	V.doc = "Toggle Sticker Maker debug mode (DEV only)", V.paramsToExecute = [];
	function H() {
	...
	function X() {
		return X = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			if (n == null) {
				o("WALogger").WARN(h || (h = babelHelpers.taggedTemplateLiteralLoose(["bulkSendMessageWithoutStoreByPn: user has no phone"])));
				return;
			}
	...
	Q.doc = "Sending multiple messages without store message in DB, which will make the sending faster";
	function Y(e) {
	...
	function J() {
		return J = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			try {
				var t;
				if (typeof e == "string") {
					var n = yield o("WAWebMsgCollection").MsgCollection.getMessagesById([e]), r = n.messages;
					t = r[0];
				} else t = e;
				var a = o("WAWebFrontendMsgGetters").getChat(t);
				yield o("WAWebSendTextMsgChatAction").sendTextMsgToChat(a, "testing", { quotedMsg: t });
			} catch (e) {
				o("WALogger").WARN(y || (y = babelHelpers.taggedTemplateLiteralLoose(["sendReplyToMsg: error ", ""])), e);
			}
	...
	ee.doc = "Returns the decoded message object of the given futureproof message";
	function te(e) {
	...
	function ae() {
		return ae = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			r("WAWebNetworkStatus").simulateNetworkStatus(!1), o("WAWebSocketModel").Socket.socket = null, o("WAWebSocketModel").Socket.state = o("WAWebSocketConstants").SOCKET_STATE.OPENING, o("WAWebSocketModel").Socket.stream = o("WAWebSocketConstants").SOCKET_STREAM.DISCONNECTED, yield o("WAComms").stopComms();
	...
	function le() {
		return le = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			r("WAWebNetworkStatus").simulateNetworkStatus(!0), yield o("WAWebStartBackend").startBackend();
	...
	de.doc = "Update draft message to update the text in the chat DB table as well as the Chat model UI.";
	function me(e) {
		return pe.apply(this, arguments);
	}
	function pe() {
		return pe = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = typeof e == "string" ? e : e.toString(), n = yield o("WAWebSchemaChat").getChatTable().get(t);
	...
	function be() {
		return be = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = Array.from(o("WAWebModelStorageUtils").getStorage().tables.keys()), t = e.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					var t = o("WAWebModelStorageUtils").getStorage().table(e), n = yield t.all();
					return [e, n];
				});
				return function(t) {
	...
	function Se() {
		return Se = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = Array.from(o("WAWebSignalStorageUtils").getStorage().tables.keys()), t = e.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					var t = o("WAWebSignalStorageUtils").getStorage().table(e), n = yield t.all();
					return [e, n];
				});
				return function(t) {
					return e.apply(this, arguments);
	...
	Re.doc = "Submit all cached metrics for the given WAM buffer type ('regular' or one of Private Stats keys)", Re.paramsToExecute = ["regular"];
	function Ee(e) {
		o("WAWebUserPrefsGeneral").setDebugFacebookGraphQLEndpointOverride(e);
	}
	Ee.doc = "Debug method for testing against a specific graphQL Facebook OD endpoint. Pass OD number as a string, e.g. \"0934\"";
	var ke = function() {
		o("WAWebModalManager").ModalManager.open(S.createElement(o("WAWebListsIntroPopupLoadable").ListsIntroPopupLoadable, null));
	};
	...
	$e.doc = "Send a raw wam event (used by UWP Hybrid app)", $e.paramsToExecute = [];
	function Ne(e, t) {
	...
	Me.doc = "Set the WebTP sharer skip confirmation preference (debug override)";
	function we(e) {
		return Ae.apply(this, arguments);
	}
	function Ae() {
		return Ae = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = 0, n = 0x7048860ddf79, r = yield o("WAWebCanonicalTokenExchange").storeCanonicalCredentials({
				userId: n,
				deviceId: t,
				accessToken: e
			}, "recovery");
	...
	function Be() {
		return Be = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			return o("WALogger").LOG(C || (C = babelHelpers.taggedTemplateLiteralLoose([
				...
				" key(s): ",
				""
			])), t.length, t), yield o("WAWebSendNonMessageDataRequest").sendPeerDataOperationRequest(o("WAWebProtobufsE2E.pb").Message$PeerDataOperationRequestType.PLACEHOLDER_MESSAGE_RESEND, { msgKeys: n }), {
	...
	Oe.doc = "Fires a real PLACEHOLDER_MESSAGE_RESEND RDU for the given serialized msg key(s).";
	function We() {
	...
	function qe() {
		return qe = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			try {
				return yield o("WAWebUploadPQPrekeysJob").generateAndUploadPQPreKeys(), "PQ key upload complete - check WALogger for details";
			} catch (e) {
