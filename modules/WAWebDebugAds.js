__d("WAWebDebugAds", [
	...
	m.doc = "Link given ad media id in Facebook, so that it is available there for native ads creation flows";
	function p(t) {
		var r = function(t) {
		...
		return t == null ? new (e || (e = (n("Promise"))))(function(e) {
	...
	p.doc = "Upload given blob as native ad media, so that it is available there for native ads creation flows";
	var _ = function() {
		o("WAWebDrawerManager").DrawerManager.openDrawerLeft(u.jsx(r("WAWebCTWADebugDrawer.react"), { onBack: function() {
	...
	f.doc = "Clear WA token from cache and storage", f.paramsToExecute = [];
	function g() {
		o("WAWebBizNativeAdsStoredFBIdentityStore").clearStoredFBIdentity();
	}
	g.doc = "Clear FB identity token from storage", g.paramsToExecute = [];
	function h(e) {
		o("WAWebUserPrefsGeneral").setLastUsedAdAccountType(e);
	}
	h.doc = "Set last used ad account type preference ('FB', 'WAA', or null to clear)";
	function y() {
	...
	y.doc = "Clear all ads identity (FB token, WA token, and last used account type)", y.paramsToExecute = [];
	function C() {
		return b.apply(this, arguments);
	}
	function b() {
		return b = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = o("WAWebUserPrefsCTWA").getFBIdentity(), t = o("WAWebUserPrefsCTWA").getAdAccountToken(), n = t != null && t.type === "WAA" ? t.bp_id + " (" + t.tokenStrength + ")" : "null", r = o("WAWebUserPrefsGeneral").getLastUsedAdAccountType(), a = yield o("WAWebLinkedAccountsJob").queryLinkedPagesInfo();
		}), b.apply(this, arguments);
	}
	C.doc = "Print current ads identity details (tokens, account type preference, and linked pages)", C.paramsToExecute = [];
	function v(e) {
	...
	S.doc = "Print the CTWA conversation depth (biz reply count) for the active chat", S.paramsToExecute = [];
	var R = {
