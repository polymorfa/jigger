__d("WAWebDebugBizBroadcast", [
	...
	s.doc = "Override primary device Business Broadcast capability (true/false)";
	function u() {
		o("WAWebTos").TosManager.setState(o("WAWebBizBroadcastTos").getBizBroadcastTosId(), "ACCEPTED", o("WATimeUtils").unixTime());
	}
	u.doc = "Accept BB TOS locally (skips server RPC, bypasses TOS modal)", u.paramsToExecute = [];
	function c() {
	...
	c.doc = "Force BB Pro onboarding status to eligible_to_onboard (E2E)", c.paramsToExecute = [];
	function d() {
	...
	function p() {
		return p = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = d(), t = r("WAWebPonyfillsCryptoRandomUUID")(), n = o("WAWebUserPrefsMeUser").getMeDevicePnOrThrow_DO_NOT_USE().getDeviceId();
			...
			}), yield o("WAWebBizBroadcastSystemMessageManager").updateBizBroadcastSystemMessage(e);
		}), p.apply(this, arguments);
	}
	m.doc = "Create a test PROCESSING campaign for the active broadcast chat (E2E)", m.paramsToExecute = [];
	function _() {
		return f.apply(this, arguments);
	}
	function f() {
		return f = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var t = d(), r = yield o("WAWebBizBroadcastCampaignAPI").getBizBroadcastCampaignsByBroadcastJid(t);
			yield (e || (e = n("Promise"))).all(r.filter(function(e) {
			...
			})), yield o("WAWebBizBroadcastSystemMessageManager").updateBizBroadcastSystemMessage(t);
		}), f.apply(this, arguments);
	}
	_.doc = "Complete all PROCESSING campaigns for the active broadcast chat (E2E)", _.paramsToExecute = [];
	function g(e) {
		return o("WAWebBizBroadcastProUpdateCampaignAction").cancelBizBroadcastProCampaign(e);
	}
	g.doc = "Cancel (pause) a BB Pro scheduled campaign by id (server mutation)";
	function h(e, t) {
		return o("WAWebBizBroadcastProUpdateCampaignAction").rescheduleBizBroadcastProCampaign(e, t);
	}
	h.doc = "Reschedule a BB Pro campaign: new start (epoch s); stop auto-set to +5d";
	var y = {
