__d("WAWebDebugBookingConfirmation", [
	...
	function _() {
		return _ = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			...
			try {
				yield o("WAWebSendMsgChatAction").addAndSendMsgToChat(_, g)[1], o("WALogger").LOG(s || (s = babelHelpers.taggedTemplateLiteralLoose(["[receiveBookingConfirmationMessage] booking confirmed ✓"])));
			} catch (e) {
	...
	f.doc = "Receive a location-based booking confirmation example in the active chat", f.paramsToExecute = [];
	function h() {
	...
	h.doc = "Receive an online meeting booking confirmation example in the active chat", h.paramsToExecute = [];
	function C() {
	...
	C.doc = "Receive a phone-based appointment confirmation example in the active chat", C.paramsToExecute = [];
	var v = {
