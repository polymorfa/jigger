__d("WAWebDebugBot", [
	...
	function L() {
		return L = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			e && (yield o("WAWebSchemaBotProfile").getBotProfileTable().clear(), o("WAWebUserPrefsBot").setUgcBotListLastBhash(null)), o("WAWebUserPrefsBot").setBotListLastRequestedTimestamp(0), o("WAWebUserPrefsBot").setUgcBotListLastRequestedTimestamp(0), window.location.reload();
	...
	I.doc = "Opens the biz bot 1p learn more modal", I.paramsToExecute = [];
	var T = function() {
	...
	D.doc = "Opens the bot feature not available modal", D.paramsToExecute = [];
	var x = function() {
	...
	$.doc = "Adds a debug default bot for testing invoke against new bot numbers";
	function P() {
	...
	function M() {
		return M = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			var n, a;
			o("WALogger").LOG(p || (p = babelHelpers.taggedTemplateLiteralLoose(["[debugBot] downloadImagineMedia: ", ""])), e);
			var i = new AbortController(), l = yield o("WAWebHttpExtendedFetch").extendedFetch(e, { signal: i.signal });
			if (!l.ok) throw r("err")("[debugBot] downloadImagineMedia failed: " + l.status + " " + l.statusText);
			var s = t != null ? t : "image/jpeg", u = yield l.arrayBuffer(), c = new Blob([u], { type: s }), d = yield crypto.subtle.digest("SHA-256", u), m = Array.from(new Uint8Array(d)), g = btoa(String.fromCharCode.apply(null, m));
			...
			var h = o("WAWebFilenameManager").getDefaultName({
				...
				filename: "",
				vcardList: []
			}), y = window.URL.createObjectURL(c), C = o("WAWebDataLink").createDataLink(y);
			return C.style.display = "none", C.download = h, (n = document.body) == null || n.appendChild(C), C.click(), (a = document.body) == null || a.removeChild(C), o("WAPromiseDelays").delayMs(100).then(function() {
				...
				o("WALogger").ERROR(_ || (_ = babelHelpers.taggedTemplateLiteralLoose(["[debugBot] downloadImagineMedia: revokeObjectURL failed, ", ""])), e);
	...
	N.doc = "Downloads imagine media and caches it. Usage: downloadImagineMedia(url, mimeType?)";
	function w(e, t) {
		var n = r("WAWebMsgKey").fromString(e), a = o("WAWebMsgCollection").MsgCollection.get(n);
		if (a == null) {
			o("WALogger").WARN(u || (u = babelHelpers.taggedTemplateLiteralLoose(["[debugBot] Message not found: ", ""])), e);
			return;
		}
		if (a.forwardedAiBotMessageInfo == null) {
			o("WALogger").WARN(c || (c = babelHelpers.taggedTemplateLiteralLoose(["[debugBot] Message has no forwardedAiBotMessageInfo"])));
			return;
		}
		var i = o("WAWebFrontendMsgGetters").getAsRichResponse(a);
		if (i == null) {
			o("WALogger").WARN(d || (d = babelHelpers.taggedTemplateLiteralLoose(["[debugBot] Message is not a rich response"])));
			return;
		}
		o("WAWebUpdateForwardedBotValidationStatusAction").updateForwardedBotValidationStatusAction(i, t), o("WALogger").LOG(m || (m = babelHelpers.taggedTemplateLiteralLoose([
			"[debugBot] Set validationStatus=",
			" on ",
			""
		])), t, e);
	}
	w.doc = "Sets validationStatus on a forwarded bot message by msgId. Usage: updateForwardedBotValidationStatus(msgId, status)";
	function A(e) {
	...
	function B(e) {
		var t, n, r = A(e), a = o("WAWebBotProfileCollection").BotProfileCollection.get(r), i = a != null ? {
			product: a.product,
	...
	B.doc = "Show a bot's Standard Bot Profile fields + computed support state. Usage: showBotProfile(widStr?) — defaults to the open chat";
	function W() {
	...
	W.doc = "Show SBP fields + computed support state for all cached bot profiles", W.paramsToExecute = [];
	function U(e, t) {
		return V.apply(this, arguments);
	}
	function V() {
		return V = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			var n = A(e), r = o("WAWebBotProfileCollection").BotProfileCollection.get(n), a = r != null ? F(r) : O(n), i = o("WAWebBotProfileDebugUtils").mergeProfileOverride(a, t);
	...
	U.doc = "Force SBP fields on a bot and persist to IndexedDB. Usage: overrideBotProfile(widStr, {product?, isDeprecated?, isDeleted?})";
	function H(e) {
	...
	function G() {
		return G = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = A(e);
			yield o("WAWebSchemaBotProfile").getBotProfileTable().remove(t.toString()), o("WAWebBotProfileCollection").BotProfileCollection.remove(t);
		}), G.apply(this, arguments);
	}
	H.doc = "Remove a bot's cached profile row so it refetches on next sync. Usage: clearBotProfileOverride(widStr)";
	var z = {
