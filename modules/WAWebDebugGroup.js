__d("WAWebDebugGroup", [
	...
	function m() {
		...
		if (!t || !t.groupMetadata) {
			o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose(["the current chat is not a group chat"])));
			return;
		}
		var n = t.groupMetadata.participants;
		if (!n.iAmAdmin()) {
			o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose(["you are not an admin in this chat"])));
			return;
		}
	...
	function p(e, t, a) {
		a === void 0 && (a = "Test");
		for (var i = self.performance.now(), l = t.map(function(e) {
			return new (r("WAWebContactModel"))({ id: o("WAWebWidFactory").createWid(e + "@s.whatsapp.net") });
		}), s = [], u = 0; u < e; u++) s.push(o("WAWebCreateGroupAction").createGroup({
			title: a + "-" + u,
			thumb: null,
			full: null
		}, l));
	...
	function f() {
		return f = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, a) {
			if (a === void 0 && (a = "Group name filter as a RegExp excluding leading ^"), !(e > 1 && a.length < 2)) {
				var i = self.performance.now(), l = new RegExp("^" + r("escapeRegex")(a)), s = o("WAWebChatCollection").ChatCollection.filter(function(e) {
					return o("WAWebChatGetters").getIsGroup(e) && l.test(e.title());
				});
				if (s.length !== 0) {
					var u = [];
					for (var d of s) for (var m = 0; m < e; m++) u.push(o("WAWebSendTextMsgChatAction").sendTextMsgToChat(d, t + " | " + m));
					yield (c || (c = n("Promise"))).all(u);
					var p = e * s.length, _ = Math.floor(self.performance.now() - i);
				}
			}
		}), f.apply(this, arguments);
	}
	_.doc = "Send [message], [n] times, to every group starting with /^[groupNameFilter]/.";
	function g(e, t) {
		t === void 0 && (t = "Test");
		var a = self.performance.now(), i = new RegExp("^" + r("escapeRegex")(t)), l = o("WAWebChatCollection").ChatCollection.filter(function(e) {
			return o("WAWebChatGetters").getIsGroup(e) && i.test(e.title());
		});
		if (l.length === 0) return (c || (c = n("Promise"))).resolve();
		var s = [];
		for (var u of l) s.push(o("WAWebSetArchiveChatAction").setArchive(u, e, !0));
		return (c || (c = n("Promise"))).all(s).then(function() {
	...
	function y() {
		return y = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			if (e == null) {
				o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose(["bulkDeleteGroups: requires a tag"])));
				return;
			}
			var t = new RegExp("^" + r("escapeRegex")(e)), a = o("WAWebChatCollection").ChatCollection.filter(function(e) {
				return o("WAWebChatGetters").getIsGroup(e) && t.test(e.title());
			});
			if (prompt("exit and delete the following groups (y/n)? \n" + a.map(function(e) {
				return e.title();
			}).join("\n")) === "y") {
				var i = a.filter(function(e) {
					var t;
					return e == null || (t = e.groupMetadata) == null ? void 0 : t.participants.iAmMember();
				});
				yield (c || (c = n("Promise"))).all(i.map(function(e) {
					return o("WAWebExitGroupAction").sendExitGroup(e);
				})), yield c.all(a.map(function(e) {
	...
	function v() {
		return v = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			if (!n) throw r("err")("You need to select a message by right clicking it");
			if (!o("WAWebMsgGetters").getIsGroupMsg(n)) throw r("err")("You need to select a group message");
			var a = o("WAWebUserPrefsMeUser").getMeUserOrThrow(), i = o("WAWebFrontendMsgGetters").getChat(n), l;
			if (e) l = a;
			else if (t != null) l = o("WAWebWidFactory").createWid(t + "@c.us");
			else {
				var s;
				l = (s = i.groupMetadata) == null ? void 0 : s.participants.filter(function(e) {
					return !e.id.equals(a) && e.isAdmin;
				})[0].id;
			}
			if (!l) throw r("err")("No admin found");
			var u = n.id, c = Math.round(Date.now() / 1e3), d = {
	...
	function R() {
		return R = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = yield o("WAWebSchemaGroupMetadata").getGroupMetadataTable().all(), t = e.filter(function(e) {
	...
	S.doc = "Get all inactive (PN-addressed, isMember) groups", S.paramsToExecute = [];
	function L() {
		return E.apply(this, arguments);
	}
	function E() {
		return E = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			return o("WAWebInactiveGroupLidMigrationJob").migrateInactiveGroupsToLid();
		}), E.apply(this, arguments);
	}
	L.doc = "Migrate inactive groups to LID addressing mode", L.paramsToExecute = [];
	function k(e) {
		return I.apply(this, arguments);
	}
	function I() {
		return I = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = o("WAWebWidFactory").createWid(e), n = yield o("WAWebGroupQueryGroupJob").queryGroupJob(t);
			return n;
	...
	var T = {
		...
		handleAddressingModeMismatch: o("WAWebGroupHandleAddressingModeMismatch").handleAddressingModeMismatch,
