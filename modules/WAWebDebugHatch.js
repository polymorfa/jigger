__d("WAWebDebugHatch", [
	...
	var e, s, u = s || (s = o("react")), c = "hitl.approval", d = [
		{
			displayName: "Gmail",
	...
	function p(e) {
		var t = d[e % d.length], n = e + 1;
		return {
			approvalId: "debug-approval-" + n,
			reason: "#" + n + ": Allow Hatch to perform \"" + t.actionLabel + "\" in " + t.displayName + "?",
			shortExplanation: "Task #" + n + ": " + t.summary,
			richExplanation: "You asked Hatch to " + t.summary + ". This is sample approval #" + n + ", injected to exercise the approval UI.",
	...
	_.doc = "List the Hatch AIMetadataOperation payloads captured this session (inbound events/responses, outbound reqs) with their raw envelope, decoded form, and timestamps";
	function f() {
		r("WAWebHatchPayloadDebugStore").clear();
	}
	f.doc = "Clear the captured Hatch AIMetadataOperation payload list";
	function g() {
		o("WDSDialogBridge").openWDSDialog(u.jsx(r("WAWebHatchApprovalDialog.react"), {
			richDescription: "Create an email draft to xyz@meta.com",
			title: "Hatch wants to use draft email from Gmail",
			onConfirm: function(n) {
				o("WALogger").LOG(e || (e = babelHelpers.taggedTemplateLiteralLoose(["[debugHatch] Hatch approval option selected: ", ""])), String(n)), o("WDSDialogBridge").closeWDSDialog();
			},
			onDismiss: o("WDSDialogBridge").closeWDSDialog
		}));
	}
	g.doc = "Opens the Hatch approval options dialog", g.paramsToExecute = [];
	function h(e) {
		e === void 0 && (e = 3);
		...
		return Array.from({ length: t }, function(e, t) {
			...
			return o("WAWebHandleHatchMetadataSync").handleHatchMetadataSync(r, "debug-" + n.approvalId), n.approvalId;
		});
	}
	h.doc = "Inject N (1-20, default 3) synthetic HITL approvals as pending requests — pass a count to test the single-approval path, the multi-approval review dialog, and its scrolling with a long list", h.paramsToExecute = [3];
	function y(e) {
		return o("WAWebHatchApprovalDebug").debugInjectHatchApprovalRequest(e);
	}
	y.doc = "Inject a synthetic Hatch HITL approval request through the real decode -> store -> UI pipeline (open a Hatch chat to see the composer swap to the approval bar). Returns the approvalId.";
	function C(e) {
		o("WAWebHatchApprovalDebug").debugResolveHatchApproval(e);
	}
	C.doc = "Resolve a pending Hatch HITL approval (synthetic decision echo) by id, clearing the approval bar";
	var b = {
