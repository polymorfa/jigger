__d("WAWebDebugLogging", [
	...
	function h() {
		return h = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			if (e == null) {
				o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose(["[uploadThreadLoggingEventsNow] CTL not initialized"])));
				return;
			}
			var t = yield e.metadataStore.getOffset();
			if (t == null) {
				o("WALogger").ERROR(c || (c = babelHelpers.taggedTemplateLiteralLoose(["uploadThreadLoggingEventsNow: thread logging offset unset"])));
				return;
			}
			var n = o("WAWebChatThreadLoggingUtils").computeStartTs(t, o("WATimeUtils").unixTime()), r = yield e.metadataStore.getLastUploadedStartTs();
			yield o("WAWebChatThreadLogging").uploadChatThreadLoggingEvents({ lastStartTs: n }), yield e.metadataStore.setLastUploadedStartTs(r), o("WALogger").LOG(d || (d = babelHelpers.taggedTemplateLiteralLoose(["[uploadThreadLoggingEventsNow] flushed CTL->WAM startTs=", ""])), n);
	...
	function y(t) {
		for (var n = window.performance.now(), r = 0; r < t; r++) o("WALogger").LOG(e || (e = babelHelpers.taggedTemplateLiteralLoose(["logging test message from WAWebDebugLogging"])));
	...
	function S() {
		o("WALogger").ERROR(s || (s = babelHelpers.taggedTemplateLiteralLoose(["some log error"]))).sendLogs("some_error", {
			sampling: 1,
			separateEmployeeLog: !1,
			employeeSampling: 1
		});
	}
	S.doc = "Send logs without employee separation (does not add -employee suffix for employees)", S.paramsToExecute = [];
	function R(e) {
	...
	R.doc = "Set a filter pattern for logs (string or regex). Only matching logs will be shown. Pass null to clear the filter.";
	function L() {
	...
	E.doc = "Set number of context lines to show before/after matching logs. Pass 0 to disable context.";
	function k() {
