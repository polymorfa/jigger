__d("WAWebDebugMedia", [
	...
	function u() {
		return u = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			...
			if (r == null) return "No active chat is open — open a chat containing a sticker first.";
			var a = r.msgs.getModelsArray().map(function(e) {
				return {
					msg: e,
			...
			for (var i of a) {
				var l = i.mediaObject;
				l == null || l.consolidate({ downloadStage: o("WAWebMediaTypes").DownloadStage.NEED_POKE });
			}
			if (t) {
				yield (e || (e = n("Promise"))).all(a.map(function(e) {
					var t = e.mediaObject;
			...
			return "Reset " + a.length + " sticker(s) to NEED_POKE in the active chat" + (t ? " and blanked their images" : "") + ". Scroll them into view to see the download button.";
		}), u.apply(this, arguments);
	}
	s.doc = "Mark every sticker in the currently open chat as not-downloaded (NEED_POKE) so the download button/label reappears. Pass false to keep the sticker image visible. Repro tool for the sticker media-control UI.";
	var c = {
