__d("WAWebDebugPrivacyExperience", [
	"WALogger",
	"WAWebApiContact",
	"WAWebChatCollection",
	...
	async function m() {
		...
		if (!(!e || e.length === 0)) {
			...
			await r("WAWebLidAwareContactsDB").bulkCreateOrMerge(t, "DebugPrivacyExperience.clearLidMetadata"), o("WAWebContactCollection").ContactCollection.forEach(function(e) {
				e.id.isLid() && (e.phoneNumber = null, e.displayNameLID = null, e.shareOwnPn = !1, e.requestedPnTimestamp = null);
			});
		}
	}
	m.doc = "Deletes all information related with LID (displayName, shareOwnPn, lidXPn mapping, requestedPnTimestamp)";
	async function p(e) {
		await o("WAWebSetUsernameJob").setUsernamesJob(e.map(function(e) {
	...
	async function _(e, t) {
		var n, a = o("WAWebWidFactory").createUserWidOrThrow(e), i = {
			id: e,
			name: t,
			shortName: (n = o("WAWebContactShortName").getShortName(t)) != null ? n : "",
			type: "in",
			isAddressBookContact: 1,
			isContactSyncCompleted: 0
		};
		await r("WAWebLidAwareContactsDB").bulkCreateOrMerge([babelHelpers.extends({}, i)], "DebugPrivacyExperience.createContact"), o("WAWebContactCollection").ContactCollection.add(babelHelpers.extends({}, i, { id: a }), { merge: !0 });
	}
	...
	async function f() {
		...
		if (!(!e || e.length === 0)) {
			var t = r("compactMap")(e, function(e) {
				...
				return n != null && o("WAWebContactUtils").setContactNotMine(n), e.id;
			});
			t.length > 0 && await o("WAWebApiContact").setNotAddressBookContacts(t);
		}
	}
	f.doc = "Removes all lid contacts from address book.";
	async function g(e, t, n) {
		t === void 0 && (t = !0), await o("WAWebDBCreateLidPnMappings").createLidPnMappings({
			mappings: e.map(function(e) {
				return {
					lid: o("WAWebWidFactory").asUserLidOrThrow(o("WAWebWidFactory").createUserWidOrThrow(e.lid, "lid")),
	...
	function h(e) {
		return o("WAWebApiContact").getCurrentLid(o("WAWebWidFactory").createUserWidOrThrow(e));
	}
	h.doc = "Get a current LID for a given Phone Number";
	function y(e) {
		return o("WAWebApiContact").getPhoneNumber(o("WAWebWidFactory").asUserLidOrThrow(o("WAWebWidFactory").createUserWidOrThrow(e, "lid")));
	}
	y.doc = "Get the Phone Number for a given LID";
	function C() {
		var t = o("WAWebChatCollection").ChatCollection.getActive();
		if (!t) {
			o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose(["the current chat is null"])));
			return;
		}
	...
	function b() {
		...
		if (!e) {
			o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose(["the current chat is null"])));
			return;
		}
	...
	function v(e) {
		var t = o("WAWebUsernameCreationActionLogger").UsernameCreationActionLogger.getSessionId();
		o("WAWebSetUsernameJob").setUsernameQueryJob(e, t);
	}
	v.doc = "Set the own username at the account level.";
	function S(e) {
		return o("WAWebStatusContactAction").getStatus(o("WAWebWidFactory").createUserWidOrThrow(e));
	}
	S.doc = "Get the about (about 1.0) status for a given JID";
	function R(e) {
		return o("WAWebContactTextStatusBridge").getTextStatus(o("WAWebWidFactory").createUserWidOrThrow(e), null);
	}
