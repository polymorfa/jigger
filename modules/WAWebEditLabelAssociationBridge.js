__d("WAWebEditLabelAssociationBridge", [
	...
	function p() {
		return p = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t, a) {
			if (!(o("WAWebListsLabelGatingUtils").canEditLabelAssociation() || o("WAWebListsGatingUtils").isListsEnabled())) return (c || (c = n("Promise"))).reject(r("err")("editLabelAssociation is not supported"));
			var i = yield r("WAWebLabelJidSync").createLabelAssociationMutations(t, a), l = o("WAWebContactManagerGating").contactManagerEnabled() ? yield _(t, a) : {
				sublistLocalRemoves: [],
				sublistMutations: []
			}, s = l.sublistLocalRemoves, u = l.sublistMutations, d = u.length > 0 ? [
				"label-association",
				"chat",
				"label_sublist"
			] : ["label-association", "chat"];
			return o("WAWebSyncdCoreApi").lockForSync(d, [].concat(i, u), n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				yield g(t, a), yield (c || (c = n("Promise"))).all(s.map(function(t) {
	...
	function f() {
		return f = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			var a = [], i = [], l = yield (c || (c = n("Promise"))).all(e.map(function(e) {
				var t = e.id;
				return o("WAWebSchemaLabel").getLabelTable().get(t);
			}));
			e.forEach(function(e, n) {
				var s, u = e.type;
				if (((s = l[n]) == null ? void 0 : s.predefinedId) === o("WAWebLeadListConstants").LEAD_LIST_PREDEFINED_ID && u === "remove") for (var c of t) {
					var d = c.modelId;
					i.push(r("WAWebLabelSublistSync").getLabelSublistRemoveMutation({
						predefinedId: o("WAWebLeadListConstants").LEAD_LIST_PREDEFINED_ID,
						chatJid: d
					})), a.push(o("WAWebSchemaLabelSublist").createLabelSublistPrimaryKey({
						predefinedId: o("WAWebLeadListConstants").LEAD_LIST_PREDEFINED_ID,
						chatJid: d
					}));
				}
	...
	function h() {
		return h = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			e.forEach(function(e) {
				...
				t.map(function(e) {
					...
					r === "add" ? a.push({
						...
						associationId: l,
						type: t
					}) : i.push(o("WAWebSchemaLabelAssociation").createLabelAssociationPrimaryKey({
						labelId: n,
						associationId: l,
						type: t
					}));
				});
			}), yield (c || (c = n("Promise"))).all([o("WAWebDBLabelAssociationDatabaseApi").addOrEditLabelAssociations(a).catch(function(e) {
			...
			}), o("WAWebDBLabelAssociationDatabaseApi").removeLabelAssociations(i).catch(function(e) {
