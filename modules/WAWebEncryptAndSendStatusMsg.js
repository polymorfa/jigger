__d("WAWebEncryptAndSendStatusMsg", [
	...
	function v() {
		return v = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = yield o("WAWebSchemaMessageInfo").getMessageInfoTable().equals(["msgKey"], String(e));
			return t.length === 0 && (t = yield o("WAWebSchemaMessageInfo").getMessageInfoTable().equals(["msgKey"], String(o("WAWebLidMigrationUtils").getAlternateMsgKey(e)))), t.map(function(e) {
	...
	function L() {
		return L = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t, n, a, i, l, h = e.metricsReporter, y = e.msgProtobuf, C = e.sendMsgRecord, v = C.data.id, R = o("WAWebWidFactory").createWid(o("WAJids").STATUS_JID), L = o("WAWebUserPrefsMeUser").getMeDeviceLidOrThrow(), k = T(y), $ = o("WAWebStatusSessionGatingUtils").shouldUseStatusSessionForOutgoingMessage() ? o("WAWebSessionScope").SessionScope.STATUS : o("WAWebSessionScope").SessionScope.DEFAULT, P = o("WAWebE2EProtoUtils").typeAttributeFromProtobuf(y);
			P === "reaction" && o("WALogger").ERROR(s || (s = babelHelpers.taggedTemplateLiteralLoose(["encryptAndSendStatusMsg: unexpected reaction type for status message ", ""])), v.id).sendLogs("status-unexpected-reaction-type");
			var N, M, w, A = yield r("WAWebUserPrefsStatus").getStatusList(), F = r("compactMap")(A.list, o("WAWebLidMigrationUtils").toUserLid);
			if (A.list = F.map(function(e) {
				...
				var B = yield b(k);
				if (D(B, A.list)) {
					o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose(["encryptAndSendStatusMsg: start to send direct revoke message"]))), S(B, "direct revoke senderList");
					var W = yield o("WAWebDBDeviceListFanout").getFanOutList({
						wids: [].concat(B, [L]),
						shouldMergeAltDevices: !0
					});
					return S(W, "direct revoke deviceList"), x({
						deviceList: W,
						metricsReporter: h,
						msgProtobuf: y,
						sendMsgRecord: C,
			...
			o("WALogger").LOG(d || (d = babelHelpers.taggedTemplateLiteralLoose(["encryptAndSendStatusMsg: all device list size ", ""])), q.length), S(q, "deviceList after fanout");
			var U = yield r("WAWebUserPrefsStatus").getStatusSkDistribList(q, { isFullAudience: M }), V = U.participantList, H = U.skDistribList;
			if (H.length > 0) {
				var G, z, j, K;
			...
			if (yield o("WAWebApiMessageInfoStore").createOrMergeReceiptRecords(q.map(function(e) {
				return {
					msgKey: v,
					receiverId: e
				};
			...
			var J = yield E(R, L, H, V, y, $), Z = J[0], ee = J[1], te = J[2];
			o("WALogger").LOG(f || (f = babelHelpers.taggedTemplateLiteralLoose(["encryptAndSendStatusMsg: encrypt message body done"])));
			var ne = yield o("WAWebReportingTokenUtils").genReportingTokenBody(C.data, y), re = o("WAWap").wap("message", {
			...
			}, Z, ee, te, w, ne), oe = I(re);
			yield o("WAWebSendMsgCommonApi").updateIdentityRange(C, q), yield o("WAWebSignalProtocolStore").getSignalProtocolStore().flushBufferToDiskIfNotMemOnlyMode(), (a = h.sendPerfReporter) == null || a.postClientEncryptStage(), (i = h.sendPerfReporter) == null || i.startWrittenWireStage(), o("WALogger").LOG(g || (g = babelHelpers.taggedTemplateLiteralLoose(["encryptAndSendStatusMsg: start sending ", ""])), v.id), yield o("WAWebDeprecatedSendIqWorkerCompatible").deprecatedSendStanzaAndReturnAck(re, o("WAWebCommsAckParser").toCoreAckTemplate({
				id: v.id,
				class: oe,
				from: R,
				participant: null
			})), (l = h.sendPerfReporter) == null || l.postWrittenWireStage(), yield r("WAWebUserPrefsStatus").markStatusHasSenderKey(H);
	...
	function k() {
		return k = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, n, r, a, i) {
			var l = o("WAWebSendMsgCommonApi").encodeAndPad(a), s = o("WAWebBackendJobsCommon").mediaTypeFromProtobuf(a), u = yield o("WAWebSignal").Cipher.encryptSenderKeyMsgSignalProto(e, t, l), c = u.ciphertext, d = u.senderKeyBytes, m;
			o("WALogger").LOG(h || (h = babelHelpers.taggedTemplateLiteralLoose([
				"[session] genMessageBody: sessionScope=",
				" skDistribListLen=",
				""
			])), i, String(n.length)), n.length > 0 && (m = yield o("WAWebGetGroupKeyDistributionMsg").getKeyDistributionMsg(null, e, n, d, !1, void 0, i));
			var p = [], _ = [], f = !1, g = null;
	...
	function $() {
		return $ = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			var d = l.data.to, m = yield o("WAWebSendMsgCreateFanoutStanza").createFanoutMsgStanza({
				chatId: d,
				deviceList: r,
				metricReporter: a,
				msgProtobuf: i,
			...
			yield o("WAWebSignalProtocolStore").getSignalProtocolStore().flushBufferToDiskIfNotMemOnlyMode(), (t = a.sendPerfReporter) == null || t.startWrittenWireStage(), o("WALogger").LOG(C || (C = babelHelpers.taggedTemplateLiteralLoose(["encryptAndSendStatusDirectMsg: start sending ", ""])), u.id), yield o("WAWebDeprecatedSendIqWorkerCompatible").deprecatedSendStanzaAndReturnAck(p, o("WAWebCommsAckParser").toCoreAckTemplate({
				id: u.id,
				class: _,
				from: c,
				participant: null
	...
	l.calculateRevokeSenderList = b, l.encryptAndSendStatusMsg = R, l.genMessageBody = E, l.maybeApplyStatusTag = I, l.encryptAndSendStatusDirectMsg = x;
}), 98);
