__d("WAWebExportChatAction", [
	...
	"use strict";
	var e, s, u, c = 50, d = 15 * 1024 * 1024, m = 3e4, p = 1e5, _ = "media";
	function f(e) {
	...
	function C() {
		return C = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = e.chat, a = e.endDate, i = e.messageLimit, l = e.onProgress, s = e.signal, u = e.startDate, d = [], m = { remote: t.id }, p = (function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
					if (!(d.length >= i)) {
						if (s != null && s.aborted) throw new (o("WAAbortError")).AbortError("Export cancelled");
						...
						if (!(e.status >= 400 || e.messages.length === 0)) {
							...
							for (var n of e.messages) {
								var _, f, g = (_ = n.t) != null ? _ : 0, y = (f = n.type) != null ? f : "";
								if (u != null && g < u) {
									t = !0;
									continue;
								}
								if (!h.has(y) && n.subtype !== "change_username" && n.isViewOnce !== !0) {
									var C = n.ephemeralDuration;
									if (!(C != null && C !== 0) && !(a != null && g > a) && (d.push(n), d.length >= i)) break;
								}
							}
							var b = e.messages[0], v = Number.isFinite(i) ? i : -1;
							if (l == null || l("loading", d.length, v), !t && d.length < i && (b == null ? void 0 : b.id) instanceof r("WAWebMsgKey")) return m = b.id, p();
						}
					}
				});
				return function() {
			...
			return yield p(), d.sort(function(e, t) {
				var n, r;
	...
	function v() {
		return v = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			var a, i = t.chat, l = t.endDate, u = t.includeMedia, c = t.onProgress, h = t.signal, C = t.startDate, b = f(i), v = u ? "with_media" : "text_only", S = C != null || l != null;
			if (g) {
			...
			var E = Date.now(), k = (a = t.messageLimit) != null ? a : p, I = "loading", T = 0;
			g = !0;
			try {
				...
				if (T = D.length, h != null && h.aborted) throw new (o("WAAbortError")).AbortError("Export cancelled");
				var x = D.map(function(e) {
					return o("WAWebMsgModelFromData").msgModelFromMsgData(e);
				}), $ = i.formattedTitle || i.name || "Chat", P = D.length >= k, N = [], M = new Set();
				if (u) {
					I = "downloading_media";
					...
					if (c == null || c("downloading_media", 0, w.length), h != null && h.aborted) throw new (o("WAAbortError")).AbortError("Export cancelled");
					yield w.reduce((function() {
						var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, n) {
							if (yield e, h != null && h.aborted) throw new (o("WAAbortError")).AbortError("Export cancelled");
							try {
								var r = yield o("WAPromiseDelays").withTimeout(o("WAWebFileSaverDownloadData").getMsgDownloadData(t), m, o("WAWebNullFunc").returnNull);
								if ((r == null ? void 0 : r.blob) != null && r.blob.size <= d) {
									var a;
									N.push({
										blob: r.blob,
										name: _ + "/" + r.name
				...
				if (h != null && h.aborted) throw new (o("WAAbortError")).AbortError("Export cancelled");
				I = "saving", c == null || c("saving", 0, 1);
				var O = $.replace(/[/\\?%*:|\"<>]/g, "_"), B = new Blob([F], { type: "text/plain" }), W = new Blob([A], { type: "text/markdown" }), q = [{
					blob: B,
					name: "chat.txt"
				}, {
					blob: W,
					name: "chat.md"
				}].concat(N), U = yield o("WAWebZipUtils").zipFiles(q);
				yield o("WAWebFileSaver").FileSaver.downloadData(U, O, o("WAWebFileSaverTypes").AllowedFileExtensions.ZIP);
				var V = Date.now() - E, H = {
				...
				throw t instanceof o("WAAbortError").AbortError ? o("WAWebExportChatLogging").logExportChat({
					...
					", includeMedia=",
					")"
				])), b, I, T, String(u)).catching(r("getErrorSafe")(t)).sendLogs("export-chat-failure"), o("WAWebExportChatLogging").logExportChat({
					exportMode: v,
					chatType: b,
					messageCount: T,
					mediaCount: 0,
