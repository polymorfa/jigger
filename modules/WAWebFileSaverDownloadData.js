__d("WAWebFileSaverDownloadData", [
	...
	function d() {
		return d = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t, a) {
			...
			try {
				var l = yield (u || (u = n("Promise"))).all(i.map(function(e) {
					return p(e, a);
				}));
				if (l.length > 1) {
				...
				throw c.name !== o("WAAbortError").ABORT_ERROR && o("WALogger").ERROR(e || (e = babelHelpers.taggedTemplateLiteralLoose(["Failed to get msg download data"]))).catching(c).sendLogs("msg-download-data-failed"), c;
			}
	...
	function _() {
		return _ = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			if (e.type === o("WAWebMsgType").MSG_TYPE.ALBUM) {
				var i = o("WAWebMessageAssociationUIUtils").getHiddenAssociatedMessages(e.id);
				if (i.length > 0) {
					var l = yield (u || (u = n("Promise"))).all(i.map(function(e) {
						return p(e, t);
					}));
	...
	l.getMultiMsgDownloadData = c, l.isMsgMediaAvailableLocally = m, l.getMsgDownloadData = p;
}), 98);
