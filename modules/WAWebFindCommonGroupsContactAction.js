__d("WAWebFindCommonGroupsContactAction", [
	...
	var e, s, u, c = 2, d = 4, m = 3;
	function p(e) {
		var t = e.split(/\s+/).filter(Boolean);
		return t.length < c || t.length > d ? !1 : t.every(function(e) {
			return e.length >= m;
		});
	}
	function _(t) {
		var a = o("WAWebStateUtils").unproxy(t);
		if (o("WAWebContactGetters").getIsMe(a)) return (u || (u = n("Promise"))).resolve(null);
		var i = a.commonGroups, l = a.id, s = a.promises;
		return s.findCommonGroups ? s.findCommonGroups : i && !i.stale ? (i.set(i.filter(function(e) {
			return !e.isParentGroup;
		})), (u || (u = n("Promise"))).resolve(i)) : s.findCommonGroups = f([a.id, o("WAWebApiContact").getAlternateUserWid(o("WAWebWidFactory").asUserWidOrThrow(a.id))].filter(Boolean)).then(function(e) {
			var t = r("compactMap")(e, function(e) {
				return o("WAWebChatCollection").ChatCollection.get(e);
			}).filter(function(e) {
				return e.isParentGroup !== !0 && !e.isLocked;
			});
			return i ? (i.set(t), i.stale = !1) : a.commonGroups = new (r("WAWebCommonGroupsCollection"))(t, l), a.commonGroups;
		}).catch(o("WAFilteredCatch").filteredCatch(o("WAWebBackendErrors").ServerStatusCodeError, function(t) {
			return o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose(["models:Contact:findCommonGroups error: ", ""])), t.status), (u || (u = n("Promise"))).reject(r("err")("models:Contact:findCommonGroups error: " + t.status));
		})).finally(function() {
	...
	function g() {
		return g = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = yield o("WAWebSchemaParticipant").getParticipantTable().anyOf(["participants"], e.map(function(e) {
				return e.toString();
			})), n = new Map(t.map(function(e) {
				return [e.groupId, o("WAWebGroupsParticipantsApi").checkMyMembershipForParticipantRecord(e)];
			})), r = t.map(function(e) {
				return e.groupId;
			}), a = yield o("WAWebSchemaGroupMetadata").getGroupMetadataTable().anyOf(["id"], r), i = a.filter(function(e) {
	...
	function y() {
		return y = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			if (t.length !== 0) {
				...
				var _;
				try {
					_ = yield o("WAWebSchemaParticipant").getParticipantTable().anyOf(["participants"], u);
				} catch (e) {
					var f = String(e);
				...
				var g = yield o("WAWebSchemaGroupMetadata").getGroupMetadataTable().anyOf(["id"], _.map(function(e) {
					return e.groupId;
	...
	function C(e, t) {
		...
		var f = e.length, g = [], h = new Map();
		return d.forEach(function(t, n) {
			h.set(n, b(t, e, f)), g.push([n, t]);
		}), g.sort(function(e, t) {
			var n, r, o = (n = h.get(e[0])) != null ? n : 0, a = (r = h.get(t[0])) != null ? r : 0;
	...
	l.shouldRunMultiContactTokenSearch = p, l.findCommonGroups = _, l.findCommonGroupsForContacts = h, l.findGroupsWithContactGroups = C;
}), 98);
