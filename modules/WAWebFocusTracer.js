__d("WAWebFocusTracer", [
	...
	function y(e) {
		...
		if (e instanceof Element) {
			...
			return "<" + e.tagName.toLowerCase() + (e.id ? "#" + e.id : "") + (t != null ? " [testid=" + t + "]" : "") + ">";
		}
	...
	function C(t, n) {
		if (t && !(m.length && t === m[m.length - 1])) {
			if (p = p || window.event != null, g(t), f() && !p) {
				o("WALogger").ERROR(e || (e = babelHelpers.taggedTemplateLiteralLoose(["Focus loop encountered."]))).sendLogs("focus-loop");
				return;
			}
