__d("WAWebFormatChangeUsernameTemplateText", [
	"fbt",
	"WALogger",
	...
	var e, u, c, d = function(t) {
		var e = t.newUsername, n = t.phoneNumber;
		return s._(
			/*BTDS*/
			"",
		...
		var e = t.newUsername, n = t.savedName;
		return s._(
			/*BTDS*/
			"",
		...
		var e = t.newUsername, n = t.oldUsername;
		return s._(
			/*BTDS*/
			"",
	...
	function S(t) {
		if (t.templateParams.length === 1) return s._(
			/*BTDS*/
			"",
			[s._param("newUsername", t.templateParams[0].toString())]
		);
		var n = t.templateParams[1];
		if (typeof n != "string") return o("WALogger").ERROR(e || (e = babelHelpers.taggedTemplateLiteralLoose(["[formatChangeUsernameTemplateText] invalid newUsername"]))).sendLogs("formatChangeUsernameTemplateText-newUsername-not-a-string"), "";
		var a = t.templateParams[0];
		if (typeof a != "string") return o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose(["[formatChangeUsernameTemplateText] invalid oldUsername"]))).sendLogs("formatChangeUsernameTemplateText-oldUsername-not-a-string"), "";
		var i = a === "", l = n === "", d = o("WAWebFrontendMsgGetters").getChat(t), m = r("WAWebWid").isGroup(d.id), p = t.templateParams[2];
		if (p == null) return "";
		var _ = o("WAWebContactCollection").ContactCollection.get(p), f = _ ? o("WAWebFrontendContactGetters").getFormattedSavedNameOrPushnameWithType(_) : null, g = f != null && f.type === o("WAWebWamEnumOppositeVisibleIdentificationType").OPPOSITE_VISIBLE_IDENTIFICATION_TYPE.SAVED_CONTACT_NAME ? f.displayName : null, h = o("WAWebWidFormat").widToFormattedUser(o("WAWebLidMigrationUtils").toPn(o("WAWebWidFactory").createUserWidOrThrow(p.toString()))), y = m ? "group" : "oneOnOne", C = r("isStringNullOrEmpty")(g) ? "noSavedName" : "savedName", b = r("isStringNullOrEmpty")(h) ? "noPhoneNumber" : "phoneNumber", S;
		i ? S = "created" : l ? S = "deleted" : S = "updated";
		var R = _ != null && _.isBusiness ? "business" : "consumer", L = y + "_" + C + "_" + b + "_" + S + "_" + R, E = v[L];
		if (E == null) return o("WALogger").ERROR(c || (c = babelHelpers.taggedTemplateLiteralLoose(["[formatChangeUsernameTemplateText] invalid lookupKey ", ""])), L).sendLogs("formatChangeUsernameTemplateText-lookupKey-not-valid"), "";
		var k = E({
			newUsername: n,
			savedName: g != null ? g : "",
