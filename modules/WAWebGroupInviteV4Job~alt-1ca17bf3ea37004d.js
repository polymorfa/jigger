__d("WAWebGroupInviteV4Job", [
	...
	async function u(t, n) {
		var r = await o("WASmaxGroupsRevokeRequestCodeRPC").sendRevokeRequestCodeRPC({
		...
		switch (r.name) {
			case "RevokeRequestCodeResponseSuccess": {
				...
				if (o("WAWebUsernameGatingUtils").usernameDisplayedEnabled()) {
					...
					i.length > 0 && await o("WAWebSetUsernameJob").setUsernamesJob(i);
				}
				return { status: 200 };
			}
			case "RevokeRequestCodeResponseClientError": {
				var l = r.value.errorClientErrors.value, u = l.code, c = l.text;
