__d("WAWebHandleAccountSyncNotification", [
	...
	function f() {
		return f = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var t = o("WAWebUserPrefsMeUser").getMeUserOrThrow(), r;
			if (o("WAWebABProps").getABPropConfigValue("mex_usync_about_status")) {
				...
				if (s.error) return o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose([
					"getStatus failed for ",
					": failed ",
					" : ",
					""
				])), t, s.error.errorCode, s.error.errorText), (d || (d = n("Promise"))).reject(s.error.errorText);
				r = s.status;
			}
			r !== "" && (yield o("WAWebBackendApi").frontendSendAndReceive("setMyStatus", { status: r }));
	...
	function C() {
		return C = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = p.parse(e);
			if (t.error) return o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose(["Parsing Error: ", ""])), t.error.toString()), (d || (d = n("Promise"))).reject(t.error);
			var r = t.success, a = o("WAWap").wap("ack", {
			...
			switch (r.type) {
				...
				case o("WAWebAccountSyncJob").AccountSyncType.TEXT_STATUS: {
					...
					var i = r.emoji, l = r.ephemeralDurationSeconds, s = r.lastUpdateTime, m = r.text, f = o("WAWebWidFactory").asUserWidOrThrow(r.from);
				...
				case o("WAWebAccountSyncJob").AccountSyncType.DEVICES: {
					if (o("WAWebOfflineHandler").OfflineMessageHandler.isResumeFromRestartComplete()) {
						if (o("WAWebOfflineHandler").OfflineMessageHandler.isResumeOnSocketDisconnectInProgress()) return yield o("WAWebApiPendingDeviceSync").addUserToPendingDeviceSync([r.from.toString()]), a;
					} else return o("WAWebOfflineDeviceCache").OfflinePendingDeviceCache.addOfflinePendingDevice(r.from.toString(), a), "NO_ACK";
					var h = r.devices;
					if (h) {
						var y, C, v = yield g({
						...
						var S = new Set((y = (C = h.deviceList) == null ? void 0 : C.map(function(e) {
							return e.id;
						})) != null ? y : []);
						yield o("WAWebBizBroadcastDeviceRemovalCleanup").cleanupCampaignsWithInvalidDevices(S);
				...
				case o("WAWebAccountSyncJob").AccountSyncType.PRIVACY:
					o("WAWebBackendEventBus").BackendEventBus.triggerAccountSyncForPrivacy(yield o("WAWebAccountSyncJob").forceUpdatePrivacySettings());
					break;
				case o("WAWebAccountSyncJob").AccountSyncType.BLOCKLIST:
					o("WAWebUsernameGatingUtils").usernameDisplayedEnabled() && r.usernames != null && (yield o("WAWebSetUsernameJob").setUsernamesJob(r.usernames)), o("WAWebFetchAndUpdateBlocklistJob").fetchAndUpdateBlocklist("notification");
					break;
				case o("WAWebAccountSyncJob").AccountSyncType.OPTOUTLIST: {
					var R = o("WAWebUserPrefsMultiDevice").getOptOutListHash(), L = r.dhash, E = r.list, k = r.prevDhash;
					if (R !== k) {
						o("WAWebWorkerSafeBackendApi").workerSafeFireAndForget("updateOptOutList");
						break;
					} else L != null && (E == null || E.forEach((function() {
				...
				case o("WAWebAccountSyncJob").AccountSyncType.NOTICE: {
					var I = r.noticeId, T = r.noticeStage, D = r.noticeTimestamp, x = r.noticeVersion;
					if (I != null && I !== "" && T != null && x != null && D != null) {
	...
	function v() {
		return v = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			if (t === "modify") {
				var i = yield o("WAWebGetDisappearingModeJob").getDisappearingMode(n, o("WAWebContactSyncLogger").SYNC_REQUEST_ORIGIN.DM_FORCE_REFRESH);
			...
			r != null && a != null && (yield o("WAWebUpdateDisappearingModeForContact").updateDisappearingModeForContact({
				contactId: n,
				newDuration: r,
