__d("WAWebJobUserExposureToQuickPromotion", [
	...
	function d(t) {
		...
		return o("WAWebModelStorageUtils").getStorage().lock(["quick-promotions"], (function() {
			var t = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
				...
				if (r.length === 0) return o("WALogger").ERROR(e || (e = babelHelpers.taggedTemplateLiteralLoose(["userExposureToQuickPromotion: missing experiment key"]))).sendLogs("user-exposure-quick-promotion-missing-exp-key"), {
					status: "missing-key",
					whatsappGkEnabled: !1
				};
				if (a == null) return {
				...
				if (c != null && c.experimentKey === r && c.exposureHoldout === a) return {
					status: "deduped",
					whatsappGkEnabled: s
				};
				...
				return yield n.merge(i, { tracking: d }), {
					status: "updated",
					whatsappGkEnabled: s
				};
			});
	...
	function p() {
		return p = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			e.whatsappGkEnabled ? g(r) : yield _({
				experimentKey: n,
				id: r,
				ts: a
			});
	...
	function f() {
		return f = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = e.experimentKey, n = e.id, r = e.ts, a = yield o("WASmaxInAppCommsEventRPC").sendEventRPC({
				eventType: "exposure",
				eventPromotionId: n,
				eventTimestampSec: r,
				eventLogdata: t
	...
	var h = o("WAWebDefinePersistedJob").defineWebPersistedJob().step("saveToDb", d).finalStep("reportToComms", m).end();
