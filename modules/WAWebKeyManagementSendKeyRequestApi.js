__d("WAWebKeyManagementSendKeyRequestApi", [
	"Promise",
	"WALogger",
	"WASyncdKeyTypes",
	...
	function m() {
		return m = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			...
			if (a.length === 0) throw r("err")("syncd: sendAppStateSyncKeyRequest: no peer devices available to request key from");
			var i = t.map(function(e) {
				return { keyId: o("WASyncdKeyTypes").fromSyncKeyId(e) };
			}), l = { keyIds: i }, d = a.map(function(e) {
			...
			o("WALogger").LOG(e || (e = babelHelpers.taggedTemplateLiteralLoose([
				...
				" to peer deviceIds ",
				""
			])), p, m), yield o("WAWebApiPeerMessageStore").storePeerMessages(d);
			var _ = yield (c || (c = n("Promise"))).allSettled(d.map(function(e) {
				return o("WAWebSendAppStateSyncMsgJob").encryptAndSendKeyMsg({ msg: e });
			})), f = _.filter(function(e) {
				return e.status === "rejected";
			});
			if (f.length > 0 && f.length < _.length) o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose([
				...
				throw o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose([
					...
					" peers failed: ",
					""
				])), _.length, g), r("err")("syncd: sendAppStateSyncKeyRequest failed for all " + _.length + " peer device(s): " + g);
			}
			var h = m.filter(function(e, t) {
				return _[t].status === "fulfilled";
			});
			return o("WAWebSyncdCriticalBootstrapProcessingApi").logCriticalBootstrapStageIfNecessary(o("WAWebWamEnumBootstrapAppStateDataStageCode").BOOTSTRAP_APP_STATE_DATA_STAGE_CODE.MISSING_KEYS_REQUESTED), h;
