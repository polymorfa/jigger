__d("WAWebKeyManagementSendKeyRequestApi", [
	...
	async function c(t) {
		...
		if (n.length === 0) throw r("err")("syncd: sendAppStateSyncKeyRequest: no peer devices available to request key from");
		var a = t.map(function(e) {
			return { keyId: o("WASyncdKeyTypes").fromSyncKeyId(e) };
		}), i = { keyIds: a }, l = n.map(function(e) {
		...
		o("WALogger").LOG(e || (e = babelHelpers.taggedTemplateLiteralLoose([
			...
			" to peer deviceIds ",
			""
		])), d, c), await o("WAWebApiPeerMessageStore").storePeerMessages(l);
		var m = await Promise.allSettled(l.map(function(e) {
			return o("WAWebSendAppStateSyncMsgJob").encryptAndSendKeyMsg({ msg: e });
		})), p = m.filter(function(e) {
			return e.status === "rejected";
		});
		if (p.length > 0 && p.length < m.length) o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose([
			...
			throw o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose([
				...
				" peers failed: ",
				""
			])), m.length, _), r("err")("syncd: sendAppStateSyncKeyRequest failed for all " + m.length + " peer device(s): " + _);
		}
		var f = c.filter(function(e, t) {
			return m[t].status === "fulfilled";
		});
		return o("WAWebSyncdCriticalBootstrapProcessingApi").logCriticalBootstrapStageIfNecessary(o("WAWebWamEnumBootstrapAppStateDataStageCode").BOOTSTRAP_APP_STATE_DATA_STAGE_CODE.MISSING_KEYS_REQUESTED), f;
	}
