__d("WAWebLogReceivedMessages", [
	...
	function p() {
		return p = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			yield (c || (c = n("Promise"))).all(e.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					o("PaymentLinkWamLogger").shouldLogReceiverEvent(e) && (yield o("PaymentLinkWamLogger").genLogReceiveEvent({
						interaction_component: null,
						msg: e
					}, e.matchedText).catch(function(e) {
						o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose(["error logging payment link message receive: ", ""])), String(e));
					}));
				});
				return function(t) {
	...
	function f() {
		return f = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, r) {
			var a, i = e.clientReceivedTsMillis, l = e.localAddressingMode, s = e.msgProcessStartTsMillis, u = e.msgs, d = e.offline, m = e.oppositeHasUsername, p = e.serverAddressingMode, _ = e.tsMillis, f = o("WATimeUtils").unixTimeMs(), g = yield (c || (c = n("Promise"))).all([o("WAWebChatThreadLoggingUtils").getMeHasUsername(), o("WAWebChatThreadLoggingUtils").getMeHasUsernamePin()]), h = g[0], y = g[1], C = (a = r.get(o("WAWebUserPrefsMeUser").getMeUserOrThrow().toJid())) == null ? void 0 : a.ephemeralDuration;
			yield c.all(u.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, n) {
					var a = e.from;
					if (a != null) {
						var u = o("WAWebWamEnumChatOriginsType").CHAT_ORIGINS_TYPE.OTHERS;
						a.isLid() && (u = o("WAWebWamEnumChatOriginsType").CHAT_ORIGINS_TYPE.LID_CTWA);
						var c = t[n];
						c != null && c.lidOriginType && (u = c.lidOriginType === o("WAWebUsernameTypes").LidOriginType.PNH_CTWA ? o("WAWebWamEnumChatOriginsType").CHAT_ORIGINS_TYPE.LID_CTWA : c.lidOriginType === o("WAWebUsernameTypes").LidOriginType.GENERAL ? o("WAWebWamEnumChatOriginsType").CHAT_ORIGINS_TYPE.OTHERS : (function() {
							throw Error("Match: No case succesfully matched. Make exhaustive or add a wildcard case using '_'. Argument: " + c.lidOriginType);
						})());
						var g = yield o("WAWebChatThreadLoggingUtils").getOppositeVisibleIdentification(a), b = a.isGroup() ? e.from.isLid() : o("WAWebWamMsgUtils").msgIsLid(e, a), v = new (o("WAWebMessageReceiveWamEvent")).MessageReceiveWamEvent({
						...
						if (d != null && (v.offlineCount = d), !a.isGroup() && !a.isStatus()) {
							var k, I = (k = r.get(a.toJid())) == null ? void 0 : k.ephemeralDuration;
							I != null && (v.senderDefaultDisappearingDuration = I), C != null && (v.receiverDefaultDisappearingDuration = C), v.isLid = a.isLid(), m != null && o("WAWebUsernameGatingUtils").usernameAdoptionAndEngagementMonitoringEnabled() && (v.oppositeHasUsername = m);
						}
						o("WAWebMsgGetters").getIsRevoke(e) && (v.revokeType = e.subtype === "admin_revoke" || e.subtype === "admin" ? o("WAWebWamEnumRevokeType").REVOKE_TYPE.ADMIN : o("WAWebWamEnumRevokeType").REVOKE_TYPE.SENDER), i != null && (v.messageReceiveT0 = i - _, v.messageReceiveT1 = f - i, s != null && (v.messageQueueTime = s - i)), e.ephemeralDuration != null && e.ephemeralDuration > 0 && (v.ephemeralityDuration = e.ephemeralDuration);
						...
						var A = yield o("WAWebWamGroupMetadataMetricUtils").getGroupTypeFromChatWid(a);
						if (A != null && (v.typeOfGroup = A), p != null && (v.serverAddressingMode = o("WAWebWamAddressingModeUtils").getWamAddressingModeFromString(p)), l != null && (v.localAddressingMode = o("WAWebWamAddressingModeUtils").getWamAddressingModeFromString(l)), o("WAWebMsgGetters").getType(e) === o("WAWebMsgType").MSG_TYPE.STICKER && (v.stickerIsPremium = e.stickerPremiumStatus === o("WAWebStickerPremiumStatus").StickerPremiumStatus.PREMIUM), a != null && a.isGroup()) {
							var F = yield o("WAWebWamGroupMetadataMetricUtils").isCagFromChatWid(a), O = o("WAWebMsgGetters").getIsReaction(e);
							F != null && O != null && (v.isLid = F && O);
							var B = yield o("WAWebWamGroupMetricCache").getGroupMetrics(a);
	...
	function h() {
		return h = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			o("WALogger").LOG(u || (u = babelHelpers.taggedTemplateLiteralLoose(["getContactData: for ", " msgs"])), e.length);
			...
			return o("WAWebRuntimeEnvironmentUtils").isWorker() ? (a = yield r("WAWebLidAwareContactsDB").bulkGet(n), new Map(a.map(function(e, t) {
				return [n[t], {
	...
	function v() {
		return v = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = yield (c || (c = n("Promise"))).all([y(e), g(e)]), r = t[0], o = t[1];
			return {
				chatData: r,
				contactData: o
			};
	...
	function I() {
		return I = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			if (r !== "missing-stanza-id") {
				...
				if (o("WAWebMsgGetters").getIsReply(e) && a == null && (a = yield L(r)), a != null && o("WAWebMsgGetters").getIsGroupStatus(a)) {
					var i = o("WAWebMsgGetters").getIsReply(e), l = o("WAWebMsgGetters").getIsReaction(e) && e.reactionText === d;
					if (i || l) {
	...
	function D(e) {
		(c || (c = n("Promise"))).all(e.filter(o("WAWebChatThreadLoggingUtils").shouldIncrementMsgSendAndReceive).map(k)).then(o("WAWebChatThreadLogging").handleActivitiesForChatThreadLogging);
	}
	function x(e) {
	...
	function $(t) {
		...
		b(r).then(function(e) {
			return (c || (c = n("Promise"))).all([
				...
				x(r),
				m(r),
				o("WAWebGalaxyFlowWamLoggerUtils").logStructuredMessageReceivedWAMEvent(r),
				o("WAWebOrderDetailsReceivedWamLogger").logOrderDetailsReceivedWAMEvent(r),
				o("WAWebPaymentInfoReceivedWamLogger").logPaymentInfoReceivedWAMEvent(r),
				o("WAWebPaymentRequestWamLogger").logPaymentRequestReceivedWAMEvent(r),
				o("WAWebQbmIncomingMessageLogger").logQbmIncomingMessages(r, e.chatData),
				T(r),
				o("WAWebUprReceivedWamLogger").logUprReceivedWAMEvent(r)
			]);
		}).catch(function(t) {
			o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose(["error logging received messages: ", ""])), String(t));
		});
	}
