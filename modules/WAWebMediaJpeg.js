__d("WAWebMediaJpeg", [
	...
	function e(e) {
		...
		return t.writeBytes(255, 216, 255, 224, 0, 16, 74, 70, 73, 70, 0), t.writeUint16(e.version), t.writeUint8(e.densityUnits), t.writeUint16(e.xDensity), t.writeUint16(e.yDensity), t.writeBytes(0, 0), t.readByteArrayView();
	}
	function s(t) {
		var n = new (o("WABinary")).Binary(t), a = new (o("WABinary")).Binary(t), i = new (o("WABinary")).Binary(), l = 2, s = 2, u = !1, c = !1, d = !1, m = !1, p, _ = {
			version: 257,
			densityUnits: 0,
			xDensity: 1,
			yDensity: 1
		};
		if (!(n.readUint8() === o("WAProgressiveJpegMarkers").MARKER_PREFIX && n.readUint8() === o("WAProgressiveJpegMarkers").MARKERS.SOI)) throw r("err")("SOI marker not at the start of the file.");
		a.advance(l);
		for (var f = void 0; f != null || n.size();) {
			if (f === void 0) {
				var g = n.readUint8();
				if (g !== o("WAProgressiveJpegMarkers").MARKER_PREFIX) throw r("err")("0x" + g.toString(16) + " is not a marker prefix.");
				f = n.readUint8();
			}
			switch (f) {
				case o("WAProgressiveJpegMarkers").MARKERS.APP0:
					if (c) p = n.readUint16(), a.advance(p + l), n.advance(n.size() - a.size());
					else {
						if (p = n.readUint16(), n.readString(5) !== "JFIF\0") throw r("err")("APP0 marker missing 'JFIF' magic bytes.");
						m = !0, _.version = n.readUint16(), _.densityUnits = n.readUint8(), _.xDensity = n.readUint16(), _.yDensity = n.readUint16(), a.advance(p + l), n.advance(n.size() - a.size()), c = !0;
					}
					break;
				case o("WAProgressiveJpegMarkers").MARKERS.SOS: {
					...
					for (var h = void 0, y = void 0, C = n.readUint8(); n.size();) {
						if (y = n.readUint8(), h = C === o("WAProgressiveJpegMarkers").MARKER_PREFIX && !o("WAProgressiveJpegMarkers").SOS_STREAM.includes(y), h) {
							f = y;
							break;
						}
						C = y;
				...
				case o("WAProgressiveJpegMarkers").MARKERS.APP1:
					if (p = n.readUint16(), !d && n.readString(6) !== "Exif\0\0") throw r("err")("APP1 marker missing 'EXIF' magic bytes.");
				...
				case o("WAProgressiveJpegMarkers").MARKERS.COM:
					if (!d && !m) throw r("err")("Missing magic bytes marker.");
					p = n.readUint16(), a.advance(p + l), n.advance(n.size() - a.size());
					break;
				...
				case o("WAProgressiveJpegMarkers").MARKERS.DRI:
					if (!d && !m) throw r("err")("Missing magic bytes marker.");
					p = n.readUint16(), i.writeByteArray(a.readByteArrayView(l + p)), n.advance(n.size() - a.size());
					break;
				...
				case o("WAProgressiveJpegMarkers").MARKERS.APP15: throw r("err")("Received unexpected APP marker 0x" + f.toString(16));
				default: throw r("err")("Did not understand marker: 0x" + f.toString(16));
			}
			if (f = void 0, u) break;
		}
