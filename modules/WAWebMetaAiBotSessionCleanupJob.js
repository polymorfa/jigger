__d("WAWebMetaAiBotSessionCleanupJob", [
	...
	function p() {
		return p = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			if (o("WAWebABProps").getABPropConfigValue("ai_meta_ai_prekey_cleanup_enabled") && o("WAWebBotBaseGating").isBotEnabled()) {
				...
				try {
					yield f(), yield o("WAWebUserPrefsBot").setMetaAiBotSessionLastCleanupTs(o("WATimeUtils").unixTimeMs());
				} catch (e) {
	...
	function g() {
		return g = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e, t = yield o("WAWebDBDeviceListFanout").getFanOutList({ wids: [
			...
			if (t.length !== 0) {
				var r = yield o("WAWebSignalSessionApi").hasSignalSessions(t), a = t.filter(function(e, t) {
					return r[t];
				});
				a.length !== 0 && (yield (d || (d = n("Promise"))).all(a.map(function(e) {
					return o("WAWebSignalSessionApi").deleteRemoteSession(e);
				})), yield o("WAWebSignalProtocolStore").getSignalProtocolStore().flushBufferToDiskIfNotMemOnlyMode(), o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose(["[bot] cleaned up ", " stale bot session(s)"])), a.length));
			}
		}), g.apply(this, arguments);
	}
	l.maybeCleanupMetaAiBotSession = m, l.registerMetaAiBotSessionCleanupOnResume = _;
}), 98);
