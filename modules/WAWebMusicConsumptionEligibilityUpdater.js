		...
		return c = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t, n, r) {
			...
			if (l != null) {
				...
				if (u != null) {
					...
					if (c !== -1) {
						var m = n[c], p = (a = m.embeddedContent) == null ? void 0 : a.embeddedMusic;
						if (p != null) {
							var _ = p.countryBlocklist, f = p.musicContentMediaId;
							if (f != null) try {
								var g = yield s.load(), h = g.fetchMusicConsumptionAvailability, y = yield h(f, u), C = _ != null ? o("WAWebMusicParsingUtils").mapToCountryCode(_) : new Set(), b = C.has(u);
								if (y !== b) return;
								y ? C.delete(u) : C.add(u), yield d({
