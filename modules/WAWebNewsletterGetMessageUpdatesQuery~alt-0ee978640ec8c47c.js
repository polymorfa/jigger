__d("WAWebNewsletterGetMessageUpdatesQuery", [
	...
	async function s(e, t) {
		var n = t.count, r = t.cursor, a = t.since, i = await o("WASmaxNewslettersGetNewsletterMessageUpdatesRPC").sendGetNewsletterMessageUpdatesRPC({
			iqTo: e,
		...
		switch (i.name) {
			case "GetNewsletterMessageUpdatesResponseSuccess": {
				var l, s = i.value.messageUpdatesMessagesNewsletterMessageResponsePayloadMixin;
				return {
					updates: s.message,
			...
			case "GetNewsletterMessageUpdatesResponseServerError": {
				var c = i.value.errorIQErrorInternalServerErrorMixin, d = c.code, m = c.text;
				throw new (o("WAWebBackendErrors")).ServerStatusCodeError(Number(d), m);
			}
			case "GetNewsletterMessageUpdatesResponseClientError": {
				...
				switch (p.name) {
					case "ItemNotFoundIQErrorResponse": {
						...
						return Promise.reject(new (o("WAWebBackendErrors")).ServerStatusCodeError(Number(f), g));
					}
					case "RateLimitedIQErrorResponse": {
						var h = p.value.errorIQErrorRateOverlimitMixin, y = h.code, C = h.text;
						return Promise.reject(new (o("WAWebBackendErrors")).ServerStatusCodeError(Number(y), C));
					}
					...
					case "SuspendedIQErrorResponse": {
						...
						return Promise.reject(new (o("WAWebBackendErrors")).ServerStatusCodeError(Number(L), E));
					}
					case "UnavailableForLegalReasonsResponse": {
						...
						return Promise.reject(new (o("WAWebBackendErrors")).ServerStatusCodeError(Number(I), T));
					}
				}
			}
		}
