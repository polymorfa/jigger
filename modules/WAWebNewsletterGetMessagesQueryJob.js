__d("WAWebNewsletterGetMessagesQueryJob", [
	...
	function p() {
		return p = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			var r = t.cursor, a = t.messagesCount, i = t.qpl, l = t.queryArgs, c = u(r);
			a > o("WAWebNewsletterGatingUtils").getMaxMsgCountFromServer() && o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose(["[queryNewsletterMessages] ", " messages requested"])), a).tags("newsletter").sendLogs("newsletter-server-msg-count-exceeded"), i == null || i.markFetchStart();
			var d = yield o("WASmaxNewslettersGetNewsletterMessagesRPC").sendGetNewsletterMessagesRPC({
				queryNewsletterParamsMixinArgs: { queryNewsletterParamsArgs: l },
				newsletterMessageRequestPayloadMixinArgs: {
					messagesCount: Math.min(a, o("WAWebNewsletterGatingUtils").getMaxMsgCountFromServer()),
					messageDirectionsArgs: c
				}
			});
			switch (i == null || i.markFetchEnd(), d.name) {
				case "GetNewsletterMessagesResponseSuccess": {
					var m = d.value.messagesNewsletterMessageResponsePayloadMixin, p = m.message, _ = m.t;
					return {
						messages: p,
						end: p.length < a,
				...
				case "GetNewsletterMessagesResponseClientError": {
					...
					switch (f.name) {
						...
						case "NotAllowedIQErrorResponse": {
							var $ = f.value.errorIQErrorNotAllowedMixin, P = $.code, N = $.text;
							throw new (o("WAWebBackendErrors")).ServerStatusCodeError(P, N);
						}
					}
					break;
				}
				case "GetNewsletterMessagesResponseServerError": {
					var M = d.value.errorIQErrorInternalServerErrorMixin, w = M.code, A = M.text;
					return (s || (s = n("Promise"))).reject(new (o("WAWebBackendErrors")).ServerStatusCodeError(Number(w), A));
				}
			}
		}), p.apply(this, arguments);
	}
	l.queryNewsletterMessagesByJid = c, l.queryNewsletterMessagesByInviteCode = d;
}), 98);
