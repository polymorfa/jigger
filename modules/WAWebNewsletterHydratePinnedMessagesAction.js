__d("WAWebNewsletterHydratePinnedMessagesAction", [
		...
		return c = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, n, r) {
			...
			for (var c of u) {
				...
				if (!(a.has(c) || e.msgs.msgLoadState.isLoadingEarlierMsgs)) {
					a.add(c);
					try {
						yield o("WAWebNewsletterPullMessagesFromServerAction").pullNewsletterMessagesFromServer(e, {
							messageCount: 1,
							cursor: { before: c + 1 },
							signal: r
						});
	...
	function m() {
		return m = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t, a, i) {
			var l;
			try {
				l = yield o("WAWebNewsletterDBUtils").bulkGetMessagesByServerIds(Array.from(a), t.id.toJid());
			} catch (t) {
			...
			var u = [];
			for (var c of l.values()) u.push(o("WAWebMsgModelFromData").msgModelFromMsgData(c));
			return yield t.addQueue.enqueue((s || (s = n("Promise"))).resolve().then(function() {
				t.msgs.add(u, { at: 0 });
			})).then(r("WAWebNoop"), r("WAWebNoop")), new Set(l.keys());
