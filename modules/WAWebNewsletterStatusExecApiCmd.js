	...
	function y() {
		return y = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			...
			try {
				...
				var i = yield o("WAWebLoadNewsletterPreviewChatAction").loadNewsletterPreviewChat(n);
				if (i == null) {
					v();
					return;
				}
				...
				var s = yield o("WAWebNewsletterStatusFetchAction").fetchNewsletterStatuses(l);
				...
				if (a != null) {
					var g = parseInt(a, 10);
					if (!Number.isNaN(g) && g > 0 && (f = C(p, g), f == null && (o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose(["[newsletter-status-deeplink] status not found, backfilling"]))), yield o("WAWebNewsletterStatusGapFillAction").fillNewsletterStatusGap(l, g), f = C(p, g))), f == null) {
						S();
						return;
					}
				}
				o("WALogger").LOG(d || (d = babelHelpers.taggedTemplateLiteralLoose(["[newsletter-status-deeplink] Opening status viewer"])));
				var h = yield r("JSResourceForInteraction")("WAWebStatusViewer.react").__setRef("WAWebNewsletterStatusExecApiCmd").load();
				o("WAWebModalManager").ModalManager.openMedia(_.jsx(h, {
					initialStatus: p,
					initialStatusMsg: f,
				...
				o("WALogger").ERROR(m || (m = babelHelpers.taggedTemplateLiteralLoose(["[newsletter-status-deeplink] failed to open deeplink"]))).catching(r("getErrorSafe")(e)).tags("newsletter-status-deeplink").sendLogs("newsletter-status-deeplink-error"), v();
			}
