__d("WAWebNonAddressBookContactsJob", [
	...
	function g() {
		return g = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = new Set();
			return yield o("WAWebSchemaParticipant").getParticipantTable().forEach(function(t) {
				var n;
				(n = t.participants) == null || n.forEach(function(t) {
					var n = h(t);
					n != null && e.add(n);
				});
			}), yield o("WAWebSchemaChat").getChatTable().forEach(function(t) {
				if (r("WAWebWid").isEligibleForUSync(t.id)) {
					var n = h(t.id);
					n != null && e.add(n);
				}
			}), o("WALogger").LOG(u || (u = babelHelpers.taggedTemplateLiteralLoose(["found ", " contacts from chat and group participant tables"])), e.size).tags("contact-sync"), Array.from(e);
	...
	function h(e) {
		try {
			...
			return o("WALogger").ERROR(s || (s = babelHelpers.taggedTemplateLiteralLoose(["toUserJid: \"", "\" is not a valid UserJid"])), e).catching(r("getErrorSafe")(t)).sendLogs("contact-id-not-user-jid"), null;
		}
	}
	function y() {
	...
	function C() {
		return C = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = yield r("WAWebLidAwareContactsDB").anyOf(["isAddressBookContact"], [1, 0]), t = new Map(e.map(function(e) {
	...
	function R() {
		return R = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = yield f();
			return o("WAWebModelStorageUtils").getStorage().lock(["contact"], n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				var t = yield y(), n = new Set(), a = Array.from(e, function(e) {
					var r = {
						id: e,
				...
				o("WALogger").LOG(m || (m = babelHelpers.taggedTemplateLiteralLoose(["mark ", " contacts dirty during the regular sync"])), a.length).tags("contact-sync"), yield r("WAWebLidAwareContactsDB").bulkCreateOrMerge(a, "NonAddressBookContactsJob.getNonAddressBookContactsAndMarkAllContactsDirty"), t.forEach(function(e, t) {
					e == null && n.add(t);
				}), yield b(n, t);
			}));
	...
	function E() {
		return E = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = yield f();
			return o("WAWebModelStorageUtils").getStorage().lock(["contact"], n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				var t = yield y(), n = e.filter(function(e) {
					return !t.has(e);
				}), a = n.map(function(e) {
					return {
						id: e,
				...
				o("WALogger").LOG(p || (p = babelHelpers.taggedTemplateLiteralLoose(["get ", " non-AB contacts, mark dirty (initial sync)"])), a.length).tags("contact-sync"), yield r("WAWebLidAwareContactsDB").bulkCreateOrMerge(a, "NonAddressBookContactsJob.getAndUpdateNonAddressBookContacts");
	...
	l.getAllContactsFromChatCollectionIntoChunks = _, l.getNonAddressBookContactsAndMarkAllContactsDirty = S, l.getAndUpdateNonAddressBookContacts = L;
}), 98);
