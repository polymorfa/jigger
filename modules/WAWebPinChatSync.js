__d("WAWebPinChatSync", [
	...
	var e, s, u, c, d;
	function m(e) {
		return e === p.Chat ? function(e) {
			return !r("WAWebWid").isNewsletter(e);
		} : e === p.Newsletter ? function(e) {
			return r("WAWebWid").isNewsletter(e);
		} : (function() {
	...
	var f = (function(t) {
		...
		return i.getVersion = function() {
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
				...
				for (var t = [], n = 0; n < e.length; n++) t.push(yield this.applyMutation(e[n]));
				return t;
			});
			function t(t) {
			...
			var t = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
				if (t.operation === "remove") return o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose(["syncd: pin_chat_sync: REMOVE not supported"]))), (d || (d = n("Promise"))).resolve({ actionState: o("WAWebSyncdConst").SyncActionState.Unsupported });
				var a = t.indexParts, i = t.timestamp, l = t.value, m = a[1];
				try {
					if (!m) return o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose(["syncd: pin_chat_sync: missing chatJid in index"]))), this.malformedActionIndex();
					if (!r("WAWebWid").isWid(m)) return this.malformedActionIndex();
					var p = l.pinAction;
					if (p == null) return o("WALogger").WARN(u || (u = babelHelpers.taggedTemplateLiteralLoose(["syncd: pin_chat_sync: missing pinAction"]))), o("WAWebSyncdIndexUtils").malformedActionValue(this.collectionName);
					var _ = p.pinned;
					if (_ == null) return o("WALogger").WARN(c || (c = babelHelpers.taggedTemplateLiteralLoose(["syncd: pin_chat_sync: missing pinned field"]))), o("WAWebSyncdIndexUtils").malformedActionValue(this.collectionName);
					var f = yield o("WAWebSyncdGetChat").resolveChatForMutationIndex(o("WAWebWidFactory").createWid(m));
					if (!f.success) return {
						actionState: o("WAWebSyncdConst").SyncActionState.Orphan,
						orphanModel: f.orphanModel
					};
					var g = f.chat.id, h = f.chat.id, y = o("WAWebWidFactory").createWid(h);
					if (!_) return yield this.applyUpdates([{
						wid: y,
						pinned: !1,
						timestamp: i
					}]), { actionState: o("WAWebSyncdConst").SyncActionState.Success };
					var C = y.isNewsletter() ? yield this.getLocalNewsletterPins() : yield this.getLocalChatPins();
					if (C.some(function(e) {
						return e.chatId.toString() === y.toString();
					})) return yield this.applyUpdates([{
						wid: y,
						pinned: _,
						timestamp: i
					}]), { actionState: o("WAWebSyncdConst").SyncActionState.Success };
					if (C.length < o("WAWebChatPinBridge").getPinLimit(y)) return yield this.applyUpdates([{
						wid: y,
						pinned: _,
						timestamp: i
					}]), { actionState: o("WAWebSyncdConst").SyncActionState.Success };
					...
					}), v = [], S = b.timestamp < i ? b.chatId : y;
					return S === b.chatId && v.push({
						wid: b.chatId,
						pinned: !1,
						timestamp: i
					}, {
						...
						pinned: !0,
						timestamp: i
					}), yield (d || (d = n("Promise"))).all([this.applyUpdates(v), this.createPendingUnpin(S, i)]), d.resolve({ actionState: o("WAWebSyncdConst").SyncActionState.Success });
				} catch (e) {
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
				if (e.length !== 0) {
					...
					})), yield d.all(e.map((function() {
						var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
							var t = yield o("WAWebChatGetExistingBridge").getExisting(e.wid);
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
				yield o("WAWebSyncdDb").appendPendingMutationsRows([yield this.getPinMutation(t, !1, e)]);
			});
			function t(t, n) {
				return e.apply(this, arguments);
			}
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
				var t = o("WAWebSchemaChat").getChatTable().all().then(function(e) {
					return e.filter(function(e) {
						return e.pin != null && e.pin > 0;
					}).map(function(e) {
						return [e.id, r("nullthrows")(e.pin)];
					});
				}), a = o("WAWebSyncdDb").getPendingMutationsRows(["action"], o("WAWebSyncdConst").Actions.Pin).then(function(e) {
				...
				}), i = o("WAWebSyncdDb").getSyncActionsRows(["action"], [o("WAWebSyncdConst").Actions.Pin]).then(function(e) {
				...
				}), l = yield (d || (d = n("Promise"))).all([
					t,
					i,
					a
				]), s = l[0], u = l[1], c = l[2], p = r("compactMap")(u, function(e) {
					return c.has(e.index) ? null : e.pin;
				}), _ = m(e);
				return [].concat(s, p).filter(function(e) {
					var t = e[0];
					return _(t);
				}).map(function(e) {
					var t = e[0], n = e[1];
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				return this.$PinChatSyncImpl$p_2(yield this.$PinChatSyncImpl$p_1(p.Chat));
			});
			function t() {
				return e.apply(this, arguments);
			}
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				return this.$PinChatSyncImpl$p_2(yield this.$PinChatSyncImpl$p_1(p.Newsletter));
			});
			function t() {
				return e.apply(this, arguments);
			}
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
				var t = this, r = o("WATimeUtils").unixTimeMs(), a = yield (d || (d = n("Promise"))).all(e.map(function(e) {
					var n = e.chatId;
					return t.getPinMutation(r, !1, n);
				}));
				return o("WAWebSyncdCoreApi").lockForSync(["chat"], a, function() {
					return (d || (d = n("Promise"))).resolve();
				}).then(function() {
					return t.applyUpdates(e.map(function(e) {
						var t = e.chatId;
						return {
							wid: t,
							pinned: !1,
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, n) {
				...
				return t && a.push(yield r("WAWebArchiveChatSync").getArchiveChatMutation(e, !1, n)), a;
			});
			function t(t, n, r) {
				return e.apply(this, arguments);
			}
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, n) {
				o("WAWebSyncdGetChat").warnIfPnMutationWithForcedLid(o("WAWebSyncdGetChat").PnMutationCaller.GetPinMutation, n);
				var r = { pinAction: { pinned: t } };
				return o("WAWebSyncdActionUtils").buildPendingMutation({
					collection: this.collectionName,
					indexArgs: [yield o("WAWebSyncdGetChat").getChatJidMutationIndexForChat(n, o("WAWebSyncdConst").Actions.Pin)],
					value: r,
	...
	})(o("WAWebSyncdAction").ChatSyncdActionBase), g = new f();
	l.PinChatSync = g;
}), 98);
