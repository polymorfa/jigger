__d("WAWebProcessMultipleMsgsAction", [
	...
	function g() {
		return g = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t, a, i, l, m) {
			var p, f, g = yield r("WAWebGetMsgUpdatesActionsUtils")(t, a, i, m), y = g.filteredRecs, b = g.reorderRecs, v = g.updates, S = yield (_ || (_ = n("Promise"))).all(v), R = S.filter(function(e) {
				return (e == null ? void 0 : e.associationType) != null;
			});
			if (o("WAWebMessageAssociationUIUtils").makeParentMessagesVisibleInChat(R), y.length === 0 && b.length === 0) {
				if (t && i.pendingMsgsDone === !0) {
					var L, E = (L = o("WAWebChatCollection").ChatCollection.get(t)) != null ? L : r("WAWebNewsletterCollection").get(t);
					E && (E.pendingMsgs = !1);
				}
			...
			if (!t) return C(y).then(function() {
				return a.reduce(function(e, t) {
					var n = o("WAWebMsgCollection").MsgCollection.get(t.id);
					return n != null && o("WAWebFrontendMsgGetters").getChat(n) != null && e.push(n), e;
				}, []);
			});
			var k = self.performance.now(), I, T = a[0];
			if (o("WAWebMsgGetters").getIsStatus(T)) {
				var D, x = (D = o("WAWebMsgGetters").getSender(T)) != null ? D : T.from;
				if (r("WAWebWid").isStatus(t) && (x = T.id.fromMe ? o("WAWebLidStatusMigrationUtils").matWidConvert(o("WAWebUserPrefsMeUser").getMeUserOrThrow()) : T.author), x == null) {
					var $, P, N;
					o("WALogger").LOG(e || (e = babelHelpers.taggedTemplateLiteralLoose([
						...
						" author=",
						""
					])), t.toLogString(), ($ = o("WAWebMsgGetters").getSender(T)) == null ? void 0 : $.toLogString(), (P = T.from) == null ? void 0 : P.toLogString(), T.id.fromMe, (N = T.author) == null ? void 0 : N.toLogString());
				}
				I = o("WAWebStatusCollection").StatusCollection.find(x);
			} else {
				if (yield h(t, a)) return o("WALogger").LOG(s || (s = babelHelpers.taggedTemplateLiteralLoose(["_processMultipleMessages: skipping ghost chat creation for ephemeral_setting-only message ", ""])), t.toLogString()), r("compactMap")(a, function(e) {
					return o("WAWebMsgCollection").MsgCollection.get(e.id);
				});
				I = yield o("WAWebFindChatAction").findExistingChat(t, l);
			}
			var M = yield I, w = self.performance.now(), A = yield C(y), F = self.performance.now(), O = i.add === "after" || i.add === "last", B = !i.isHistory, W, q, U = !1, V = M.id.isBot(), H = ((p = M.contact.businessProfile) == null ? void 0 : p.isBizBot3p) === !0;
			if ((V || H) && i.isHistory === !1) {
				var G = M.msgs.last();
				(G == null ? void 0 : G.subtype) === o("WAWebBotGenTypingIndicatorMsg").BOT_TYPING_PLACEHOLDER_MSG_SUBTYPE && (G == null || G.delete({ skipUpdatingSortTime: !0 }));
			}
			A.forEach(function(e) {
				e.subtype === "biz_bot_1p_disclosure" ? M.set({ bizBotSystemMsgType: o("WAWebBotTypes").BizBotType.BIZ_1P }) : e.subtype === "biz_bot_3p_disclosure" && M.set({ bizBotSystemMsgType: o("WAWebBotTypes").BizBotType.BIZ_3P });
			}), i.add === "unread" ? (W = m, M.msgChunks.push(W), q = i.firstUnreadKey) : i.add === "last" && i.resume === !0 ? (W = m, U = !0) : W = typeof m == "function" ? m() : m;
			var z = typeof W == "function" ? W() : W, j = M;
			if ((z == null ? void 0 : z.threadId) != null && M instanceof o("WAWebChatModel").Chat && (j = o("WAWebThreadModelResolver").resolveThreadOrChat(M, z.threadId)), r("WAWebAddAndReorderMsgsActionsUtils")({
				...
				threadOrChat: j
			}), F - k >= 500 && o("WALogger").LOG(u || (u = babelHelpers.taggedTemplateLiteralLoose([
				"[processMM]: ",
				" msgs findChat=",
				"ms msgPrepWork=",
				"ms"
			])), a.length, Math.round(w - k), Math.round(F - w)), M instanceof o("WAWebChatModel").Chat && (z == null || z.threadId == null) && o("WAWebThreadWriteThroughAction").writeThroughToLiveThreads(M, A), i.pendingMsgsDone === !0 && (M.pendingMsgs = !1), B && r("WAWebWid").isBroadcast(t) && !r("WAWebWid").isStatus(t) && A.forEach(function(e) {
				...
				o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose(["processed ", " recent sticker messages"])), K.length), K.forEach(function(e) {
					return o("WAWebRecentStickerCollectionMd").RecentStickerCollectionMd.addNewSticker(e.sticker, e.msgId, e.timestamp);
				});
			}
			var Q = 0, X = r("compactMap")(A, function(e) {
			...
			Q > 0 && o("WALogger").ERROR(d || (d = babelHelpers.taggedTemplateLiteralLoose(["[sticker] Unexpected null media key"]))).sendLogs("sticker-unexpected-null-media-key"), o("WAWebRecentStickerCollection").RecentStickerCollection.enqueue(X);
			var Y = A.filter(function(e) {
				return !o("WAWebMsgGetters").getIsStatus(e);
			});
			if (r("compactMap")(Y, function(e) {
			...
			if (r("compactMap")(A, function(e) {
				...
				var Z = o("WAWebUserPrefsGeneral").getLastStatusUsage(), ee = 1e3 * 60 * 60 * 24;
	...
	function b() {
		return b = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = o("WAWebMsgCollection").MsgCollection.add(e, { merge: !0 }), r = 0, a = t.reduce(function(e, t) {
				return t == null && r++, t != null && t.subtype !== "is_hosted_group" && e.push(t), e;
			}, []);
			if (r > 0) {
				...
				o("WALogger").ERROR(m || (m = babelHelpers.taggedTemplateLiteralLoose([
					...
					""
				])), r, i).sendLogs("msgPrepWork-empty-message");
			}
			var l = null, s = null;
			return yield (_ || (_ = n("Promise"))).all(a.map(function(e) {
				return e.waitForPrep().catch(function(t) {
					l = t, s = e;
				});
			})), l != null && s != null && o("WALogger").ERROR(p || (p = babelHelpers.taggedTemplateLiteralLoose([
				"[msgPrepWork] id=",
				" type=",
				" subtype=",
				" hasMedia=",
				""
			])), s.id.toString(), s.type, s.subtype, s.mediaData != null).catching(l).sendLogs("bad-msg-prep"), a;
