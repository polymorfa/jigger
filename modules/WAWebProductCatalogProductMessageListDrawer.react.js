__d("WAWebProductCatalogProductMessageListDrawer.react", [
	...
	function g(t) {
		var a = t.ref, i = babelHelpers.objectWithoutPropertiesLoose(t, e), l = i.catalogWid, s = i.onCartClick, c = i.onProductDetail, g = i.productListId, y = i.setScrollOffset, C = r("useWAWebUnmountSignal")(), b = d(o("WAWebDrawerContext").DrawerContext), v = r("useLazyRef")(function() {
			return new (r("WAWebFlatListController"))();
		}), S = o("useWAWebContactValues").useContactValues(i.contact.id, [o("WAWebContactGetters").getId]), R = S[0], L = p(!1), E = L[0], k = L[1], I = p(0), T = I[0], D = I[1], x = p(!1), $ = x[0], P = x[1], N = p(function() {
			return o("WAWebProductMessageListCollection").ProductMessageListCollection.get(g);
		}), M = N[0], w = N[1], A = p(function() {
			return o("WAWebCartCollection").CartCollection.findCart(R.toString());
		}), F = A[0], O = p(F.itemCount), B = O[0], W = O[1], q = p(!1), U = q[0], V = q[1], H = p(M ? "SUCCESS" : "NONE"), G = H[0], z = H[1], j = function(t) {
			var e = t === !0, n = o("WAWebProductMessageListCollection").ProductMessageListCollection.assertGet(g), r = n.getTotalProductsFetchedOrFailed(), a = n.getProductSize();
			if ($ || r === a) {
				e && (o("WAWebQplFlowWrapper").QPL.markerAnnotate(f, { int: { ProductsCount: r } }), o("WAWebQplFlowWrapper").QPL.markerEnd(f, 2)), P(!0);
				return;
			}
			k(!0), D(r);
			var i = e ? _ : void 0;
			o("WAWebProductMessageListCollection").ProductMessageListCollection.update(g, { markerId: i }).then(function(t) {
				if (C.aborted) {
				...
				a === T && P(!0), e && (o("WAWebQplFlowWrapper").QPL.markerAnnotate(f, { int: { ProductsCount: a } }), o("WAWebQplFlowWrapper").QPL.markerEnd(f, 2)), r * o("WAWebProductCatalogCatalogConstants").PRODUCT_LIST_ITEM_HEIGHT < window.innerHeight && j();
			}).catch(function() {
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				try {
					var e = yield o("WAWebProductMessageListCollection").ProductMessageListCollection.find(g);
					if (C.aborted) {
		...
		m(function() {
			M ? j() : K(), r("WAWebProductCatalogCheckCartEnabled")(R).then(function(e) {
				C.aborted || (V(e), o("WAWebProductCatalogLogEvents").logCatalogListView({
					catalogOwnerWid: l,
		...
		var Q = function(t) {
			E || t.scrollTop + o("WAWebFrontendConstants").SCROLL_FUDGE > t.scrollHeight - t.clientHeight && j();
		}, X = function(t) {
			Q(t.currentTarget), y == null || y(t.currentTarget.scrollTop);
		}, Y = function() {
			o("WAWebUtilsLogQplEvents").qplStartCartView("PLM"), s == null || s(R.toString());
		}, J = s != null && U ? u.jsx(r("WAWebCommonCartIconMenuBarItem.react"), {
			cartCountText: o("WATypeUtils").isNumber(B) && B > 0 ? B.toString() : void 0,
			onClick: Y,
