__d("WAWebSendAiMediaCollectionBatch", [
	"fbt",
	"Promise",
	"WALogger",
	...
	function _() {
		return _ = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			var a = t.botPersonaId, i = t.chat, l = t.medias, s = t.threadId;
			if (l.length !== 0) {
				f(i, l);
				var d = yield r("WAWebMsgKey").newId(), m = yield o("WAWebCreateAiMediaCollectionMsgData").createAiMediaCollectionMsgData({
					botPersonaId: a,
					collectionId: d,
					hasGlobalCaption: !1,
					chat: i,
				...
				try {
					yield o("WAWebSendAiMediaCollectionEnvelope").sendAiMediaCollectionEnvelope(i, m);
				} catch (n) {
					o("WALogger").ERROR(e || (e = babelHelpers.taggedTemplateLiteralLoose(["[ai-media-collection] envelope send failed: ", ""])), n).sendLogs("ai-media-collection-envelope-send-failed"), h(t);
					return;
				}
				var p = i.composeQuotedMsg;
				i.composeQuotedMsg = null, i.setAttachMediaContents(null);
				var _ = m.botPersonaId, g = m.botMetricsMetadata, y = m.aiThreadInfo, C = yield (c || (c = n("Promise"))).allSettled(l.map(function(e, t) {
					var n = {
						type: e.type,
						caption: e.caption,
						addEvenWhilePreparing: e.previewable && e.state === o("WAWebAttachMediaModel").ATTACH_MEDIA_STATE.PROCESSING,
						botPersonaId: _,
						threadId: s,
					...
					return t === 0 && (n.quotedMsg = p), e.sendToChat({
						chat: i,
						options: n
					});
				...
				b < l.length && o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose([
					"[ai-media-collection] ",
					"/",
					" child send fail; may buffer"
				])), l.length - b, l.length).sendLogs("ai-media-collection-child-send-shortfall");
			}
