__d("WAWebSendAiMediaCollectionBatch", [
	...
	async function m(t) {
		var n = t.botPersonaId, a = t.chat, i = t.medias, l = t.threadId;
		if (i.length !== 0) {
			p(a, i);
			var s = await r("WAWebMsgKey").newId(), c = await o("WAWebCreateAiMediaCollectionMsgData").createAiMediaCollectionMsgData({
				botPersonaId: n,
				collectionId: s,
				hasGlobalCaption: !1,
				chat: a,
			...
			try {
				await o("WAWebSendAiMediaCollectionEnvelope").sendAiMediaCollectionEnvelope(a, c);
			} catch (n) {
				o("WALogger").ERROR(e || (e = babelHelpers.taggedTemplateLiteralLoose(["[ai-media-collection] envelope send failed: ", ""])), n).sendLogs("ai-media-collection-envelope-send-failed"), _(t);
				return;
			}
			var d = a.composeQuotedMsg;
			a.composeQuotedMsg = null, a.setAttachMediaContents(null);
			var m = c.botPersonaId, f = c.botMetricsMetadata, g = c.aiThreadInfo, h = await Promise.allSettled(i.map(function(e, t) {
				var n = {
					type: e.type,
					caption: e.caption,
				...
				return t === 0 && (n.quotedMsg = d), e.sendToChat({
					chat: a,
					options: n
				});
			...
			y < i.length && o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose([
				"[ai-media-collection] ",
				"/",
				" child send fail; may buffer"
			])), i.length - y, i.length).sendLogs("ai-media-collection-child-send-shortfall");
		}
	}
