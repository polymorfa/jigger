__d("WAWebSendBroadcastMsgAction", [
		...
		return R = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t) {
			try {
				...
				for (var l of a) {
					...
					if (c == null) {
						o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:ctl] missing clone for fanoutKey=", ""])), l.toString());
						continue;
					}
					i.push(o("WAWebMsgUtilsBridge").logMessageSendForChatThreadLogging(c));
				}
	...
	function E() {
		return E = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			if (a != null) {
				var b = o("WAWebBroadcastEphemeralUtils").addSharedSecretToProto(y, a), v = b.injected, R = b.proto;
				y = R, v || (C = null);
			}
			o("WAWebBroadcastODS").bumpBroadcastSend();
			try {
				yield o("WAWebEncryptAndSendBroadcastMsg").encryptAndSendBroadcastMsg(h, y, s, t, n, C);
				var L = yield o("WAWebBatchUpdateBroadcastAck").batchUpdateAckForBroadcastMessages(l, o("WAWebAck").ACK.SENT);
				return L != null && o("WALogger").WARN(d || (d = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:send] Batch ack update failed after send: error=", ""])), L).sendLogs("broadcast-batch-ack-sent-" + L), o("WALogger").LOG(m || (m = babelHelpers.taggedTemplateLiteralLoose([
					...
					", ack=",
					""
				])), f, o("WAWebAck").ACK.SENT), o("WAWebBroadcastODS").bumpBroadcastSendSuccess(), u.postSuccess(), L == null && S(l), {
					messageSendResult: o("WAWebSendMsgResultAction").SendMsgResult.OK,
					msgId: f
				};
			} catch (e) {
				var E = yield o("WAWebBatchUpdateBroadcastAck").batchUpdateAckForBroadcastMessages(l, o("WAWebAck").ACK.FAILED);
	...
	function I() {
		return I = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = e.msgData, a = v(), i = yield (b || (b = n("Promise"))).all(e.recipients.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					try {
						var t = yield o("WAWebMessageProcessUtils").selectChatForOneOnOneMessage({ lid: e }), n = t.chatId;
			...
			var d = o("WAWebBuildBroadcastMsgModels").buildBroadcastMsgModelsFromMsgData(t, i), m = o("WAWebBuildBroadcastMsgModels").bulkBroadcastMessagesToBulkUpdateInput(d), p = m.chatsIdWithNewMsgs, _ = m.msgs;
			yield o("WAWebDBMessageBulkHelper").persistNewMessagesInBulk(_, p);
			var S = o("WAWebBroadcastMsgCollectionUtils").addMsgsToCollections(d.mainMessage, d.messageClones), R = new (o("WAWebMessageSendReporter")).MessageSendReporter(S, { frontendDeps: o("WAWebMessageSendReporterFrontendDeps").MAIN_WEB_MESSAGE_SEND_REPORTER_FRONTEND_DEPS });
			R.setParticipantCount(e.recipients.length);
			var E = e.beforeSend;
			try {
				E != null && (yield S.waitForPrep(), o("WALogger").LOG(g || (g = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:send] Media prep complete: msgId=", ""])), S.id), yield E(S), o("WALogger").LOG(h || (h = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:send] Media upload complete: msgId=", ""])), S.id), yield o("WAWebBuildBroadcastMsgModels").propagateMediaFieldsToBroadcastClones(S), o("WALogger").LOG(y || (y = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:send] media fields persisted to clones msgId=", ""])), S.id));
			} catch (e) {
				return o("WALogger").ERROR(C || (C = babelHelpers.taggedTemplateLiteralLoose(["[broadcast:send] Media prep/upload failed"]))).catching(r("getErrorSafe")(e)).sendLogs("broadcast-media-prep-upload-error"), o("WAWebBroadcastODS").bumpBroadcastSendError(), R.postFailure({
					isTerminal: !1,
		...
		return D = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, r) {
			var a = yield (b || (b = n("Promise"))).all(t.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					...
					return {
						lid: t,
						setting: yield o("WAWebBroadcastEphemeralUtils").getBroadcastEphemeralSettingForRecipient(n)
					};
				});
				return function(t) {
					return e.apply(this, arguments);
			...
			var i = o("WAWebEphemeralEncodeBroadcastSetting").generateEphemeralSharedSecret(), l = e.to, s = new Map(), u = [].concat(a, [{
				lid: r,
				setting: {
					duration: 0,
					timestamp: 0
			...
			return yield b.all(u.map((function() {
				var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
					var t = e.lid, n = e.setting, a = yield o("WAWebEphemeralEncodeBroadcastSetting").encodeBroadcastEphemeralSetting({
						broadcastJid: l,
						duration: n.duration,
						recipient: t,
						sender: r,
