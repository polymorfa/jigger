__d("WAWebSendMsgToDeviceList", [
	...
	function s() {
		return s = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			var f = m.data, g = f.id, h = f.to, y = yield o("WAWebSendMsgCreateFanoutStanza").createFanoutMsgStanza({
				chatId: s,
				deviceList: u,
				metricReporter: c,
				msgProtobuf: d,
			...
			var b = yield o("WAWebDeprecatedSendIqWorkerCompatible").deprecatedSendStanzaAndReturnAck(C, o("WAWebCommsAckParser").toCoreAckTemplate({
				id: g.id,
				class: "message",
				from: h,
				participant: null
