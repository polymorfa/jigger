__d("WAWebSendScheduledMsgAction", [
	...
	function g() {
		return g = n("asyncToGeneratorRuntime").asyncToGenerator(function* (t, a, i, l, c) {
			if (l === void 0 && (l = {}), c === void 0 && (c = !0), !o("WAWebIsScheduledMessagesAvailableForChat").isScheduledMessagesAvailableForChat(t)) throw r("err")("[scheduled_msg] Scheduled messages not available for this chat");
			var d = o("WAWebStateUtils").unproxy(t), m = yield o("WAWebSendTextMsgChatAction").createTextMsgData(d, a, l);
			if (m != null) {
				if (yield o("WAWebScheduledMsgStore").isChatAtScheduleLimit(o("WAWebWidToJid").widToChatJid(d.id))) {
				...
				o("WALogger").LOG(e || (e = babelHelpers.taggedTemplateLiteralLoose([
					...
					" at ",
					""
				])), d.id.toLogString(), String(i));
				var _ = null;
				try {
					var f;
					yield o("WAWebLidMigrationFrontendUtils").validateMissingAccountLid(d, p, "addAndSendTextMsg");
					var g = new (o("WAWebMsgModel")).Msg(p), h = !!((f = d.groupMetadata) != null && f.isLidAddressingMode), y = o("WAWebMsgInfoUtils").getGroupMessageSendReporterOptions(d.id, o("WAWebWamMsgUtils").msgIsLid(p, d.id, h));
					g.wamMessageSendReporter = new (o("WAWebMessageSendReporter")).MessageSendReporter(g, babelHelpers.extends({}, y, { frontendDeps: o("WAWebMessageSendReporterFrontendDeps").MAIN_WEB_MESSAGE_SEND_REPORTER_FRONTEND_DEPS })), g.wamMessageSendPerfReporter = new (o("WAWebMessageSendPerfReporter")).MessageSendPerfReporter({
						chatWid: g.to,
						mediaType: o("WAWebWamMsgUtils").getWamMediaType(g),
						messageType: o("WAWebWamMsgUtils").getWamMessageType(g)
					}), yield o("WAWebOrchestratorNonPersistedJob").createNonPersistedJob("sendMessage", n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
						var e, t;
						(e = g.wamMessageSendPerfReporter) == null || e.startSavedStage(), yield o("WAWebDBProcessMessage").storeMessages([p], d.id), (t = g.wamMessageSendPerfReporter) == null || t.postSavedStage();
						var n = yield o("WAWebSendMsgRecordAction").sendMsgRecord(g);
				...
				if (_ === o("WAWebScheduledMsgConstants").SCHEDULED_MSG_RESOURCE_LIMIT_NACK_CODE && (yield o("WAWebScheduledMsgRevealKeyStore").updateRevealKeyStatus(p.id.toString(), "FAILED"), o("WAWebScheduledMsgLimitDialog.react").showScheduledMsgLimitReachedDialog()), c) try {
					yield C(d);
				} catch (e) {
	...
	function y() {
		return y = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, a, i, l) {
			if (i === void 0 && (i = {}), l === void 0 && (l = !0), !o("WAWebIsScheduledMessagesAvailableForChat").isScheduledMediaAvailableForChat(e)) throw r("err")("[scheduled_msg] Scheduled media not available for this chat");
			if (t.length !== 0) {
				var s = o("WAWebStateUtils").unproxy(e);
				if (yield o("WAWebScheduledMsgStore").isChatAtScheduleLimit(o("WAWebWidToJid").widToChatJid(s.id))) {
					o("WAWebScheduledMsgLimitDialog.react").showScheduledMsgLimitReachedDialog();
					return;
				}
				o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose([
					...
					" at ",
					""
				])), String(t.length), s.id.toLogString(), String(a));
				var u = s.composeQuotedMsg;
				s.composeQuotedMsg = null;
				var p = !1, f = [];
				for (var g of t.entries()) {
					var h = g[0], y = g[1], b = y.media, v = {
						type: b.type,
						caption: b.caption,
						mentionedJidList: y.mentionedJidList,
						groupMentions: y.groupMentions,
						addEvenWhilePreparing: b.previewable && b.state === o("WAWebAttachMediaModel").ATTACH_MEDIA_STATE.PROCESSING,
					...
					try {
						var S = yield b.sendToChat({
							chat: s,
							options: v
						});
						if (S.ackErrorCode === o("WAWebScheduledMsgConstants").SCHEDULED_MSG_RESOURCE_LIMIT_NACK_CODE) {
				...
				if (p && (yield (_ || (_ = n("Promise"))).all(f.map(function(e) {
					return o("WAWebScheduledMsgRevealKeyStore").updateRevealKeyStatus(e, "FAILED");
				})), o("WAWebScheduledMsgLimitDialog.react").showScheduledMsgLimitReachedDialog()), l) try {
					yield C(s);
				} catch (e) {
