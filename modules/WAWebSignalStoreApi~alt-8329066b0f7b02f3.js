__d("WAWebSignalStoreApi", [
	...
	var e, s = 1, u = (function() {
		...
		return n.getMeta = function(t) {
			...
			var e = this;
			if (this.idToSignedPreKey.has(t)) return Promise.resolve(this.idToSignedPreKey.get(t));
			...
			var e = o("WAWebSignalStorage").getSignedPreKeyTable(), n = t.map(function(t) {
				var n = t.keyId, o = t.keyPair, a = t.signature;
				return e.get(n).then(function(t) {
					if (t) throw r("err")("signed preKey id " + n + " already exists");
					var i = {
						keyId: n,
						keyPair: o,
						signature: a
			...
			var e = this;
			return this.registrationInfo != null ? Promise.resolve(this.registrationInfo) : Promise.all([
				...
				if (!(!n || !r || !a)) return Promise.all([o("WAWebSignalCommonUtils").decryptRegistrationMaterial(r), o("WAWebSignalCommonUtils").decryptRegistrationMaterial(a)]).then(function(t) {
					var r = t[0], o = t[1];
			...
			var e = this;
			await Promise.all([o("WAWebSignalCommonUtils").encryptRegistrationMaterial(t.identityKeyPair.pubKey), o("WAWebSignalCommonUtils").encryptRegistrationMaterial(t.identityKeyPair.privKey)]).then(function(n) {
				var r = n[0], a = n[1];
				return Promise.all([e.putMeta([
					{
			...
			return o("WAWebSignalStorageUtils").getStorage().lock(["signal-meta-store", "prekey-store"], function() {
				return Promise.all([t.getMeta(o("WAWebSignalConst").META_KEYS.FIRST_UNUPLOAD_PK_ID), t.getMeta(o("WAWebSignalConst").META_KEYS.NEXT_PK_ID)]).then(function(a) {
					var i = a[0], l = a[1], u = i || s, d = l || s, m = d - u, p = n - m;
					if (p <= 0) return o("WALogger").LOG(e || (e = babelHelpers.taggedTemplateLiteralLoose([
						"[getPreKeys] no gen needed avail=",
						" need=",
						""
					])), m, n), t.getPreKeysByRange(u, n);
			...
			if (t.length !== 0) {
				var e = t[t.length - 1];
				await Promise.all([this.putMeta([{
			...
			var e = this;
			return Promise.all([this.getMeta(o("WAWebSignalConst").META_KEYS.FIRST_UNUPLOAD_PK_ID), this.getMeta(o("WAWebSignalConst").META_KEYS.NEXT_PK_ID)]).then(function(n) {
				var a = n[0], i = n[1];
			...
			return o("WAWebSignalStorageUtils").getStorage().lock(["signal-meta-store", "signed-prekey-store"], function() {
				return e.getMeta(o("WAWebSignalConst").META_KEYS.LAST_SPK_ID).then(function(r) {
					var a = r == null || r + 1 >= o("WASignalKeys").PRE_KEY_NON_INCLUSIVE_UPPER_BORDER ? s : r + 1, i = n(t, a);
					return Promise.all([e.putSignedPreKeys([i]), e.putMeta([{
