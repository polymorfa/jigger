__d("WAWebSocketModel", [
	...
	var e, s, u, c, d, m, p, _, f, g, h, y, C, b, v, S, R, L, E, k, I, T, D, x, $, P, N, M, w, A, F, O = r("requireDeferred")("WAWebClearAppStates").__setRef("WAWebSocketModel"), B = [
		(F = o("WAWebLogoutReasonConstants")).LogoutReason.WebFailAddChat,
		F.LogoutReason.WebFailEncSalt,
		F.LogoutReason.WebFailOfflineResume,
		F.LogoutReason.WebFailStorageInitialization
	], W = (function(t) {
		function a() {
		...
		return i.initialize = function() {
			var t = this, n;
			this.clearAppStatesDeferred = O.load(), o("WAWebLogStorageSizeForCrash").initStorageSizeCrashLogging(), this.listenTo(this, "change:state", function() {
				...
				t.isIncognito = e;
			}), (n = o("WAWebBackendEventBus")).BackendEventBus.onSetSocketState(function(e) {
				t.state = e;
				...
				if (o("WALogger").LOG(u || (u = babelHelpers.taggedTemplateLiteralLoose(["[ws2] observed main_stream_mode_ready"]))), !await o("WAWebUserPrefsAppStateSync").getAllCriticalDataSynced()) {
			...
			if (r("WAWebEnvironment").isWindows) {
				var e, i = n("cr:17219") == null || (e = n("cr:17219").getWindowsBridge(r("WAWebWindowsHybridBridgeInitiator").WAWebSocketModel)) == null ? void 0 : e.getClientKeyBridge();
				if (i != null) try {
					await o("WAPromiseTimeout").promiseTimeout(i.clearClientKey(), 5e3);
				} catch (e) {
			...
			var l = !1;
			if (t != null && B.includes(t) && (l = !0), this.$SocketImpl$p_4(), l = await this.clearCredentials(), this.$SocketImpl$p_6()) {
				l = !0;
			...
			try {
				await o("WAWebSocketLogoutStorageUtils").destroyStorage();
			} catch (e) {
			...
			o("WAWebABPropsCache").clearABPropConfigs(), o("WAWebGroupABPropsCache").clearGroupABPropConfigs(), l || this.$SocketImpl$p_5(), o("WAWebReloadAfterLogout").reloadAfterLogout(l, t, a);
		}, i.logout = function(t) {
			var e = this;
 ...
 `,
			...
			var a = Promise.resolve(), i = 0, l = 20, s = Math.min(l, Math.max(i, o("WAWebABProps").getABPropConfigValue("syncd_sentinel_timeout_seconds")));
			a = o("WAPromiseTimeout").promiseTimeout(r("WAWebSentinel")(), s * 1e3).catch(function() {
				return o("WALogger").WARN(b || (b = babelHelpers.taggedTemplateLiteralLoose(["ws2: multi-device logout sentinel patch failed"])));
			});
			var u = 3;
			o("WAPromiseTimeout").promiseTimeout(a.then(function() {
				return o("WAWebSocketLogoutUtils").sendCurrentLogout(t);
			}).catch(async function(n) {
				o("WALogger").ERROR(v || (v = babelHelpers.taggedTemplateLiteralLoose(["ws2: [logout error] sendCurrentLogoutPromise errored"]))).catching(r("getErrorSafe")(n)), await e.clearCredentialsAndStoredData(t), o("WAWebBackendEventBus").BackendEventBus.triggerLogout();
			}), u * 1e3, "network request took more than " + u + " seconds").then(async function() {
				await e.clearCredentialsAndStoredData(t), e.state === o("WAWebSocketConstants").SOCKET_STATE.OPENING && e.trigger("change:state"), o("WAWebBackendEventBus").BackendEventBus.triggerLogout();
			}).catch(async function(n) {
				o("WALogger").ERROR(S || (S = babelHelpers.taggedTemplateLiteralLoose(["ws2: [logout] sentinel or sendCurrentLogout timed out"]))).catching(r("getErrorSafe")(n)), await e.clearCredentialsAndStoredData(t), o("WAWebBackendEventBus").BackendEventBus.triggerLogout();
			}).finally(function() {
			...
			try {
				await r("nullthrows")(await this.clearAppStatesDeferred)();
			} catch (n) {
				e = !0;
	...
	})(o("WAWebBaseModel").BaseModel), q = o("WAWebBaseModel").defineModel(W), U = new q();
	async function V() {
		var e = await Promise.allSettled([r("WAWebDeleteAllCacheStorage")(), o("WAWebMediaStore").LruMediaStore.clear()]);
