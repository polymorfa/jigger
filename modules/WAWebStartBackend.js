__d("WAWebStartBackend", [
	...
	var e, s, u, c, d, m, p, _, f, g, h, y, C, b, v, S, R, L, E = !1;
	function k() {
		o("WAWebBackendEventBus").BackendEventBus.onStorageInitializationError(n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			!o("WAWebUserPrefsMultiDevice").isRegistered() && o("WAWebLogoutReason").getPrevLogoutReasonCode() === o("WAWebLogoutReasonConstants").LOGOUT_REASON_CODE.CLIENT_FATAL || (o("WALogger").LOG(e || (e = babelHelpers.taggedTemplateLiteralLoose(["storage initialization error, logging out"]))), yield o("WAWebSocketModel").Socket.clearCredentialsAndStoredData(o("WAWebLogoutReasonConstants").LogoutReason.WebFailStorageInitialization), o("WAWebStartBackendLoginListeners").handleStorageInitError());
		...
		}), o("WAWebBackendEventBus").BackendEventBus.onceOfflineDeliveryEnd(n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			try {
				o("WALogger").LOG(s || (s = babelHelpers.taggedTemplateLiteralLoose(["[StartBackend]: start initial setup"]))), yield F(), o("WALogger").LOG(u || (u = babelHelpers.taggedTemplateLiteralLoose(["[StartBackend]: complete initial setup"])));
			} catch (e) {
				o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose(["[StartBackend] initial setup failed:"])));
			}
		})), o("WAWebMetaAiBotSessionCleanupJob").registerMetaAiBotSessionCleanupOnResume(), E = !0;
	}
	function I() {
	...
	function $() {
		return $ = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			E || k();
			var t = yield o("WAWebSignalStoreApi").waSignalStore.getRegistrationInfo();
			if (!t) {
			...
			if (o("WALogger").LOG(m || (m = babelHelpers.taggedTemplateLiteralLoose(["[startBackend]: connected as ", ""])), o("WAWebUserPrefsMeUser").getMeDeviceLidOrThrow()), r("WAWebEnvironment").isWindows) {
				...
				if (a != null) {
					...
					yield n("cr:17219") == null || (l = n("cr:17219").getWindowsBridge(r("WAWebWindowsHybridBridgeInitiator").WAWebStartBackend)) == null || (l = l.getClientKeyBridge()) == null ? void 0 : l.setClientKey(c), yield o("WAWebFtsClient").ftsClient.initExternalStorage(), r("WAWebLocalStorage") == null || r("WAWebLocalStorage").removeItem(o("WAWebUserPrefsKeys").KEYS.WINDOWS_PENDING_CLIENT_KEY_SETUP);
				}
			}
			try {
				yield o("WAWebApiContact").warmUpAllLidPnMappings(), yield o("WAPromiseTimeout").promiseTimeout(o("WAWebModelStorage").initialize(), 20 * 1e3), yield o("WAPromiseTimeout").promiseTimeout(o("WAWebOffdStorage").initialize(), 20 * 1e3), o("WAWebCrashlog").registerCrashlogUploadIsUserInExternalBetaFunction(o("WAWebUserPrefsGeneral").getWhatsAppWebExternalBetaJoinedIdb);
			} catch (e) {
				o("WAWebBackendEventBus").BackendEventBus.triggerStorageInitializationError(e);
			}
			o("WAWebInitWaitForEvents").initOfflineResumeWaitForEvents(), o("WAWebWamOfflineResumeReporter").OfflineResumeReporter.setLastPushCompleteTimestamp(), B(), yield o("WAWebPrimaryFeatures").loadPrimaryFeatures(), o("WAWebMediaHosts").mediaHosts.maybeLoadMediaConnFromStorage();
			...
			if (yield q(), M(), o("WAWebABPropsWamGlobals").setAbPropDependingGlobalWamAttributes(), o("WAWebL10NHelpers").isLocalLanguageOverrideEnabled() && (yield o("WAWebUserPrefsIndexedDBStorage").userPrefsIdb.set(o("WAWebUserPrefsKeys").KEYS.LANG_FROM_USER, o("WAWebUserPrefsLocales").getUserLangPref() || "en")), yield o("WAWebUserPrefsIndexedDBStorage").userPrefsIdb.set(o("WAWebUserPrefsKeys").KEYS.LANG_FROM_PHONE, o("WAWebUserPrefsLocales").getPhoneLangPref() || "en"), o("WAWebBackendApi").frontendFireAndForget("restoreTimeSpentSession", {}), o("WAWebMobilePlatforms").isSMB() && o("WAWebBootstrapBizBroadcastCampaigns").bootstrapBizBroadcastCampaigns().catch(function(e) {
			...
			}), o("WAWebMobilePlatforms").isSMB() && o("WAWebAuthAgentFeatureControlLifecycle").startAuthAgentFeatureControlLifecycle(), yield o("WAWebInitChatThreadLogging").initChatThreadLogging(), yield o("WAWebPersistedJobManager").startWebPersistedJobManager(), o("WAWebPersistedJobManagerWorkerCompatible").setInstance(o("WAWebPersistedJobManager").getJobManager()), o("WAWebOfflineResumeMainThread").setupMainThreadResume(o("WAWebOfflineHandler").OfflineMessageHandler), o("WAWebBuildConstants").WINDOWS_OFFLINE) {
				var b, v = yield n("cr:17219") == null || (b = n("cr:17219").getWindowsBridge(r("WAWebWindowsHybridBridgeInitiator").WAWebStartBackend)) == null || (b = b.serverEncKeySaltBridge) == null ? void 0 : b.getServerEncKeySalt();
				if (v == null) o("WALogger").ERROR(f || (f = babelHelpers.taggedTemplateLiteralLoose(["[_startBackend] encodedServerEncKeySalt is not available"])));
					...
					var S = new TextDecoder(), R = o("WABase64").decodeB64(v), L = S.decode(R);
					yield o("WAWebDbEncryptionKey").DbEncKeyStore.generateFinalDbEncryptionAndFtsKey(L), yield o("WAWebCryptoEncKeyHelper").generateFinalDbEncryptionAndFtsKeyForInvoker(L);
				}
				o("WAWebBackendEventBus").BackendEventBus.triggerOfflineDeliveryEnd(), yield o("WAWebDbEncryptionKey").DbEncKeyStore.waitForFinalDbMsgEncKey(), o("WAWebOfflineResumeUtils").loadMainScreen({});
			} else yield o("WAComms").startHandlingRequests(), self.setTimeout(function() {
	...
	function N() {
		return N = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			}), o("WAComms").startComms(r("WAWebCommsHandleStanza"), i, function(e) {
				return (L || (L = n("Promise"))).resolve(o("WAGzip").inflate(e));
			}, l), l && (yield o("WAComms").waitForConnection()), o("WAWebPageLoadLogging").endPageLoadQplMeasure("startWebComms");
	...
	function O() {
		return O = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			if (o("WAWebDBCreateLidPnMappings").flushLidPnMappingsToDb(), o("WAWebCallsOnlyGating").isCallsOnlyModeEnabled()) {
				o("WAWebTasksDefinitions").registerTasks(), yield o("WAWebSyncContactsJob").runSyncDirtyContactsJob(), o("WAWebInboxFiltersGatingUtils").inboxFavoritesEnabled() && o("WAWebInitFavoritesFromStorage").restoreFavorites(), o("WAWebSyncd").markCollectionsForSync([
	...
	function W() {
		return W = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			if (o("WAWebPushNotificationsGatingUtils").canSupportOfflineNotifications() && r("WAWebFeatureDetectionSwSupport").supported) {
				var e, t;
				(e = window.navigator.serviceWorker) != null && e.controller && (yield r("WAWebSWBus").request(window.navigator.serviceWorker.controller, r("WAWebSWBusActions").STOP_COMMS).catch(function(e) {
				...
				})), (t = window.navigator.serviceWorker) == null || (t = t.ready) == null || t.then((function() {
					var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
						var t, n = (t = yield e == null || e.getNotifications == null ? void 0 : e.getNotifications()) != null ? t : [];
	...
	function U() {
		return U = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			...
			}), o("WAWebOfflineResumeMsgProcessReporterWorkerCompatible").setInstance(o("WAWebOfflineResumeMsgProcessReporter").msgProcessReporter), yield P(e);
		}), U.apply(this, arguments);
	}
	l.setupStartBackendListeners = k, l.startBackend = T, l.startWebComms = P;
}), 98);
