__d("WAWebStorage", [
	...
	var e, s, u, c, d, m, p, _ = 1, f = 2, g = 4, h = { packColumns: !1 }, y = (function(e) {
		function t() {
		...
		return babelHelpers.inheritsLoose(t, e), t;
	})(babelHelpers.wrapNativeSuper(Error)), R = (function() {
		function t(e, t) {
			this.tables = new Map(), this.views = new Map(), this.state = _, this.versions = new (o("WAWebStorageVersionManager")).StorageVersionManager(), this.errorHandlers = new Map(), this.$1 = e, this.config = t != null ? t : h, this.$2();
		}
		var a = t.prototype;
		return a.addErrorHandler = function(t, n) {
			var e = new t().name;
			...
			if (this.tables.has(n)) throw new C("Table \"" + n + "\" already added. Reuse the definition.");
			...
			var e = this;
			return function() {
				if (e.state & (f | _)) throw new S("Storage must be initialized before accessing a table!");
				return t();
			};
		}, a.table = function(t) {
			var e = this, n = this.tables.get(t);
			if (!n || !n.tableExists()) throw new v("Unknown table " + t + " requested, ensure table is defined!");
			var o = this.$3();
			if (!o) throw new b("Unable to find associated database with table \"" + t + "\"");
			return new (r("WAWebIdbLazyView"))(this.$5(function() {
				return e.$4(n);
			}), n.name, o.getDbName());
		}, a.lock = function(t, r) {
			var e = this, o;
			if (this.state & (f | _)) return (p || (p = n("Promise"))).reject(new S("Initialize storage before attempting to lock tables!"));
			for (var a = 0; a < t.length; ++a) {
				var i;
				if (!(this.tables.has(t[a]) && (i = this.tables.get(t[a])) != null && i.tableExists())) return (p || (p = n("Promise"))).reject(new v("Requested lock of unknown table \"" + t[a] + "\""));
			}
			var l = t.map(function(t) {
				...
				return e.$4(t);
			}), u = this.$3().transact(t, n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				return r(s);
			}));
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				var e = this;
				try {
					return r("WAWebIndexedDB") == null ? (p || (p = n("Promise"))).resolve(null) : new (p || (p = (n("Promise"))))(function(t) {
					...
					return o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose(["[storage] failed to get existing db version"]))), (p || (p = n("Promise"))).resolve(null);
				}
			});
			function t() {
			...
			var e = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
				if (this.state & g) throw new y("Storage instance has already been initialized");
				if (this.versions.validate(), this.$1) yield this.$1.available();
				else throw new b("\"" + this.$1.constructor.name + "\" failed availability check!");
				...
				if (!(!e || !t)) {
					...
					if (o("WAWebRuntimeEnvironmentUtils").isWorker()) o("WALogger").LOG(u || (u = babelHelpers.taggedTemplateLiteralLoose([
						...
						" in worker db=",
						""
					])), this.versions.getMax(), t.getDbName()), yield this.$7(t, this.versions.getMax(), e), n = !1;
					...
					if (n) {
						...
						for (var i = 0; i <= this.versions.getMax(); i++) yield this.$7(t, i, e);
					}
			...
			return this.state = f, this.views = new Map(), this.$3().reset().then(function() {});
	...
	l.AlreadyInitializedError = y, l.NoSuchDatabaseError = b, l.NoSuchTableError = v, l.Storage = R;
}), 98);
