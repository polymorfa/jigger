__d("WAWebVoipSctpConnectionManager", [
	...
	"use strict";
	var e, s, u, c, d, m, p, _, f, g, h, y, C, b, v, S, R, L, E, k, I, T, D, x, $, P, N, M, w, A, F, O, B, W, q, U, V, H, G, z, j, K, Q, X, Y, J, Z, ee, te, ne, re, oe = 1e4, ae = 1e4;
	function ie() {
		return 2 * oe;
	}
	var le = !1, se = 0, ue = new Set();
	function ce(e) {
	...
	function de(e) {
		ue.delete(e) && o("WAWebCoreActionsODS").logCallSctpObsoleteRelayEvent("reconnect_succeeded");
	}
	function me(e) {
		ue.delete(e) && o("WAWebCoreActionsODS").logCallSctpObsoleteRelayEvent("reconnect_exhausted");
	}
	function pe() {
		var e = [];
	...
	function he() {
		return he = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			r && r.state !== o("WAWebVoipRelayConnectionUtils").ConnectionState.None && Ue(e.id), yield Oe(e, n);
	...
	function be(e) {
		...
		t.state === o("WAWebVoipRelayConnectionUtils").ConnectionState.Connecting && (t.state = o("WAWebVoipRelayConnectionUtils").ConnectionState.Open, t.stats.connectionReadyTime = Date.now(), t.isReconnecting = !1, de(e), o("WAWebVoipTransportFallbackTracker").notifySctpConnectionOpened(), o("WALogger").LOG(m || (m = babelHelpers.taggedTemplateLiteralLoose(["voip: [DCThread] Connection ", " state updated to Open (notified from pthread)"])), e), o("WAWebVoipSctpStatsInstrumentation").addConnectionSource("relay", pe, o("WAWebVoipSctpDataChannelThreadManager").getDataChannelThread), t.connectionTimeout && (window.clearTimeout(t.connectionTimeout), t.connectionTimeout = null), o("WAWebVoipTsLogger").logIceConnectionComplete({
			relayId: t.relayId,
			ip: t.relayIp,
			port: t.relayPort
		}), o("WAWebVoipSctpBufferDrain").drainBuffer(e), ke(e));
	}
	function ve(e, t, n, r) {
		var a, i = e.id;
		if (!(e.state === o("WAWebVoipRelayConnectionUtils").ConnectionState.Failed || e.state === o("WAWebVoipRelayConnectionUtils").ConnectionState.Closed)) {
			...
			if (!le && c != null && d < o("WAWebVoipSctpConnectionManagerConstants").MAX_SAME_PATH_RECONNECT_ATTEMPTS) {
				var m;
				u && !ue.has(i) && (ue.add(i), o("WAWebCoreActionsODS").logCallSctpObsoleteRelayEvent("transport_failed")), o("WAWebVoipSctpConnectionState").samePathReconnectAttempts.set(i, d + 1);
				...
				o("WALogger").LOG(p || (p = babelHelpers.taggedTemplateLiteralLoose([
					...
					", backoff=",
					"ms)"
				])), r, i, t, d + 1, o("WAWebVoipSctpConnectionManagerConstants").MAX_SAME_PATH_RECONNECT_ATTEMPTS, h), Ve(e, n, !0);
				var y = function(n) {
					if (n != null && o("WAWebVoipSctpConnectionState").pendingReconnectTimeouts.delete(n), !le) {
						var e = o("WAWebVoipSctpConnectionState").currentRelayState.get(i), a = e == null;
						a ? (ue.has(i) || (ue.add(i), o("WAWebCoreActionsODS").logCallSctpObsoleteRelayEvent("transport_failed")), o("WAWebCoreActionsODS").logCallSctpObsoleteRelayEvent("reconnect_attempted")) : ue.delete(i), ge(e != null ? e : c, !1).catch(function(e) {
							a && me(i), o("WALogger").ERROR(_ || (_ = babelHelpers.taggedTemplateLiteralLoose([
								"voip: ",
			...
			} else le || (c == null ? o("WALogger").LOG(f || (f = babelHelpers.taggedTemplateLiteralLoose([
				"voip: ",
				" No relay info for ",
				", cannot same-path reconnect (attempts=",
				")"
			])), r, i, d) : (u && me(i), o("WALogger").LOG(g || (g = babelHelpers.taggedTemplateLiteralLoose([
				"voip: ",
				" Max same-path reconnect attempts reached for ",
				" (attempts=",
				"/",
				")"
			])), r, i, d, o("WAWebVoipSctpConnectionManagerConstants").MAX_SAME_PATH_RECONNECT_ATTEMPTS))), Ve(e, t);
		}
	}
	function Se(e) {
		var t = e.id;
		o("WAWebVoipSctpConnectionState").sctpConnections.get(t) === e && ve(e, "data_channel_error", "data_channel_error_reconnecting", "[DCThread]");
	}
	function Re(e) {
		ve(e, "ice_connection_failed", "ice_connection_failed_reconnecting", "[SCTP]");
	}
	function Le(e, t) {
		var n = o("WAWebVoipSctpConnectionState").sctpConnections.get(e);
		if (n == null) {
		...
		if (o("WALogger").LOG(y || (y = babelHelpers.taggedTemplateLiteralLoose(["voip: [DCThread] Connection ", " errored (notified from pthread)"])), e), ce(t), (t === "no_first_response_timeout" || t === "rx_stall_timeout") && n.peerConnection != null) {
	...
	function Te() {
		return Te = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			le = !0, se++;
			try {
				...
				o("WALogger").LOG(A || (A = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Cleaning up ", " connections"])), e.length), yield o("WAWebVoipSctpDataChannelThreadManager").stopDataChannelWorker();
				for (var t of e) fe(t);
				o("WAWebVoipSctpConnectionState").currentRelayState.clear(), o("WAWebVoipTsLogger").cleanup(), o("WALogger").LOG(F || (F = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] All connections and relay state cleared"])));
			} finally {
				...
				o("WAWebVoipSctpConnectionState").connectionOpenedResolvers.clear(), o("WAWebVoipSctpConnectionState").samePathReconnectAttempts.clear(), ue.clear(), o("WAWebVoipRelayConnectQpl").resetVoipRelayConnectQpl(), le = !1;
			}
	...
	function xe() {
		return xe = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			var t = se;
			o("WALogger").LOG(O || (O = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Staggered creation: ", " connections"])), e.length), yield $e(e, 0, t);
	...
	function Pe() {
		return Pe = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t, r) {
	...
	function Me() {
		return Me = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			oe = r("justknobx")._("5402") || 1e4, ae = r("justknobx")._("5558") || oe, se++;
			var t = o("WAWebVoipGatingUtils").shouldUseOriginalRelayPort(), a = o("WAWebVoipRelayConnectionUtils").extractRelayConnectionMap(e, { portOverride: function(n) {
			...
			for (var i of o("WAWebVoipSctpConnectionState").currentRelayState) {
				var l = i[0], s = i[1];
				if (!a.has(l)) if (o("WAWebVoipSctpConnectionManagerConstants").SctpConnectionConfig.CLOSE_OLD_CONNECTION_BEFORE_CALL_END) fe(l);
				else {
					var u = o("WAWebVoipSctpConnectionState").sctpConnections.get(l);
					u != null && u.state !== o("WAWebVoipRelayConnectionUtils").ConnectionState.Failed && u.state !== o("WAWebVoipRelayConnectionUtils").ConnectionState.Closed && (u.relayConnectionInfo = s, o("WAWebCoreActionsODS").logCallSctpObsoleteRelayEvent("retained"));
				}
			}
			var c = [];
			for (var d of a) {
				var m = d[0], p = d[1];
				(!o("WAWebVoipSctpConnectionState").currentRelayState.has(m) || !o("WAWebVoipSctpConnectionState").sctpConnections.has(m)) && c.push(p);
			}
			...
			if (c.length > 0) {
				...
				h ? yield De(c) : yield (re || (re = n("Promise"))).all(c.map(function(e) {
					return ge(e, !1);
				}));
			}
	...
	function we(e, t, n) {
		n === void 0 && (n = !1), !n && (e.onopen = function(n) {
			...
			o("WALogger").ERROR(v || (v = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Data channel error for ", ":"])), t.id);
			var r = o("WAWebVoipSctpConnectionState").sctpConnections.get(t.id);
			if (r != null && r.channel === e) {
				var a = r;
				ce(), ve(a, "data_channel_error", "data_channel_error_reconnecting", "[SCTP]");
			}
		});
	}
	function Ae(e, t, n, r) {
		...
		var s = !1;
		return o("WAWebVoipRelayConnectionUtils").isDcTransferDisabled() ? o("WALogger").LOG(S || (S = babelHelpers.taggedTemplateLiteralLoose(["voip: [DCThread] DC transfer disabled for ", ""])), n.id) : s = _e(l, t, n.id, r), t.channel = l, we(l, n, s), l;
	}
	function Fe(e, t, n) {
		var a = n != null ? " " + n : "";
		e.oniceconnectionstatechange = function() {
			...
			r && (n === "connected" && (r.iceConnectedTime = Date.now(), o("WAWebVoipSctpOdsPortLogging").logCallIceConnectedForPort(r.relayPort), o("WAWebVoipSctpOdsPortLogging").logCallDtlsStartedForPort(r.relayPort), r.dtlsStallTimeout != null && window.clearTimeout(r.dtlsStallTimeout), r.dtlsStallTimeout = window.setTimeout(function() {
				...
				if (n !== "connected") {
					...
					o("WALogger").WARN(L || (L = babelHelpers.taggedTemplateLiteralLoose([
						...
						"",
						""
					])), i, oe, n, t, a), o("WAWebVoipSctpOdsPortLogging").logCallDtlsFailedStallForPort(r.relayPort), ve(r, "dtls_stall", "dtls_stall_reconnecting", "[SCTP]");
				}
			}, oe)), n === "failed" && (o("WAWebVoipSctpOdsPortLogging").logCallIceFailedForPort(r.relayPort), Re(r)));
			...
			if (i) {
				...
				if (n === "connected") {
					...
					s !== "" && r("justknobx")._("3110") && o("WAWebVoipStackInterface").getVoipStackInterface().then(function(e) {
						...
						o("WALogger").ERROR(I || (I = babelHelpers.taggedTemplateLiteralLoose(["voip: [SCTP] markRelayConnected failed for ", ""])), t).catching(r("getErrorSafe")(e)).sendLogs("voip_sctp_mark_relay_connected_failed");
					});
				}
	...
	function Be() {
		return Be = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e, t) {
			...
			n.connectionTimeout = window.setTimeout(function() {
				a.state === o("WAWebVoipRelayConnectionUtils").ConnectionState.Connecting && (o("WALogger").WARN(q || (q = babelHelpers.taggedTemplateLiteralLoose([
					"voip: [SCTP] Connection timeout (",
					"ms) in Connecting state for ",
					""
				])), ie(), e.id), Ve(a, "connection_timeout"));
			}, ie());
			var i = !1;
			if (o("WAWebVoipRelayConnectionUtils").isDcTransferDisabled() || (i = yield Qe()), le || (o("WAWebVoipTsLogger").logIceConnectionStart({
				relayId: e.relayId,
				ip: e.ip,
				port: e.port
			}), o("WAWebVoipSctpOdsPortLogging").logCallIceStartedForPort(e.port), yield o("WAWebReleaseToEventLoop").releaseToEventLoop(), le)) return Ue(e.id), ke(e.id), !1;
			if (qe(n)) return o("WALogger").WARN(U || (U = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Aborting stale connect for ", " after yield"])), e.id), o("WAWebVoipSctpConnectionTeardown").clearConnectionTimers(n), !1;
			try {
				var l, s, u = r("justknobx")._("404"), c = {};
				if (u && (c.certificates = [yield RTCPeerConnection.generateCertificate({
					name: "ECDSA",
					namedCurve: "P-256"
				})], qe(n))) return o("WALogger").WARN(V || (V = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Aborting stale connect for ", " after certificate generation"])), e.id), o("WAWebVoipSctpConnectionTeardown").clearConnectionTimers(n), !1;
				var d = Date.now(), m = new RTCPeerConnection(c), p = Date.now() - d;
				p > o("WAWebVoipSctpConnectionManagerConstants").SLOW_WEBRTC_SETUP_THRESHOLD_MS && o("WALogger").WARN(H || (H = babelHelpers.taggedTemplateLiteralLoose([
					"voip: [SCTP] slow RTCPeerConnection ctor ",
					": ",
					"ms"
				])), e.id, p), n.peerConnection = m, m.onicecandidate = function(t) {
					t.candidate || o("WALogger").LOG(G || (G = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] ICE gathering complete for ", ""])), e.id);
				}, Fe(m, e.id), Ae(m, n, e);
				var _ = Date.now(), f = yield m.createOffer();
				yield m.setLocalDescription(f);
				var g = f.sdp || "", h = o("WAWebVoipRelayConnectionUtils").createAnswerSdp(g, e);
				if (yield m.setRemoteDescription({
					sdp: h,
					type: "answer"
				}), qe(n)) return o("WALogger").WARN(z || (z = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Aborting stale connect for ", " after setRemoteDescription"])), e.id), o("WAWebVoipSctpConnectionTeardown").clearConnectionTimers(n), o("WAWebVoipSctpConnectionTeardown").closeConnectionDataChannel(n), o("WAWebVoipSctpConnectionTeardown").detachPeerConnectionHandlers(m), m.close(), n.peerConnection = null, !1;
				var y = Date.now() - _;
				y > o("WAWebVoipSctpConnectionManagerConstants").SLOW_WEBRTC_SETUP_THRESHOLD_MS && o("WALogger").WARN(j || (j = babelHelpers.taggedTemplateLiteralLoose([
				...
				return o("WALogger").LOG(K || (K = babelHelpers.taggedTemplateLiteralLoose([
					"voip: [SCTP] SDP done ",
					" DC=",
					" ICE=",
					""
				...
				return o("WALogger").ERROR(Q || (Q = babelHelpers.taggedTemplateLiteralLoose(["voip: [SCTP] createDataChannel failed: ", ""])), e), Ve(n, "channel_creation_failed"), !1;
			}
	...
	function We(e) {
		...
		o("WALogger").LOG(T || (T = babelHelpers.taggedTemplateLiteralLoose([
			...
			" rxB=",
			""
		])), i, e.id, r.sentPackets, r.receivedPackets, r.sentBytes, r.receivedBytes), o("WALogger").LOG(D || (D = babelHelpers.taggedTemplateLiteralLoose([
	...
	function qe(e) {
		return le || o("WAWebVoipSctpConnectionState").sctpConnections.get(e.id) !== e;
	}
	function Ue(e) {
		var t = o("WAWebVoipSctpConnectionState").sctpConnections.get(e);
		if (t) {
	...
	function Ve(e, t, n) {
		n === void 0 && (n = !1), e && e.state !== o("WAWebVoipRelayConnectionUtils").ConnectionState.Failed && e.state !== o("WAWebVoipRelayConnectionUtils").ConnectionState.Closed && (n || me(e.id), o("WALogger").LOG(x || (x = babelHelpers.taggedTemplateLiteralLoose([
			...
			ip: e.relayIp,
			port: e.relayPort
		}, 1), e.state = o("WAWebVoipRelayConnectionUtils").ConnectionState.Failed, Ue(e.id), ke(e.id));
	}
	function He(e) {
		var t = o("WAWebVoipSctpConnectionState").sctpConnections.get(e);
		t && (t.isReconnecting == null || !t.isReconnecting) && (t.state = o("WAWebVoipRelayConnectionUtils").ConnectionState.Closed, Ue(e), ke(e));
	}
	function Ge(e) {
		return ze.apply(this, arguments);
	}
	function ze() {
		return ze = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			if (e.isReconnecting !== !0) {
				if (o("WALogger").LOG(X || (X = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Restarting ICE process for connection ", ""])), e.id), !e.hasNonStunPacketSent) {
					o("WALogger").WARN(Y || (Y = babelHelpers.taggedTemplateLiteralLoose(["voip: [SCTP] ICE restart skip: no non-STUN sent ", ""])), e.id);
					return;
				}
				...
				if (!n) {
					o("WALogger").WARN(Z || (Z = babelHelpers.taggedTemplateLiteralLoose(["voip: [SCTP] ICE restart skip: no PC ", ""])), e.id);
					return;
				}
				...
				if (e.isReconnecting = !0, e.state = o("WAWebVoipRelayConnectionUtils").ConnectionState.Connecting, e.peerConnection) {
					o("WALogger").LOG(ee || (ee = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Closing previous connection for ", ""])), e.id), window.clearTimeout(e.connectionTimeout), e.connectionTimeout = null, o("WAWebVoipSctpConnectionTeardown").closeConnectionDataChannel(e);
				...
				try {
					...
					var l = {};
					r("justknobx")._("404") && (l.certificates = [yield RTCPeerConnection.generateCertificate({
						name: "ECDSA",
						namedCurve: "P-256"
					})]);
					var s = new RTCPeerConnection(l);
					e.peerConnection = s, e.iceConnectedTime = 0, e.dtlsStallTimeout != null && (window.clearTimeout(e.dtlsStallTimeout), e.dtlsStallTimeout = null), Fe(s, e.id, "(ICE restart)"), Ae(s, e, t, "ICE restart"), e.packetBuffer = a, o("WAWebVoipSctpConnectionState").sctpConnections.set(e.id, e);
					...
					yield s.setRemoteDescription({
						sdp: d,
						type: "answer"
					}), o("WALogger").LOG(te || (te = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] ICE restart completed for connection ", ""])), e.id);
				} catch (t) {
					o("WALogger").ERROR(ne || (ne = babelHelpers.taggedTemplateLiteralLoose([
						"voip: [SctpConnectionManager] ICE restart failed for connection ",
						": ",
						""
					])), e.id, t), Ve(e, "ice_restart_failed");
				}
			}
	...
	function je(e, t, n) {
		...
		if (r) {
			...
			if (r.channel == null || r.channel !== n) {
				o("WALogger").WARN($ || ($ = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] Ignoring stale DataChannel open for ", ""])), t);
				try {
					n.close();
				} catch (e) {
			...
			r.state = o("WAWebVoipRelayConnectionUtils").ConnectionState.Open, r.stats.connectionReadyTime = Date.now(), r.isReconnecting = !1, de(t), o("WAWebVoipTransportFallbackTracker").notifySctpConnectionOpened(), r.connectionTimeout && (window.clearTimeout(r.connectionTimeout), r.connectionTimeout = null), o("WAWebVoipSctpStatsInstrumentation").addConnectionSource("relay", pe, o("WAWebVoipSctpDataChannelThreadManager").getDataChannelThread);
			...
			o("WALogger").LOG(N || (N = babelHelpers.taggedTemplateLiteralLoose([
				...
				" ",
				"ms"
			])), t, s, l), o("WALogger").LOG(M || (M = babelHelpers.taggedTemplateLiteralLoose([
				...
				ip: r.relayIp,
				port: r.relayPort
			}), o("WAWebVoipSctpBufferDrain").drainBuffer(t), ke(t);
		}
	}
	function Ke(e, t) {
		var n = o("WAWebVoipSctpConnectionState").sctpConnections.get(t);
		n && (o("WALogger").LOG(w || (w = babelHelpers.taggedTemplateLiteralLoose(["voip: [SctpConnectionManager] DataChannel closed by relay for ", ", reconnecting"])), t), ce("remote_close"), ve(n, "remote_close", "remote_close_reconnecting", "[SCTP]"));
	}
	function Qe() {
	...
	l.sendWAWebVoipDataToRelay = ye, l.mergeWorkerStats = Ce, l.handleDataChannelOpened = be, l.handleDataChannelErrored = Le, l.cleanupAllConnections = Ie, l.handleRelayListUpdate = Ne, l.initDataChannelWorker = Qe;
}), 98);
