__d("WAWebWamMemoryStat", [
	...
	var e, s, u, c, d, m = self.performance.now(), p = !1, _ = 100, f = 0;
	function g() {
		var e;
	...
	function h() {
		...
		if (t) return E = Math.max(E, t.usedJSHeapSize), k += t.usedJSHeapSize, I++, C(t);
	}
	function y() {
		var e, t = (e = self.performance) == null ? void 0 : e.memory;
		if (t) return C(t);
	...
	var v = 5e3, S = 60, R = 2 * 1024, L = o("WAWebWamEnumWebcScenarioType").WEBC_SCENARIO_TYPE.IDLE, E = 0, k = 0, I = 0, T;
	function D(t) {
		try {
			var n;
			...
			if (t !== L) {
				if (L !== o("WAWebWamEnumWebcScenarioType").WEBC_SCENARIO_TYPE.IDLE) {
					var r;
					(r = T) == null || r.cancel(), T = null, A(), m = self.performance.now();
				}
				L = t, M();
			}
			T == null && N();
	...
	function N() {
		T = new (o("WAShiftTimer")).ShiftTimer(function() {
			var e = h();
			e != null && Math.abs(e.usedJsHeapSize - f) > _ && (f = e.usedJsHeapSize, o("WALogger").LOG(s || (s = babelHelpers.taggedTemplateLiteralLoose(["[MemoryStat] js heap size: ", "Gb"])), (e.usedJsHeapSize / 1024).toFixed(2))), L === o("WAWebWamEnumWebcScenarioType").WEBC_SCENARIO_TYPE.IDLE && I !== 0 && I % S === 0 && (A(), M()), N();
		}), T.onOrAfter(v);
	}
	function M() {
		E = 0, k = 0, I = 0;
	...
	function F() {
		return F = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e, t = yield (d || (d = n("Promise"))).all([q(), B()]), a = t[0], i = t[1], l = r("nullthrows")(h()), s = Math.round(k / I / 1e3 / 1e3), m = new (o("WAWebWebcMemoryStatWamEvent")).WebcMemoryStatWamEvent(babelHelpers.extends({}, l, {
				scenario: L,
				peakUsedJsHeapSize: Math.round(E / 1e3 / 1e3),
				usedJsHeapSize: s,
			...
			var _ = (s / 1024).toFixed(2), f = (l.usedJsHeapSize / 1024).toFixed(2), g = "average: " + _ + "Gb, current: " + f + "Gb";
			if (!p && s > R) {
				p = !0;
				var y = R / 1024;
				o("WALogger").WARN(u || (u = babelHelpers.taggedTemplateLiteralLoose([
					...
					"Gb: ",
					""
				])), y, g).sendLogs("memory-usage-exceeded-" + y + "gb", { sampling: .01 });
			} else {
				var C;
				o("WALogger").LOG(c || (c = babelHelpers.taggedTemplateLiteralLoose([
					...
					" (cores: ",
					")"
				])), g, (C = self.navigator) == null ? void 0 : C.hardwareConcurrency);
			}
	...
	function U() {
		return U = n("asyncToGeneratorRuntime").asyncToGenerator(function* () {
			var e = yield o("WAWebBackendApi").frontendSendAndReceive("getCollectionSizes");
			return {
	...
	l.isPerformanceMemoryApiSupported = g, l.getMemoryInfo = h, l.readMemoryInfo = y, l.getMemoryTableRows = b, l.setCurrentMemoryScenario = D, l.uploadMemoryIfChatWasOpened = $, l.uploadMemoryInfoOnChatClose = P, l.roundIntForMetrics = V;
}), 98);
