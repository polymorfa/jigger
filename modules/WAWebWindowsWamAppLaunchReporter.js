__d("WAWebWindowsWamAppLaunchReporter", [
	...
	function L() {
		return L = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			...
			if (!e) {
				...
				if (n) {
					var a = Date.now(), i = (t = o("WAWebWindowsHybridBridgeCommon").WAWebWindowsGetBridge()) == null ? void 0 : t.nativeAppStateBridge;
					if (i == null) {
						o("WALogger").WARN(s || (s = babelHelpers.taggedTemplateLiteralLoose(["onAppStateChanged: ignoring: bridge is null"])));
						return;
					}
					var l = i.detectNativeClockSkew();
					if (l != null && l > h) {
						o("WALogger").ERROR(u || (u = babelHelpers.taggedTemplateLiteralLoose(["commitEventAsync: large clock skew: ", ""])), l).sendLogs("native-clock-skew");
						return;
					}
					var d = i.getLastAppRestoreTimeStamp();
					if (d === 0) {
						o("WALogger").WARN(c || (c = babelHelpers.taggedTemplateLiteralLoose(["onAppStateChanged: ignoring: appRestoreT not available"])));
						return;
					}
					var m = a - d, p = new (o("WAWebAppLaunchWamEvent")).AppLaunchWamEvent({
	...
	function E() {
		...
		if (n == null) {
			o("WALogger").WARN(e || (e = babelHelpers.taggedTemplateLiteralLoose(["initializeEvent: ignoring: bridge is null"])));
			return;
		}
		return n.getEvents().on("appStateChanged", R), new (o("WAWebAppLaunchWamEvent")).AppLaunchWamEvent();
	}
	function k(e) {
	...
	function T() {
		return T = n("asyncToGeneratorRuntime").asyncToGenerator(function* (e) {
			try {
				...
				var r = Date.now(), a, i, l, s = o("WAWebBuildConstants").getWindowsVersion(), u = s != null && s >= y && o("WAWebABProps").getABPropConfigValue("web_anr_async_native_app_state_bridge_enabled") ? yield n.takeAppLaunchSnapshotAsync() : null;
				...
				if (L > 6e5) {
					...
					}, k = E(a), I = E(i), T = E(R), x = E(r), $ = E(self.performance.timing.fetchStart);
					o("WALogger").ERROR(_ || (_ = babelHelpers.taggedTemplateLiteralLoose([
						"commitEventAsync: appLaunchT large t=",
						" launch=",
						" restore=",
						""
					])), L, k, I).sendLogs("app_launch_time_is_large"), o("WALogger").ERROR(f || (f = babelHelpers.taggedTemplateLiteralLoose([
						"commitEventAsync: appLaunchT large start=",
						" end=",
						" fetch=",
						""
					])), T, x, $).sendLogs("app_launch_time_is_large");
				}
				e.appLaunchT = L, e.appLaunchTypeT = S(a, i), o("WAWebAppTracker").attachWAMAppContext(e, L), D(e), e.commit(), v = !0, C.resolve(!1);
			} catch (e) {
	...
	var x = new (o("WAWebWamBaseAppLaunchReporter")).BaseAppLaunchReporter(E, k);
